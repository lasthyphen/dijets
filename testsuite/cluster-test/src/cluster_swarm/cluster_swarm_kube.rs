...
use dijets_config::config::DEFAULT_JSON_RPC_PORT;
...
macro_rules! DIJETS_NODE_SERVICE_TEMPLATE {
    () => {
        "templates/dijets_node_service_template.yaml"
    };
}
macro_rules! DIJETS_NODE_SPEC_TEMPLATE {
    () => {
        "templates/dijets_node_spec_template.yaml"
    };
}
...
impl ClusterSwarmKube {
    pub async fn new() -> Result<Self> {
        ...
        dijets_retrier::retry_async(k8s_retry_strategy(), || {
            Box::pin(async move {
    ...
    fn service_spec(&self, peer_id: String) -> Result<Service> {
        let service_yaml = format!(
            include_str!(DIJETS_NODE_SERVICE_TEMPLATE!()),
            peer_id = &peer_id
        );
    ...
    fn dijets_node_spec(
        ...
        let pod_yaml = format!(
            include_str!(DIJETS_NODE_SPEC_TEMPLATE!()),
            pod_app = pod_app,
            pod_name = pod_name,
            image_tag = image_tag,
            node_name = node_name,
            ...
            + Sync,
    {
        ...
        let resource_names: Vec<String> = api
            .list(&ListParams {
                label_selector: Some("dijets-node=true".to_string()),
                ..Default::default()
            })
            .await?
            .iter()
    ...
    async fn config_fluentbit(&self, input_tag: &str, pod_name: &str, node: &str) -> Result<()> {
        ...
        let dir = "/opt/dijets/data/fluent-bit/";
        self.put_file(
            node,
            pod_name,
...
impl ClusterSwarm for ClusterSwarmKube {
    ...
    async fn clean_data(&self, node: &str) -> Result<()> {
        self.util_cmd("rm -rf /opt/dijets/data/*", node, "clean-data")
            .await
    }
...
fn k8s_retry_strategy() -> impl Iterator<Item = Duration> {
    dijets_retrier::exp_retry_strategy(1000, 5000, 30)
}
