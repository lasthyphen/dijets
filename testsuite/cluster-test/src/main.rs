...
use dijets_sdk::types::LocalAccount;
use futures::{
...
struct Args {
    ...
    #[structopt(long, help = "If set, tries to connect to a dijets-swarm instead of aws")]
    swarm: bool,
...
fn setup_log() {
    ...
    ::dijets_logger::Logger::new().is_async(true).init();
}
...
impl ClusterTestRunner {
    ...
    pub async fn setup(args: &Args) -> Result<Self> {
        ...
        let prometheus_ip = "dijets-testnet-prometheus-server.default.svc.cluster.local";
    ...
    fn get_changelog(&self, prev_commit: Option<&String>, upstream_commit: &str) -> String {
        let commits = self.github.get_commits("dijets/dijets", upstream_commit);
        match commits {
            Err(e) => {
