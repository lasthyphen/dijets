...
use dijets_logger::info;
use futures::future::try_join_all;
use std::{fs::File, io::Write, path::Path};
use structopt::StructOpt;

use consensus_types::safety_data::SafetyData;
use dijets_genesis_tool::layout::Layout;
use dijets_global_constants::{
    CONSENSUS_KEY, DIJETS_ROOT_KEY, EXECUTION_KEY, FULLNODE_NETWORK_KEY, GENESIS_WAYPOINT,
    OPERATOR_KEY, OWNER_KEY, SAFETY_DATA, TREASURY_COMPLIANCE_KEY, VALIDATOR_NETWORK_ADDRESS_KEYS,
    VALIDATOR_NETWORK_KEY, WAYPOINT,
};
use dijets_secure_storage::{CryptoStorage, KVStorage, Namespaced, Storage, VaultStorage};
use dijets_types::{chain_id::ChainId, network_address::NetworkAddress, waypoint::Waypoint};
use std::str::FromStr;

const VAULT_TOKEN: &str = "root";
const VAULT_PORT: u32 = 8200;
const DIJETS_ROOT_NS: &str = "val-0";
...
impl ClusterBuilder {
    ...
    pub async fn setup_cluster(
        ...
        let modules_dir = if let Some(modules_dir) = &params.move_modules_dir {
            ...
            // No modules specified on command line. Create a tmpdir and populate it with the Dijets genesis modules
            let mut tempdir = dijets_temppath::TempPath::new();
    ...
    pub async fn spawn_validator_and_fullnode_set(
        ...
        if !vault_nodes.is_empty() {
            ...
            try_join_all(vault_nodes.iter().enumerate().map(|(i, node)| async move {
                dijets_retrier::retry_async(dijets_retrier::fixed_retry_strategy(5000, 15), || {
    ...
    async fn initialize_vault(&self, validator_index: u32, vault_node: &KubeNode) -> Result<()> {
        ...
        tokio::task::spawn_blocking(move || {
            ...
            if validator_index == 0 {
                vault_storage.create_key(DIJETS_ROOT_KEY).map_err(|e| {
                    format_err!("Failed to create {}__{} : {}", pod_name, DIJETS_ROOT_KEY, e)
                })?;
                let key = vault_storage
                    .export_private_key(DIJETS_ROOT_KEY)
                    .map_err(|e| {
                        format_err!("Failed to export {}__{} : {}", pod_name, DIJETS_ROOT_KEY, e)
                    })?;
                vault_storage
            ...
            vault_storage
                ...
                .map_err(|e| format_err!("Failed to create {}/{} : {}", pod_name, WAYPOINT, e))?;
            dijets_network_address_encryption::Encryptor::new(vault_storage)
    ...
    async fn generate_genesis(
        ...
        let layout = Layout {
            owners: owners.clone(),
            operators: owners,
            dijets_root: DIJETS_ROOT_NS.to_string(),
            treasury_compliance: DIJETS_ROOT_NS.to_string(),
        };
        ...
        genesis_helper
            ...
            .await
            .map_err(|e| format_err!("Failed to dijets_root_key : {}", e))?;
        genesis_helper
            .treasury_compliance_key(
                VAULT_BACKEND,
                format!("http://{}:{}", vault_nodes[0].internal_ip, VAULT_PORT).as_str(),
        ...
        genesis_helper
            .extract_private_key(
                format!("{}__{}", DIJETS_ROOT_NS, DIJETS_ROOT_KEY).as_str(),
                "/tmp/mint.key",
                VAULT_BACKEND,
                format!("http://{}:{}", vault_nodes[0].internal_ip, VAULT_PORT).as_str(),
                token_path,
