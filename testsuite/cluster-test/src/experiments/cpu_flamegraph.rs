...
impl Experiment for CpuFlamegraph {
    ...
    async fn run(&mut self, context: &mut Context<'_>) -> Result<()> {
        ...
        let filename = "dijets-node-perf.svg";
...
fn generate_perf_flamegraph_command(filename: &str, run_id: &str, duration_secs: usize) -> String {
    format!(
        ...
        mkdir /tmp/perf-data;
        cd /tmp/perf-data;
        perf record -F 99 -p $(ps aux | grep dijets-node | grep -v grep | awk '{{print $2}}') --output=perf.data --call-graph dwarf -- sleep {duration_secs};
