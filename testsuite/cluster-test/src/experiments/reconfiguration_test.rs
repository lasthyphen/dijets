...
use dijets_client::Client;
use dijets_logger::prelude::*;
use dijets_operational_tool::json_rpc::JsonRpcClientWrapper;
...
impl Experiment for Reconfiguration {
    ...
    async fn run(&mut self, context: &mut Context<'_>) -> anyhow::Result<()> {
        ...
        for i in 0..self.count / 2 {
            let remove_txn = dijets_root_account.sign_with_transaction_builder(
            ...
            execute_and_wait_transactions(
                ...
                &mut dijets_root_account,
                vec![remove_txn],
            )
            .await?;
            version = expect_epoch(&full_node_client, version, (i + 1) * 2).await?;
            let add_txn = dijets_root_account.sign_with_transaction_builder(
        ...
        if self.count % 2 == 1 {
            let magic_number = 42;
            info!("Bump DijetsVersion to {}", magic_number);
            let update_txn = dijets_root_account.sign_with_transaction_builder(
                TransactionFactory::new(ChainId::test())
                    .update_dijets_version(allowed_nonce, magic_number),
            );
            execute_and_wait_transactions(
                &mut full_node_client,
                &mut dijets_root_account,
