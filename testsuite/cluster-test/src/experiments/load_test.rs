...
use dijets_config::{config::NodeConfig, network_id::NetworkId};
use dijets_logger::*;
use dijets_mempool::network::{MempoolNetworkEvents, MempoolNetworkSender};
use dijets_sdk::transaction_builder::TransactionFactory;
use dijets_time_service::TimeService;
use dijets_types::{account_config::dijets_root_address, chain_id::ChainId};
    ...
    async fn run(&mut self, context: &mut Context<'_>) -> anyhow::Result<()> {
        ...
        if let Some(j) = emit_job {
            ...
            let mut sender = context
                .tx_emitter
                .load_dijets_root_account(&full_node_client)
                .await?;
            let receiver = dijets_root_address();
...
// An actor that can participate in DijetsNet
...
// that interact with the remote VFN via DijetsNet mempool and state sync protocol
struct StubbedNode {
...
impl StubbedNode {
    async fn launch(node_endpoint: String, runtime_handle: Handle, index: usize) -> Self {
        ...
        let mempool_handle = Some(network_builder.add_protocol_handler(
            dijets_mempool::network::network_endpoint_config(
...
async fn mempool_load_test(
    ...
    while Instant::now().duration_since(task_start) < duration {
        let msg = dijets_mempool::network::MempoolSyncMsg::BroadcastTransactionsRequest {
