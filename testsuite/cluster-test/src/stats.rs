...
impl<'a> PrometheusRangeView<'a> {
    ...
    pub fn avg_txns_per_block(&self) -> Option<f64> {
        self.query_avg(
            "txn_per_block",
            "irate(dijets_consensus_num_txns_per_block_sum[1m])/irate(dijets_consensus_num_txns_per_block_count[1m])".to_string(),
        )
    }
    ...
    pub fn avg_backup_bytes_per_second(&self) -> Option<f64> {
        self.query_avg(
            ...
            "sum(irate(dijets_backup_service_sent_bytes[1m])) by(peer_id)".to_string(),
        )
    }
}
