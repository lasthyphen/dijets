...
use dijets_genesis_tool::{command::Command, layout::Layout};
use dijets_management::{error::Error, secure_backend::DISK};
use dijets_operational_tool::command::Command as OperationalCommand;
use dijets_types::{
...
impl GenesisHelper {
    ...
    pub async fn dijets_root_key(
        ...
        let command = Command::from_iter(args.split_whitespace());
        spawn_blocking(|| command.dijets_root_key())
            .await
            .expect("tokio spawn_blocking runtime error")
    }
