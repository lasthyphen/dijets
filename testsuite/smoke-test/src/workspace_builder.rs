...
const WORKSPACE_BUILD_ERROR_MSG: &str = r#"
    ...
    Try running 'cargo build --all --bins --exclude cluster-test --exclude dijets-node' yourself.
"#;
...
static WORKSPACE_BUILT: Lazy<bool> = Lazy::new(|| {
    ...
    let args = if cfg!(debug_assertions) {
        ...
        // use get_dijets_node_with_failpoints to get dijets-node binary
        vec![
            "build",
            "--all",
            "--bins",
...
static DIJETS_NODE: Lazy<bool> = Lazy::new(|| {
    let args = vec!["build", "--features", "failpoints"];
    let mut path = workspace_root();
    path.push("dijets-node/");
    info!("Building dijets-node binary with failpoints");
    let cargo_build = Command::new("cargo")
        .current_dir(path)
        .args(&args)
        .output()
        .expect("Failed to build dijets node");
    if cargo_build.status.success() {
        info!("Finished building dijets-node with failpoints");
        true
    } else {
        error!("Output: {:?}", cargo_build);
        false
...
pub fn get_dijets_node_with_failpoints() -> PathBuf {
    if !*DIJETS_NODE {
        panic!("Failed to build dijets node with failpoints");
    }
    let bin_path = build_dir().join(format!("{}{}", "dijets-node", env::consts::EXE_SUFFIX));
    if !bin_path.exists() {
        panic!(
            "Can't find binary dijets-node in expected path {:?}",
            bin_path
        );
    }
