...
use dijets_crypto::{x25519, x25519::PrivateKey};
use dijets_operational_tool::{
    keys::{EncodingType, KeyType},
    test_helper::OperationalTool,
};
use dijets_swarm::swarm::{modify_network_config, DijetsNode, DijetsSwarm};
...
fn inbound_connected_peers(swarm: &mut DijetsSwarm, node_index: usize, network_id: NetworkId) -> i64 {
    swarm
        .mut_node(node_index)
        .unwrap()
