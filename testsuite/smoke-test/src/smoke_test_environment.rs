...
use crate::{test_utils::dijets_swarm_utils::get_client_proxy, workspace_builder};
use cli::client_proxy::ClientProxy;
use dijets_config::config::NodeConfig;
use dijets_crypto::ed25519::Ed25519PrivateKey;
use dijets_genesis_tool::{config_builder::FullnodeType, swarm_config::SwarmConfig};
use dijets_infallible::Mutex;
...
impl SmokeTestEnvironment {
    pub fn new_with_chunk_limit(num_validators: usize, chunk_limit: u64) -> Self {
        ...
        let mnemonic_file = dijets_temppath::TempPath::new();
        mnemonic_file
            .create_as_file()
            .expect("could not create temporary mnemonic_file_path");

        let key = generate_key::load_key(&validator_swarm.config.dijets_root_key_path);
    ...
    pub fn setup_vfn_swarm(&mut self) {
        ...
        let swarm = DijetsSwarm::configure_fn_swarm(
            "ValidatorFullNode",
            &workspace_builder::get_dijets_node_with_failpoints(),
            None,
            None,
    ...
    pub fn add_fn_swarm(&mut self, swarm_key: FullNodeSwarmType, fn_swarm: DijetsSwarm) {
        if self.fn_swarms.contains_key(&swarm_key) {
            panic!("Already setup full node {:?} swarm", swarm_key);
        }
        self.fn_swarms
    ...
    pub fn fn_swarm(&self, fn_type: FullNodeSwarmType) -> Option<Arc<Mutex<DijetsSwarm>>> {
        self.fn_swarms.get(&fn_type).cloned()
    }

    pub fn vfn_swarm(&self) -> Arc<Mutex<DijetsSwarm>> {
        self.fn_swarm(FullNodeSwarmType::Validator)
            .expect("VFN swarm is not initialized")
    }

    pub fn public_swarm(&self, name: &'static str) -> Arc<Mutex<DijetsSwarm>> {
