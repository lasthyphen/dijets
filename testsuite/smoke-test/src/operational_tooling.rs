...
use crate::{
    ...
    test_utils::{
        dijets_swarm_utils::{
            get_json_rpc_dijets_interface, get_op_tool, load_backend_storage, load_dijets_root_storage,
            load_node_config,
        },
...
use dijets_key_manager::dijets_interface::DijetsInterface;
use dijets_management::storage::to_x25519;
use dijets_operational_tool::{
    keys::{EncodingType, KeyType},
    test_helper::OperationalTool,
};
use dijets_sdk::client::views::VMStatusView;
...
fn create_operator_with_file_writer(file_writer: fn(&Ed25519PublicKey, PathBuf)) {
    ...
    let dijets_json_rpc = get_json_rpc_dijets_interface(&env.validator_swarm, 0);
    dijets_json_rpc
    ...
    let backend = load_dijets_root_storage(&env.validator_swarm, 0);
    ...
    let account_state = dijets_json_rpc
        .retrieve_account_state(operator_account)
        .unwrap();
...
fn set_operator_and_add_new_validator_helper() -> VMStatusView {
    ...
    let dijets_backend = load_dijets_root_storage(&env.validator_swarm, 0);
    let val_human_name = "new_validator";
    let (txn_ctx, _) = op_tool
        .create_validator(
            val_human_name,
    ...
    let txn_ctx = op_tool
        .add_validator(validator_account, &dijets_backend, true)
        .unwrap();
    ...
    let txn_ctx = op_tool
        .add_validator(validator_account, &dijets_backend, false)
        .unwrap();
