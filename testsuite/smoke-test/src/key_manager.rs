...
use crate::{
    ...
    test_utils::dijets_swarm_utils::load_backend_storage,
};
use dijets_config::config::NodeConfig;
use dijets_global_constants::CONSENSUS_KEY;
use dijets_key_manager::{
    dijets_interface::{DijetsInterface, JsonRpcDijetsInterface},
    KeyManager,
};
use dijets_secure_storage::{CryptoStorage, Storage};
use dijets_smoke_test_attribute::smoke_test;
...
fn test_key_manager_consensus_rotation() {
    ...
    let dijets_interface = JsonRpcDijetsInterface::new(json_rpc_endpoint.clone());
    ...
    let validator_info = dijets_interface.retrieve_validator_info(account).unwrap();
