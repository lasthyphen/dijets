...
fn test_startup_sync_state() {
    ...
    let state_db_path = node_config.storage.dir().join("dijetsdb");
...
fn test_state_sync_multichunk_epoch() {
    ...
    let dijets_framework_dir = dijets_framework::dijets_stdlib_modules_full_path();
    let script_params = &[
        "compile",
        unwrapped_script_path,
        move_stdlib_dir.as_str(),
        dijets_framework_dir.as_str(),
    ];
