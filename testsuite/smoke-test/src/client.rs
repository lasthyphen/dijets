...
use crate::{
    ...
    test_utils::{
        compare_balances,
        dijets_swarm_utils::{
            get_op_tool, load_dijets_root_storage, load_node_config, save_node_config,
        },
...
use dijets_json_rpc_types::Id;
use dijets_sdk::client::stream::{
    request::StreamMethod, response::StreamJsonRpcResponseView, StreamingClientConfig,
};
use dijets_types::{ledger_info::LedgerInfo, waypoint::Waypoint};
...
fn test_client_waypoints() {
    ...
    let _ = op_tool
        .remove_validator(peer_id, &dijets_root, false)
        .unwrap();
