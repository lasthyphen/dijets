...
use crate::{
    test_utils::{
        compare_balances,
        dijets_swarm_utils::{insert_waypoint, load_node_config, save_node_config},
...
use dijets_types::{transaction::Version, waypoint::Waypoint};
use rand::random;
use std::{
    fs,
    path::Path,
...
fn transfer_and_reconfig(
    ...
    while !quit.load(Ordering::Relaxed) {
        if random::<u16>() % 10 == 0 {
            println!(
                "Changing dijets version to {}: {:?}",
                transferred,
                client
                    .change_dijets_version(&["change_dijets_version", &transferred.to_string()], true)
            );
        }
