...
use dijets_config::config::{Identity, NodeConfig, SecureBackend};
use dijets_crypto::ed25519::Ed25519PublicKey;
...
/// DijetsSwarm instances. It is particularly useful for working with tests that
/// require a SmokeTestEnvironment, as it provides a generic interface across
/// DijetsSwarms, regardless of if the swarm is a validator swarm, validator full
/// node swarm, or a public full node swarm.
pub mod dijets_swarm_utils {
    use crate::test_utils::fetch_backend_storage;
    use cli::client_proxy::ClientProxy;
    use dijets_config::config::{NodeConfig, SecureBackend, WaypointConfig};
    use dijets_key_manager::dijets_interface::JsonRpcDijetsInterface;
    use dijets_operational_tool::test_helper::OperationalTool;
    use dijets_secure_storage::{KVStorage, Storage};
    use dijets_swarm::swarm::DijetsSwarm;
    use dijets_transaction_replay::DijetsDebugger;
    use dijets_types::{chain_id::ChainId, waypoint::Waypoint};
    ...
    /// node index.
    pub fn get_json_rpc_url(swarm: &DijetsSwarm, node_index: usize) -> String {
        format!("http://127.0.0.1:{}", swarm.get_client_port(node_index))
    }

    /// Returns a JSON RPC based Dijets Interface pointing to a node at the given
    /// node index.
    pub fn get_json_rpc_dijets_interface(
        swarm: &DijetsSwarm,
        node_index: usize,
    ) -> JsonRpcDijetsInterface {
        let json_rpc_endpoint = format!("http://127.0.0.1:{}", swarm.get_client_port(node_index));
        JsonRpcDijetsInterface::new(json_rpc_endpoint)
    }

    /// Returns a Dijets Debugger pointing to a node at the given node index.
    pub fn get_dijets_debugger(swarm: &DijetsSwarm, node_index: usize) -> DijetsDebugger {
        ...
        DijetsDebugger::json_rpc(swarm_rpc_endpoint.as_str()).unwrap()
    }

    /// Returns an operational tool pointing to a validator node at the given node index.
    pub fn get_op_tool(swarm: &DijetsSwarm, node_index: usize) -> OperationalTool {
        OperationalTool::new(
            format!("http://127.0.0.1:{}", swarm.get_client_port(node_index)),
            ChainId::test(),
        )
    ...
    pub fn load_backend_storage(swarm: &DijetsSwarm, node_index: usize) -> SecureBackend {
        let (node_config, _) = load_node_config(swarm, node_index);
        fetch_backend_storage(&node_config, None)
    }

    /// Loads the dijets root's storage backend identified by the node index in the given swarm.
    pub fn load_dijets_root_storage(swarm: &DijetsSwarm, _node_index: usize) -> SecureBackend {
    ...
    /// config path.
    pub fn load_node_config(swarm: &DijetsSwarm, node_index: usize) -> (NodeConfig, PathBuf) {
    ...
    pub fn save_node_config(node_config: &mut NodeConfig, swarm: &DijetsSwarm, node_index: usize) {
        let node_config_path = swarm.config.config_files.get(node_index).unwrap();
    ...
    pub fn insert_waypoint(node_config: &mut NodeConfig, waypoint: Waypoint) {
        let f = |backend: &SecureBackend| {
            ...
            storage
                .set(dijets_global_constants::WAYPOINT, waypoint)
                .expect("Unable to write waypoint");
            storage
                .set(dijets_global_constants::GENESIS_WAYPOINT, waypoint)
                .expect("Unable to write waypoint");
        };
