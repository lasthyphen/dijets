...
use crate::test_utils::{dijets_swarm_utils::get_json_rpc_url, setup_swarm_and_client_proxy};
use dijets_framework_releases::current_modules_with_blobs;
use dijets_types::{
    chain_id::ChainId, on_chain_config::DIJETS_MAX_KNOWN_VERSION, transaction::TransactionPayload,
};
use dijets_validator_interface::{DijetsValidatorInterface, JsonRpcDebuggerInterface};
use dijets_writeset_generator::{
...
fn test_move_release_flow() {
    ...
    // Commit the release
    client
        .association_transaction_with_local_dijets_root_account(
    ...
    let remote_modules = validator_interface
        .get_dijets_framework_modules_by_version(latest_version)
        .unwrap();
    ...
    let payload_2 = create_release(
        ...
        false,
        &old_modules,
        Some(DIJETS_MAX_KNOWN_VERSION.major + 1),
        "",
    )
    .unwrap();
    // Verifying the generated payload against release modules should pass.
    ...
    assert_eq!(
        client.client.get_metadata().unwrap().dijets_version,
        Some(DIJETS_MAX_KNOWN_VERSION.major + 1)
    );
