...
use crate::{
    ...
    test_utils::dijets_swarm_utils::{
        get_op_tool, insert_waypoint, load_dijets_root_storage, load_node_config, save_node_config,
    },
    workspace_builder,
...
use dijets_transaction_builder::stdlib::encode_remove_validator_and_reconfigure_script;
use dijets_types::{
...
/// 2. Test the db-bootstrapper applying a manual genesis transaction (remove validator 0) on dijetsdb directly
...
fn test_genesis_transaction_flow() {
    ...
    let dijets_root = load_dijets_root_storage(&env.validator_swarm, 0);
    let config = op_tool
        .validator_config(validator_address, Some(&dijets_root))
        .unwrap();
    ...
    let genesis_transaction = Transaction::GenesisTransaction(WriteSetPayload::Script {
        execute_as: dijets_root_address(),
    ...
    client_proxy_1
        .change_dijets_version(&["change_dijets_version", "12345"], true)
        .unwrap();
    ...
    let _ = op_tool
        .add_validator(
            validator_address,
            &load_dijets_root_storage(&env.validator_swarm, 0),
            false,
        )
        .unwrap();
