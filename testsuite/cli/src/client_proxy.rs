...
use dijets_logger::prelude::{error, info};
use dijets_resource_viewer::{AnnotatedAccountStateBlob, DijetsValueAnnotator};
use dijets_temppath::TempPath;
use dijets_transaction_builder::stdlib as transaction_builder;
use dijets_types::{
    access_path::AccessPath,
    account_address::AccountAddress,
    account_config::{
        dijets_root_address, from_currency_code_string, testnet_dd_account_address,
...
use dijets_wallet::{io_utils, WalletLibrary};
...
pub struct ClientProxy {
    /// chain ID of the Dijets network this client is interacting with
    pub chain_id: ChainId,
    /// client for admission control interface.
    pub client: DijetsClient,
    /// Created accounts.
    ...
    faucet_url: Url,
    /// Account used for Dijets Root operations (e.g., adding a new transaction script)
    pub dijets_root_account: Option<AccountData>,
...
impl ClientProxy {
    ...
    pub fn new(
        ...
        let client = DijetsClient::new(url.clone(), waypoint)?;

        let accounts = vec![];

        let dijets_root_account = if dijets_root_account_file.is_empty() {
            None
        } else {
            let dijets_root_account_key = generate_key::load_key(dijets_root_account_file);
            let dijets_root_account_data = Self::get_account_data_from_address(
                &client,
                dijets_root_address(),
                true,
                Some(KeyPair::from(dijets_root_account_key)),
                None,
            )?;
            Some(dijets_root_account_data)
        };
        ...
        Ok(ClientProxy {
            ...
            wallet: Self::get_dijets_wallet(mnemonic_file)?,
    ...
    /// Gets a websocket client for the same node `DijetsClient` connects to
    ...
    fn get_account_data(&self, address: &AccountAddress) -> Result<&AccountData> {
        if let Some(account) = &self.dijets_root_account {
    ...
    pub fn print_all_accounts(&self) {
        ...
        if let Some(dijets_root_account) = &self.dijets_root_account {
            println!(
                "AssocRoot account address: {}, sequence_number: {}, status: {:?}",
                hex::encode(&dijets_root_account.address),
                dijets_root_account.sequence_number,
                dijets_root_account.status,
            );
        }
    ...
    pub fn enable_custom_script(
        ...
        let script_body = {
            let code = format!(
                "
                import 0x1.DijetsTransactionPublishingOption;

                main(account: signer) {{
                    DijetsTransactionPublishingOption.set_open_script(&account);
                    {}
            ...
            ",
                if open_module {
                    "DijetsTransactionPublishingOption.set_open_module(&account, true);"
                } else {
                    ""
                }
            );
            ...
            let compiler = Compiler {
                address: dijets_types::account_config::CORE_CODE_ADDRESS,
        ...
        match self.dijets_root_account {
            Some(_) => self.association_transaction_with_local_dijets_root_account(
    ...
    /// Modify the stored DijetsVersion on chain.
    pub fn change_dijets_version(
        &mut self,
        space_delim_strings: &[&str],
        is_blocking: bool,
    ) -> Result<()> {
        ensure!(
            space_delim_strings[0] == "change_dijets_version" || space_delim_strings[0] == "v",
            "inconsistent command '{}' for change_dijets_version",
            space_delim_strings[0]
        );
        ensure!(
            space_delim_strings.len() == 2,
            "Invalid number of arguments for changing dijets_version"
        );
        match self.dijets_root_account {
            Some(_) => self.association_transaction_with_local_dijets_root_account(
                TransactionPayload::Script(transaction_builder::encode_update_dijets_version_script(
                    self.dijets_root_account.as_ref().unwrap().sequence_number,
    ...
    pub fn compile_program(&mut self, space_delim_strings: &[&str]) -> Result<Vec<String>> {
        ...
        for (name, addr) in dijets_framework::dijets_framework_named_addresses() {
    ...
    /// Submit a writeset transaction signed by local dijets root account.
    pub fn submit_writeset(&mut self, space_delim_strings: &[&str]) -> Result<()> {
        ...
        self.association_transaction_with_local_dijets_root_account(payload, true)
    }
    ...
    fn get_annotate_account_blob(
        ...
        if let Some(account_blob) = blob {
            ...
            for (blob, module) in dijets_framework_releases::current_modules_with_blobs() {
                storage.publish_or_overwrite_module(module.self_id(), blob.clone())
            }
            let annotator = DijetsValueAnnotator::new(&storage);
            let annotate_blob =
    ...
    fn update_account_seq(&mut self, address: &AccountAddress, seq: u64) {
        if let Some(dijets_root_account) = &mut self.dijets_root_account {
            if &dijets_root_account.address == address {
                dijets_root_account.sequence_number = seq;
            }
        }
    ...
    fn get_dijets_wallet(mnemonic_file: Option<String>) -> Result<WalletLibrary> {
    ...
    /// Send a transaction signed by the local dijets_root credential
    pub fn association_transaction_with_local_dijets_root_account(
        &mut self,
        payload: TransactionPayload,
        is_blocking: bool,
    ) -> Result<()> {
        ensure!(
            self.dijets_root_account.is_some(),
            "No assoc root account loaded"
        );
        let sender = self.dijets_root_account.as_ref().unwrap();
    ...
    pub fn convert_to_scaled_representation(
        ...
        ensure!(!input.is_empty(), "Empty input not allowed for dijets unit");
...
fn encode_stdlib_upgrade_transaction() -> ChangeSet {
    ...
    for module in dijets_framework::modules() {
        let mut bytes = vec![];
...
mod tests {
    ...
    #[test]
    fn test_micro_dijets_conversion() {
        ...
        // Max of micro dijets is u64::MAX (18446744073709551615).
        assert!(ClientProxy::convert_to_scaled_representation(
    ...
    proptest! {
        ...
        #[test]
        fn test_micro_dijets_conversion_random_string(req in any::<String>()) {
            let _res = ClientProxy::convert_to_scaled_representation(&req, 1_000_000, 1_000_000);
        }
        #[test]
        fn test_micro_dijets_conversion_random_f64(req in any::<f64>()) {
            let req_str = req.to_string();
            let _res = ClientProxy::convert_to_scaled_representation(&req_str, 1_000_000, 1_000_000);
        }
        #[test]
        fn test_micro_dijets_conversion_random_u64(req in any::<u64>()) {
            let req_str = req.to_string();
            let _res = ClientProxy::convert_to_scaled_representation(&req_str, 1_000_000, 1_000_000);
        }
    }
