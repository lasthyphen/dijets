...
#[structopt(
    ...
    about = "Dijets client to connect to a specific validator"
)]
struct Args {
    ...
    /// If not passed, a new mnemonic file will be generated by dijets-wallet in the current
    /// directory.
...
fn main() {
    ...
    if !args.verbose {
        logger.level(::dijets_logger::Level::Warn);
    }
    logger.init();
    ...
    loop {
        let readline = rl.readline("dijets% ");
        match readline {
            Ok(line) => {
