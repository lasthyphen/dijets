...
//! The following document is a minimalist version of Dijets Wallet. Note that this Wallet does
...
//! foundation that is simple to understand and incrementally improve the DijetsWallet
...
impl WalletLibrary {
    ...
    pub fn generate_addresses(&mut self, depth: u64) -> Result<()> {
        ...
        if current > depth {
            return Err(WalletError::DijetsWalletGeneric(
    ...
    pub fn new_address(&mut self) -> Result<(AuthenticationKey, ChildNumber)> {
            ...
            .is_none()
        {
            ...
            Err(WalletError::DijetsWalletGeneric(
    ...
    pub fn get_addresses(&self) -> Result<Vec<AccountAddress>> {
        ...
        for i in 0..self.addr_map.len() as u64 {
            match rev_map.get(&i) {
                ...
                None => {
                    return Err(WalletError::DijetsWalletGeneric(format!(
    ...
    /// Simple public function that allows to sign a Dijets RawTransaction with the PrivateKey
    ...
    pub fn get_private_key(&self, address: &AccountAddress) -> Result<Ed25519PrivateKey> {
        if let Some(child) = self.addr_map.get(address) {
            ...
            Err(WalletError::DijetsWalletGeneric("missing address".to_string()).into())
        }
    }
