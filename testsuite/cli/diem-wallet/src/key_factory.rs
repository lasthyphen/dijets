...
//! DijetsWallet.
//!
//! Note that the Dijets Blockchain makes use of ed25519 Edwards Digital Signature Algorithm
//! (EdDSA) and therefore, BIP32 Public Key derivation is not available without falling back to
//! a non-deterministic Schnorr signature scheme. As DijetsWallet is meant to be a minimalist
...
use dijets_types::{account_address::AccountAddress, transaction::authenticator::AuthenticationKey};
use hmac::Hmac;
...
impl ExtendedPrivKey {
    ...
    /// ChildNumber are not used in this iteration of DijetsWallet, but in order to
    ...
    pub fn get_address(&self) -> AccountAddress {
        dijets_types::account_address::from_public_key(&self.get_public())
    }
    ...
    /// Dijets specific sign function that is capable of signing an arbitrary
    /// Serializable value.
    ///
    /// NOTE: In Dijets, we do not sign the raw bytes of a transaction, but
    ...
    /// described in `dijets_crypto::hash`.
    ///
...
impl KeyFactory {
    const MNEMONIC_SALT_PREFIX: &'static [u8] = b"DIJETS WALLET: mnemonic salt prefix$";
    const MAIN_KEY_SALT: &'static [u8] = b"DIJETS WALLET: main key salt$";
    const INFO_PREFIX: &'static [u8] = b"DIJETS WALLET: derived key$";
    ...
    pub fn private_child(&self, child: ChildNumber) -> Result<ExtendedPrivKey> {
        // application info in the HKDF context is defined as Dijets derived key$child_number.
        let mut le_n = [0u8; 8];
...
fn test_key_derivation() {
    ...
    let seed = Seed::new(&mnemonic, "DIJETS");
