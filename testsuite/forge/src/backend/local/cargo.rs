...
/// Get the dijets node binary from the current working directory
pub fn get_dijets_node_binary_from_worktree() -> Result<(String, PathBuf)> {
    ...
    let bin_path = cargo_build_dijets_node(&metadata.workspace_root, &metadata.target_directory)?;

    Ok((revision, bin_path))
}

/// This function will attempt to build the dijets-node binary at an arbitrary revision.
/// Using the `target/forge` as a working directory it will do the following:
///     1. Look for a binary named `dijets-node--<revision>`, if it already exists return it
...
///        binary and then move it to `target/forge/dijets-node--<revision>`
pub fn get_dijets_node_binary_at_revision(revision: &str) -> Result<(String, PathBuf)> {
    let metadata = metadata()?;
    ...
    let dijets_node_bin = forge_directory.join(format!(
        "dijets-node--{}{}",
        revision,
        env::consts::EXE_SUFFIX
    ));

    if dijets_node_bin.exists() {
        return Ok((revision, dijets_node_bin));
    }
    ...
    fs::rename(
        cargo_build_dijets_node(&checkout_dir, &forge_target_directory)?,
        &dijets_node_bin,
    )?;

    let _ = fs::remove_dir_all(&checkout_dir);

    Ok((revision, dijets_node_bin))
}
...
/// dijets/dijets repository, falling back to "origin" if unable to locate the remote
pub fn git_get_upstream_remote() -> Result<String> {
    let output = Command::new("sh")
        .arg("-c")
        .arg("git remote -v | grep \"https://github.com/dijets/dijets.* (fetch)\" | cut -f1")
        .output()
...
fn cargo_build_dijets_node<D, T>(directory: D, target_directory: T) -> Result<PathBuf>
where
    D: AsRef<Path>,
    T: AsRef<Path>,
{
    ...
    let output = Command::new("cargo")
        ...
        .args(&["build", "--bin=dijets-node"])
        .output()
        .context("Failed to build dijets-node")?;

    if output.status.success() {
        let bin_path =
            target_directory.join(format!("debug/{}{}", "dijets-node", env::consts::EXE_SUFFIX));
        if !bin_path.exists() {
            bail!(
                "Can't find binary dijets-node at expected path {:?}",
                bin_path
            );
        }
        ...
        bail!(
            "Failed to build dijets-node: 'cd {} && CARGO_TARGET_DIR={} cargo build --bin=dijets-node",
            directory.display(),
