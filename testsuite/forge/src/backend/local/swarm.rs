...
use dijets_genesis_tool::validator_builder::ValidatorBuilder;
use dijets_sdk::{
...
struct ValidatorNetworkAddressEncryptionKey {
    key: dijets_sdk::types::network_address::encrypted::Key,
    version: dijets_sdk::types::network_address::encrypted::KeyVersion,
}
...
impl LocalSwarmBuilder {
        ...
        R: ::rand::RngCore + ::rand::CryptoRng,
    {
        ...
        let root_account = LocalAccount::new(
            dijets_sdk::types::account_config::dijets_root_address(),
        ...
        let designated_dealer_account = LocalAccount::new(
            dijets_sdk::types::account_config::testnet_dd_account_address(),
        ...
        let treasury_compliance_account = LocalAccount::new(
            dijets_sdk::types::account_config::treasury_compliance_account_address(),
            ...
            for (node, done) in self.validators.values_mut().zip(done.iter_mut()) {
                ...
                match node.health_check() {
                    ...
                    Err(HealthCheckError::Unknown(e)) => {
                        return Err(anyhow!("Dijets node '{}' is not running: {}", node.name(), e));
                    }
                    Err(HealthCheckError::NotRunning) => {
                        return Err(anyhow!("Dijets node '{}' is not running", node.name()));
                    }
