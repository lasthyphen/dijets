...
async fn wait_genesis_job(kube_client: &K8sClient, era: &str) -> Result<()> {
    dijets_retrier::retry_async(k8s_retry_strategy(), || {
        ...
        Box::pin(async move {
            let job_name = format!("dijets-testnet-genesis-e{}", era);
...
fn upgrade_validator(validator_name: &str, helm_repo: &str, options: &[&str]) -> Result<()> {
    let validator_upgrade_base_args = [
        "upgrade",
        validator_name,
        &format!("{}/dijets-validator", helm_repo),
    ];
...
fn upgrade_testnet(
    ...
    let testnet_upgrade_output = Command::new(HELM_BIN)
        ...
        .output()
        .expect("failed to helm upgrade dijets");
    assert!(
...
fn get_helm_values(helm_release_name: &str) -> Result<Value> {
    let mut v: Value = get_helm_status(helm_release_name)
        .map_err(|e| format_err!("failed to helm get values dijets: {}", e))?;
    Ok(v["config"].take())
}
...
fn get_new_era() -> Result<String> {
    let v: Value = get_helm_values("dijets")?;
...
pub async fn k8s_client() -> K8sClient {
    ...
    let _ = dijets_retrier::retry_async(k8s_retry_strategy(), || {
        Box::pin(async move {
