...
use dijets_sdk::crypto::ed25519::ED25519_PRIVATE_KEY_LENGTH;
use dijets_secure_storage::{CryptoStorage, KVStorage, VaultStorage};
...
impl K8sFactory {
    pub fn new(helm_repo: String, image_tag: String, base_image_tag: String) -> Result<K8sFactory> {
        ...
        vault.available()?;
        let root_key = vault
            .export_private_key("dijets__dijets_root")
            .unwrap()
            .to_bytes();
        let treasury_compliance_key = vault
            .export_private_key("dijets__treasury_compliance")
            .unwrap()
            .to_bytes();
