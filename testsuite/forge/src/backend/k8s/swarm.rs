...
impl K8sSwarm {
    pub async fn new(
        ...
        let address = dijets_sdk::types::account_config::dijets_root_address();
        ...
        let account_key = AccountKey::from_private_key(key);
        let address = dijets_sdk::types::account_config::treasury_compliance_account_address();
        let sequence_number = query_sequence_numbers(&client, &[address])
            .await
            .map_err(|e| {
                format_err!(
        ...
        let key = load_tc_key(treasury_compliance_key);
        let account_key = AccountKey::from_private_key(key);
        let address = dijets_sdk::types::account_config::testnet_dd_account_address();
        let sequence_number = query_sequence_numbers(&client, &[address])
            .await
            .map_err(|e| {
                format_err!(
...
pub fn nodes_healthcheck<'a>(
    ...
    let unhealthy_nodes = nodes
        .filter_map(|node| {
            ...
            let check = dijets_retrier::retry(k8s_retry_strategy(), || match node.health_check() {
                Ok(_) => {
