...
use dijets_genesis_tool::config_builder::FullnodeType;
use dijets_swarm::{client, faucet, swarm::DijetsSwarm};
...
#[structopt(about = "Dijets swarm to start local nodes")]
struct Args {
    ...
    /// Directory used by launch_swarm to output DijetsNodes' config files, logs, dijetsdb, etc,
    ...
    /// Path to the dijets-node binary
    #[structopt(long)]
    pub dijets_node: String,
...
fn main() {
    ...
    let mut validator_swarm = DijetsSwarm::configure_validator_swarm(
        args.dijets_node.as_ref(),
        num_nodes,
    ...
    let mut full_node_swarm = if num_full_nodes > 0 {
        Some(
            DijetsSwarm::configure_fn_swarm(
                "ValidatorFullNode",
                args.dijets_node.as_ref(),
                None, /* config dir */
                None,
    ...
    let dijets_root_key_path = &validator_swarm.config.dijets_root_key_path;
    ...
    println!("To run the Dijets CLI client in a separate process and connect to the validator nodes you just spawned, use this command:");
    ...
    println!(
        ...
        dijets_root_key_path, node_address_list,
    );
    ...
    let faucet = if args.start_faucet {
        let faucet_port = dijets_config::utils::get_available_port();
    ...
    if args.start_client {
        ...
        // Explicitly capture CTRL-C to drop DijetsSwarm.
