...
pub struct DijetsNode {
    ...
    config: NodeConfig,
    config_path: PathBuf,
    dijets_node_bin_path: PathBuf,
}

impl DijetsNode {
    /// Launch a `DijetsNode`
    pub fn launch(
        dijets_node_bin_path: &Path,
        node_id: String,
        node_type: NodeType,
        config_path: &Path,
        log_path: PathBuf,
        ...
        // Start node process
        let mut node_command = Command::new(dijets_node_bin_path);
        ...
        Ok(Self {
            ...
            dijets_node_bin_path: PathBuf::from(dijets_node_bin_path),
        })
    }
    ...
    pub fn start(&mut self) -> Result<()> {
        ...
        // Start node process
        let mut node_command = Command::new(self.dijets_node_bin_path.as_path());
        ...
        let process = node_command.spawn().with_context(|| {
            format!(
                "Error launching node process with binary: {:?}",
                self.dijets_node_bin_path.as_path()
            )
        })?;
    ...
    pub fn get_connected_peers(
        ...
        self.get_metric_with_fields("dijets_connections", map)
    }
...
#[derive(Debug)]
pub enum DijetsSwarmDir {
    Persistent(PathBuf),
    Temporary(TempPath),
}

impl AsRef<Path> for DijetsSwarmDir {
    fn as_ref(&self) -> &Path {
        match self {
            DijetsSwarmDir::Persistent(path_buf) => path_buf.as_path(),
            DijetsSwarmDir::Temporary(temp_dir) => temp_dir.path(),
        }
    }
}
...
/// Struct holding instances and information of Dijets Swarm
pub struct DijetsSwarm {
    label: &'static str,
    dijets_node_bin_path: PathBuf,
    // Output log, DijetsNodes' config file, dijetsdb etc, into this dir.
    dir: DijetsSwarmDir,
    // Maps the node id of a node to the DijetsNode struct
    nodes: HashMap<String, DijetsNode>,
...
impl DijetsSwarm {
    ...
    fn setup_config_dir(config_dir: &Option<String>) -> DijetsSwarmDir {
        if let Some(dir_str) = config_dir {
            ...
            DijetsSwarmDir::Persistent(path_buf)
        } else {
            let temp_dir = TempPath::new();
            temp_dir
                .create_as_dir()
                .expect("unable to create temporary config dir");
            DijetsSwarmDir::Temporary(temp_dir)
        }
    }
    ...
    pub fn configure_fn_swarm(
        ...
        let builder = FullnodeBuilder::new(
            ...
            upstream_config.dijets_root_key_path.clone(),
            node_config,
            fn_type,
        );
        ...
        Ok(Self {
            label,
            dijets_node_bin_path: dijets_node_bin_path.to_path_buf(),
    ...
    pub fn configure_validator_swarm(
        ...
        let builder = ValidatorBuilder::new(
            &config_path,
            dijets_framework_releases::current_module_blobs().to_vec(),
        )
    ...
    pub fn launch_attempt(&mut self, check_connectivity: bool) -> Result<(), SwarmLaunchFailure> {
        ...
        for (index, path) in self.config.config_files.iter().enumerate() {
            ...
            let node = DijetsNode::launch(
                &self.dijets_node_bin_path,
                node_id.clone(),
                self.node_type,
                path,
            ...
            for (node, done) in self.nodes.values_mut().zip(done.iter_mut()) {
                ...
                match node.health_check() {
                    ...
                    HealthStatus::Crashed(status) => {
                        error!(
                            "{} Dijets node '{}' has crashed with status '{}'. Log output: '''{}'''",
                            log_header,
                            node.node_id(),
                            status,
                    ...
                    HealthStatus::Stopped => {
                        panic!(
                            "{} Dijets node '{} child process is not created",
                            log_header,
                            node.node_id()
                        )
                    }
    ...
    pub fn wait_for_all_nodes_to_catchup(&mut self) -> bool {
        let num_attempts = 60;
        let last_committed_round_str = "dijets_consensus_last_committed_round{}";
        let mut done = vec![false; self.nodes.len()];
        let log_header = self.log_header();
    ...
    pub fn get_node(&self, idx: usize) -> Option<&DijetsNode> {
        let node_id = format!("{}", idx);
        self.nodes.get(&node_id)
    }

    pub fn mut_node(&mut self, idx: usize) -> Option<&mut DijetsNode> {
        let node_id = format!("{}", idx);
        self.nodes.get_mut(&node_id)
    }

    pub fn nodes(&mut self) -> &mut HashMap<String, DijetsNode> {
        &mut self.nodes
    }
    ...
    pub fn start_node(&mut self, idx: usize) -> Result<(), SwarmLaunchFailure> {
        ...
        let node_id = format!("{}", idx);
        let mut node = DijetsNode::launch(
            &self.dijets_node_bin_path,
            node_id.clone(),
            self.node_type,
            path,
...
impl Drop for DijetsSwarm {
    fn drop(&mut self) {
        let log_header = self.log_header();
        // If panicking, we don't want to gc the swarm directory.
        if std::thread::panicking() {
            // let dir = self.dir;
            if let DijetsSwarmDir::Temporary(temp_dir) = &mut self.dir {
                temp_dir.persist();
                let log_path = temp_dir.path();
                println!("{} logs located at {:?}", log_header, log_path);

                // Dump logs for each validator to stdout when `DIJETS_DUMP_LOGS`
                // environment variable is set
                if env::var_os("DIJETS_DUMP_LOGS").is_some() {
