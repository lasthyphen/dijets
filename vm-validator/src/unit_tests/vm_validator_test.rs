...
use dijets_transaction_builder::stdlib::encode_peer_to_peer_with_metadata_script;
use dijets_types::{
...
struct TestValidator {
    ...
    _db_path: dijets_temppath::TempPath,
}

impl TestValidator {
    fn new() -> Self {
        let _db_path = dijets_temppath::TempPath::new();
        _db_path.create_as_dir().unwrap();
        let (db, db_rw) = DbReaderWriter::wrap(DijetsDB::new_for_test(_db_path.path()));
        executor_test_helpers::bootstrap_genesis::<DijetsVM>(
            &db_rw,
...
//   language e2e tests in: dijets/language/e2e-testsuite/src/tests/verify_txn.rs ->
...
// * SENDING_ACCOUNT_FROZEN: Tested in functional-tests/tests/dijets_account/freezing.move.
// * Errors arising from deserializing the code -- these are tested in
//   - dijets/language/move-binary-format/src/unit_tests/deserializer_tests.rs
//   - dijets/language/move-binary-format/tests/serializer_tests.rs
...
//   dijets/language/e2e-testsuite/src/tests/genesis.rs

#[test]
fn test_validate_transaction() {
    let vm_validator = TestValidator::new();

    let address = account_config::dijets_root_address();
...
fn test_validate_non_genesis_write_set() {
    ...
    // A WriteSet txn is only valid when sent from the Dijets root account.
