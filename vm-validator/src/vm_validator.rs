...
use dijets_types::{
    ...
    on_chain_config::{DijetsVersion, OnChainConfigPayload, VMConfig, VMPublishingOption},
    transaction::{SignedTransaction, VMValidatorResult},
};
use dijets_vm::DijetsVMValidator;
use fail::fail_point;
...
pub trait TransactionValidation: Send + Sync + Clone {
    type ValidationInstance: dijets_vm::VMValidator;
...
impl VMValidator {
    pub fn new(db_reader: Arc<dyn DbReader>) -> Self {
        ...
        let vm = DijetsVMValidator::new(&state_view);
        VMValidator { db_reader, vm }
    }
}

impl TransactionValidation for VMValidator {
    type ValidationInstance = DijetsVMValidator;
    ...
    fn restart(&mut self, config: OnChainConfigPayload) -> Result<()> {
        ...
        let version = config.get::<DijetsVersion>()?;
        let publishing_option = config.get::<VMPublishingOption>()?;

        self.vm = DijetsVMValidator::init_with_config(version, vm_config, publishing_option);
        Ok(())
    }
}
