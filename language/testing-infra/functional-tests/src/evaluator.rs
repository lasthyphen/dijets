...
use dijets_crypto::ed25519::{Ed25519PrivateKey, Ed25519PublicKey};
...
use dijets_types::{
    ...
    transaction::{
        ...
        SignedTransaction, Transaction as DijetsTransaction, TransactionOutput, TransactionPayload,
        TransactionStatus,
    },
...
/// Commands that drives the operation of DijetsVM. Such as:
/// 1. Execute user transaction
/// 2. Publish a new block metadata
///
/// In the future we will add more commands to mimic the full public API of DijetsVM,
/// including reloading the on-chain configuration that will affect the code path for DijetsVM,
/// cleaning the cache in the DijetsVM, etc.
#[derive(Debug)]
pub enum Command<'a> {
...
pub fn eval_block_metadata(
    ...
    let outputs =
        executor.execute_transaction_block(vec![DijetsTransaction::BlockMetadata(block_metadata)]);
...
pub fn eval<TComp: Compiler>(
    ...
    let mut exec = if config.validator_accounts == 0 {
        ...
        let module_blobs = if compiler.use_compiled_genesis() {
            dijets_framework_releases::current_module_blobs()
        } else {
            dijets_framework::module_blobs()
        };
