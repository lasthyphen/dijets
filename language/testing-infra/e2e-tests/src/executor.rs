...
use dijets_types::{
    ...
    on_chain_config::{DijetsVersion, OnChainConfig, VMPublishingOption, ValidatorSet},
    transaction::{
...
use dijets_vm::{
    convert_changeset_and_events, data_cache::RemoteStorage, DijetsVM, DijetsVMValidator, VMExecutor,
    VMValidator,
};
use move_core_types::{
...
///
/// This struct is a mock in-memory implementation of the Dijets executor.
#[derive(Debug)]
...
impl FakeExecutor {
    ...
    pub fn set_golden_file(&mut self, test_name: &str) {
        ...
        if let Some(env_trace_dir) = env::var_os(ENV_TRACE_DIR) {
            let dijets_version = DijetsVersion::fetch_config(&self.data_store).map_or(0, |v| v.major);
            ...
            let mut name_file = OpenOptions::new()
                ...
                .unwrap();
            write!(name_file, "{}::{}", test_name, dijets_version).unwrap();
            for sub_dir in &[
                TRACE_DIR_META,
                TRACE_DIR_DATA,
                TRACE_DIR_INPUT,
    ...
    pub fn execute_block_and_keep_vm_status(
        ...
        DijetsVM::execute_block_and_keep_vm_status(
            txn_block
                .into_iter()
                .map(Transaction::UserTransaction)
                .collect(),
    ...
    pub fn execute_transaction_block(
        ...
        let output = DijetsVM::execute_block(txn_block, &self.data_store);
    ...
    pub fn verify_transaction(&self, txn: SignedTransaction) -> VMValidatorResult {
        let vm = DijetsVMValidator::new(self.get_state_view());
    ...
    pub fn exec(
        ...
        let write_set = {
            ...
            let vm = MoveVM::new(dijets_vm::natives::dijets_natives()).unwrap();
