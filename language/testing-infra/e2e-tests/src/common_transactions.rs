...
use dijets_transaction_builder::stdlib::encode_peer_to_peer_by_signers_script_function;
use dijets_types::{
    account_config,
    transaction::{
...
pub static CREATE_ACCOUNT_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    main<Token>(account: signer, fresh_address: address, auth_key_prefix: vector<u8>, initial_amount: u64) {
      ...
      DijetsAccount.create_parent_vasp_account<Token>(
        &account,
        copy(fresh_address),
...
pub static MULTI_AGENT_SWAP_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    main(alice: signer, bob: signer, amount_xus: u64, amount_xdx: u64) {
        ...
        let alice_withdrawal_cap: DijetsAccount.WithdrawCapability;
        let bob_withdrawal_cap: DijetsAccount.WithdrawCapability;
        let alice_addr: address;
        let bob_addr: address;

        alice_withdrawal_cap = DijetsAccount.extract_withdraw_capability(&alice);
        bob_addr = Signer.address_of(&bob);
        DijetsAccount.pay_from<XUS.XUS>(
            &alice_withdrawal_cap, move(bob_addr), move(amount_xus), h\"\", h\"\"
        );
        DijetsAccount.restore_withdraw_capability(move(alice_withdrawal_cap));

        // Then, Bob pays Alice in currency XDX.
        bob_withdrawal_cap = DijetsAccount.extract_withdraw_capability(&bob);
        alice_addr = Signer.address_of(&alice);
        DijetsAccount.pay_from<XDX.XDX>(
            &bob_withdrawal_cap, move(alice_addr), move(amount_xdx), h\"\", h\"\"
        );
        DijetsAccount.restore_withdraw_capability(move(bob_withdrawal_cap));
        return;
    }
";
...
pub static MULTI_AGENT_MINT_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    main<CoinType>(
        ...
        let dd_withdrawal_cap: DijetsAccount.WithdrawCapability;
        ...
        dd_withdrawal_cap = DijetsAccount.extract_withdraw_capability(&dd_account);
        ...
        DijetsAccount.restore_withdraw_capability(move(dd_withdrawal_cap));
        return;
    }
";
