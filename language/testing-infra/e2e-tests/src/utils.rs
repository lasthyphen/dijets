...
pub fn start_with_released_df() -> (FakeExecutor, Account, Account, Account) {
    ...
    let mut dr_account = Account::new_dijets_root();
...
pub fn upgrade_df(
    ...
    for compiled_module_bytes in dijets_framework_releases::current_module_blobs()
        .iter()
        .cloned()
    {
    ...
    if let Some(version_number) = update_version_number {
        executor.execute_and_apply(
            dr_account
                .transaction()
                .script(encode_update_dijets_version_script(0, version_number))
                .sequence_number(*dr_seqno)
                .sign(),
        );
        *dr_seqno = dr_seqno.checked_add(1).unwrap();
