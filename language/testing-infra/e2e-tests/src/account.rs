...
//! Test infrastructure for modeling Dijets accounts.

use crate::gas_costs;
use anyhow::{Error, Result};
use dijets_crypto::ed25519::*;
...
/// Details about a Dijets account.
///
/// Tests will typically create a set of `Account` instances to run transactions on. This type
/// encodes the logic to operate on and verify operations on any Dijets account.
...
impl Account {
    ...
    /// not automatically get added to the Dijets store. To add an account to the store, use
    ...
    pub fn with_keypair(privkey: Ed25519PrivateKey, pubkey: Ed25519PublicKey) -> Self {
        let addr = dijets_types::account_address::from_public_key(&pubkey);
        Account {
            addr,
            privkey,
            pubkey,
    ...
    /// Creates a new account representing the dijets root account in memory.
    ///
    /// The address will be [`dijets_root_address`][account_config::dijets_root_address], and
    /// the account will use [`GENESIS_KEYPAIR`][struct@GENESIS_KEYPAIR] as its keypair.
    pub fn new_dijets_root() -> Self {
        Self::new_genesis_account(account_config::dijets_root_address())
    }
...
impl AccountData {
    ...
    /// Returns the (Move value) layout of the DijetsAccount::DijetsAccount struct
    pub fn layout() -> MoveStructLayout {
