...
/// The Dijets transaction test adapter.
///
/// This differs from the SimpleVMTestAdapter in a few ways to ensure that our tests mimics
/// production settings:
///   - It uses a Dijets StateView as its storage backend
///   - It executes transactions through DijetsVM, instead of MoveVM directly
struct DijetsTestAdapter<'a> {
    compiled_state: CompiledState<'a>,
    storage: FakeDataStore,
    default_syntax: SyntaxChoice,
}

/// Parameters *required* to create a Dijets transaction.
...
/// Dijets-specific arguments for the publish command.
#[derive(StructOpt, Debug)]
struct DijetsPublishArgs {
    #[structopt(short = "k", long = "private-key", parse(try_from_str = parse_ed25519_private_key))]
    privkey: Ed25519PrivateKey,
}

/// Dijets-specifc arguments for the run command,
#[derive(StructOpt, Debug)]
struct DijetsRunArgs {
    #[structopt(short = "k", long = "private-key", parse(try_from_str = parse_ed25519_private_key))]
    privkey: Ed25519PrivateKey,
}

impl<'a> DijetsTestAdapter<'a> {
    ...
    /// Perform a single Dijets transaction.
    ///
    ...
    fn run_transaction(&mut self, txn: SignedTransaction) -> Result<()> {
        let mut outputs = DijetsVM::execute_block_and_keep_vm_status(
...
impl<'a> MoveTestAdapter<'a> for DijetsTestAdapter<'a> {
    type ExtraInitArgs = EmptyCommand;
    type ExtraPublishArgs = DijetsPublishArgs;
    type ExtraRunArgs = DijetsRunArgs;
    ...
    fn init(
        ...
        let mut named_address_mapping = dijets_framework::dijets_framework_named_addresses();
        for (name, addr) in additional_mapping {
            if named_address_mapping.contains_key(&name) {
                panic!(
                    "Invalid init. The named address '{}' is reserved by either the move-stdlib or dijets-framework",
                    name
                )
            }
...
static PRECOMPILED_DIJETS_FRAMEWORK: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    let program_res = move_lang::construct_pre_compiled_lib(
        &dijets_framework::dijets_stdlib_files(),
        None,
        move_lang::Flags::empty().set_sources_shadow_deps(false),
        dijets_framework::dijets_framework_named_addresses(),
    )
    .unwrap();
    match program_res {
        Ok(df) => df,
        Err((files, errors)) => {
            eprintln!("!!!Dijets Framework failed to compile!!!");
            move_lang::diagnostics::report_diagnostics(&files, errors)
        }
    }
});

/// Run the Dijets transactional test flow, using the given file as input.
pub fn run_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    run_test_impl::<DijetsTestAdapter>(path, Some(&*PRECOMPILED_DIJETS_FRAMEWORK))
}
