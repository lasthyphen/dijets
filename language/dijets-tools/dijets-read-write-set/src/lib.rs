...
use dijets_vm::system_module_names::{SCRIPT_PROLOGUE_NAME, USER_EPILOGUE_NAME};
use move_core_types::{
    ident_str,
    identifier::IdentStr,
...
impl ReadWriteSetAnalysis {
    /// Create a Dijets transaction read/write set analysis from a generic Move module read/write set
    /// analysis
    ...
    fn get_concretized_keys_tx(
        ...
        match tx.payload() {
            TransactionPayload::ScriptFunction(s) => {
                ...
                // to do as of Dijets 1.3, but should be re-evaluated if the epilogue changes
