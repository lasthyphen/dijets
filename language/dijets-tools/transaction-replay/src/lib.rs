...
use dijets_resource_viewer::{AnnotatedAccountStateBlob, AnnotatedMoveStruct, DijetsValueAnnotator};
use dijets_state_view::StateView;
use dijets_types::{
    access_path,
    account_address::AccountAddress,
    account_config::dijets_root_address,
...
use dijets_validator_interface::{
    DBDebuggerInterface, DebuggerStateView, DijetsValidatorInterface, JsonRpcDebuggerInterface,
};
use dijets_vm::{
    convert_changeset_and_events, data_cache::RemoteStorage, logging::AdapterLogSchema, DijetsVM,
    VMExecutor,
};
...
pub struct DijetsDebugger {
    debugger: Box<dyn DijetsValidatorInterface>,
    build_dir: PathBuf,
    storage_dir: PathBuf,
}

impl DijetsDebugger {
    pub fn new(debugger: Box<dyn DijetsValidatorInterface>) -> Self {
        Self {
            debugger,
    ...
    pub fn execute_transactions_at_version(
        ...
        DijetsVM::execute_block(txns, &state_view)
    ...
    pub fn execute_writeset_at_version(
        ...
        let vm = DijetsVM::new(&state_view);
        let cache = dijets_vm::data_cache::StateViewCache::new(&state_view);
        let sequence_number = match self
            .debugger
            .get_account_state_by_version(dijets_root_address(), version)?
        {
            Some(account) => account
                .get_account_resource()?
                .ok_or_else(|| anyhow!("Dijets root account doesn't exist"))?
                .sequence_number(),
            None => bail!("Dijets root account blob doesn't exist"),
        };
        let txn_data = dijets_vm::transaction_metadata::TransactionMetadata {
            sequence_number,
            sender: dijets_root_address(),
            ..Default::default()
        };
    ...
    pub fn get_dijets_framework_modules_at_version(
        &self,
        version: Version,
        save_write_sets: bool,
        ...
        let modules = self
            .debugger
            .get_dijets_framework_modules_by_version(version)?;
        if save_write_sets {
            let state_view = OnDiskStateView::create(&self.build_dir, &self.storage_dir)?;
            for m in &modules {
    ...
    pub fn annotate_events(&self, events: &[EventWithProof]) -> Result<Vec<AnnotatedMoveStruct>> {
        ...
        let annotator = DijetsValueAnnotator::new(&remote_storage);
    ...
    {
        let move_vm = MoveVM::new(dijets_vm::natives::dijets_natives()).unwrap();
        let state_view = DebuggerStateView::new(&*self.debugger, version);
        ...
        let is_version_ok = |version| {
            self.run_session_at_version(version, override_changeset.clone(), |session| {
                ...
                session.execute_script(
                    ...
                    vec![],
                    vec![],
                    vec![dijets_root_address(), sender],
                    &mut gas_status,
                )
            })
            .map(|_| ())
...
fn compile_move_script(file_path: &str) -> Result<Vec<u8>> {
    ...
    let (files, units_or_diags) = Compiler::new(targets, &dijets_framework::dijets_stdlib_files())
        .set_flags(Flags::empty().set_sources_shadow_deps(false))
        .set_named_address_values(dijets_framework::dijets_framework_named_addresses())
        .build()?;
