...
struct Opt {
    /// Path to the local DijetsDB file
...
enum Command {
    ...
    /// Execute a writeset as if it is signed by the Dijets Root and print the result.
    #[structopt(name = "replay-writeset")]
    ReplayWriteSetAtVersion {
        /// Path to a serialized WriteSetPayload. Could be generated by the `dijets-writeset-generator` tool.
    ...
    /// Annotate the resources stored under `dijets_root`, `treasury_compliance` and all validator addresses.
    ...
    /// Get the bytecode for all Dijets Framework modules at `version`
...
fn main() -> Result<()> {
    ...
    let debugger = if let Some(p) = opt.db {
        ...
        DijetsDebugger::json_rpc(url.as_str())?
    } else {
    ...
    match opt.cmd {
        ...
        Command::GetModules { version } => {
            let modules =
                debugger.get_dijets_framework_modules_at_version(version, opt.save_write_sets)?;
