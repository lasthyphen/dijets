...
use crate::{unit_tests::TestInterface, DijetsDebugger};
use anyhow::bail;
use dijets_types::{
...
#[test]
fn test_bisection() {
    let debugger = DijetsDebugger::new(Box::new(TestInterface::empty(100)));
...
fn test_changeset_override() {
    let debugger = DijetsDebugger::new(Box::new(TestInterface::genesis()));
