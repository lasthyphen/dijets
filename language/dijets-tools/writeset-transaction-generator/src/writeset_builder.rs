...
use dijets_types::{
    ...
    account_config::{self, dijets_root_address},
    transaction::{ChangeSet, Script, Version},
};
use dijets_vm::{convert_changeset_and_events, data_cache::RemoteStorage};
use move_core_types::{
...
impl<'r, 'l, S: MoveResolver> GenesisSession<'r, 'l, S> {
    ...
    fn disable_reconfiguration(&mut self) {
        self.exec_func(
            ...
            vec![],
            serialize_values(&vec![MoveValue::Signer(dijets_root_address())]),
        )
    }
    ...
    pub fn set_dijets_version(&mut self, version: Version) {
        self.exec_func(
            "DijetsVersion",
            "set",
            vec![],
