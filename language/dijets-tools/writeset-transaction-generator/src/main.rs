...
enum Command {
    ...
    /// Create a release writeset by comparing local Dijets Framework against a remote blockchain state.
    #[structopt(name = "create-release")]
    CreateDijetsFrameworkRelease {
...
fn dijets_framework_modules(release_name: &str) -> Vec<(Vec<u8>, CompiledModule)> {
    ...
    dijets_framework_releases::load_modules_from_release(release_name)
        .unwrap_or_else(|_| {
            panic!(
...
fn main() -> Result<()> {
    ...
    let payload = match opt.cmd {
        ...
        Command::CreateDijetsFrameworkRelease {
            chain_id,
            url,
            version,
            first_release,
            ...
            let release_modules = dijets_framework_modules(release_name.as_str());
            create_release(
                chain_id,
                url,
                version,
        ...
        Command::VerifyDijetsFrameworkRelease {
            url,
            chain_id,
            writeset_path,
            use_latest_version,
