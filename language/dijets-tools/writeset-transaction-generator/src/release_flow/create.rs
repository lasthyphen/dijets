...
use dijets_validator_interface::{
    DebuggerStateView, DijetsValidatorInterface, JsonRpcDebuggerInterface,
};
use move_binary_format::CompiledModule;
use std::collections::{BTreeMap, BTreeSet, HashSet};

pub fn create_release(
    // ChainID to distinguish the dijets network. e.g: PREMAINNET
    chain_id: ChainId,
    ...
    updated_dijets_version: Option<u64>,
    release_name: &str,
    ...
    let release_artifact = ReleaseArtifact {
        ...
        dijets_version: updated_dijets_version,
    ...
    if !first_release {
        ...
        if let (Some(old_version), Some(new_version)) =
            (artifact.dijets_version, updated_dijets_version)
        {
            if old_version >= new_version {
                bail!("DijetsVersion should be strictly increasing")
            }
        }
    }
...
pub(crate) fn create_release_from_artifact(
    ...
    let remote_modules = remote
        .get_dijets_framework_modules_by_version(override_version.unwrap_or(artifact.version))?;
    let modules_payload = create_release_writeset(&remote_modules, release_modules)?;

    Ok(if let Some(updated_dijets_version) = artifact.dijets_version {
        ...
        let (updated_version_writeset, events) = build_changeset(&state_view, |session| {
            session.set_dijets_version(updated_dijets_version);
        })
        .into_inner();
        ...
        {
            bail!("DijetsVersion WriteSet collides with module upgrade WriteSet");
        }
