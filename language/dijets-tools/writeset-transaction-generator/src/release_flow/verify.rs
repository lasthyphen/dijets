...
/// states contains the exact same Dijets Framework modules as the locally compiled stdlib.
pub(crate) fn verify_payload_change<'a>(
    validator: Box<dyn DijetsValidatorInterface>,
    ...
    // Applying this writeset should make Dijets framework equal to its on-disk status
    let mut old_modules = validator
        .get_dijets_framework_modules_by_version(block_height)?
        .into_iter()
        .map(|m| (m.self_id(), m))
        .collect::<BTreeMap<_, _>>();

    let output = {
        let txn_replay = DijetsDebugger::new(validator);
