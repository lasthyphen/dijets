...
pub trait DijetsValidatorInterface: Sync {
    ...
    fn get_dijets_framework_modules_by_version(
        &self,
        version: Version,
        ...
        for module_bytes in self
            ...
            .ok_or_else(|| anyhow!("Failure reading dijets root address state"))?
            .get_modules()
        {
            acc.push(
    ...
    /// 1. Dijets Framework code address
    /// 2. Dijets Root address
    ...
    fn get_admin_accounts(&self, version: Version) -> Result<Vec<(AccountAddress, AccountState)>> {
        let mut result = vec![];
        let dijets_root = self
            .get_account_state_by_version(account_config::dijets_root_address(), version)?
            .ok_or_else(|| anyhow!("dijets_root_address doesn't exist"))?;

        // Get all validator accounts
        let validators = dijets_root
            .get_config::<ValidatorSet>()?
            .ok_or_else(|| anyhow!("validator_config doesn't exist"))?;

        // Get code account, dijets_root and treasury compliance accounts.
        result.push((
        ...
        result.push((account_config::dijets_root_address(), dijets_root));
        result.push((
...
pub struct DebuggerStateView<'a> {
    db: &'a dyn DijetsValidatorInterface,
    version: Version,
}

impl<'a> DebuggerStateView<'a> {
    pub fn new(db: &'a dyn DijetsValidatorInterface, version: Version) -> Self {
        Self { db, version }
    }
}
