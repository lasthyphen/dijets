...
use dijets_vm::{
    ...
    system_module_names::{
        BLOCK_PROLOGUE, DIJETS_BLOCK_MODULE, SCRIPT_PROLOGUE_NAME, USER_EPILOGUE_NAME,
...
pub struct ReplayFlags {
    /// Filter based on which dijets version the trace was executed under
    pub dijets_version: DijetsVersion,
...
impl ReplayFlags {
    fn should_replay_trace(&self, test: &str, step: Option<usize>) -> Result<bool> {
        ...
        if vnum != self.dijets_version.major {
            return Ok(false);
        }
...
impl<'env> CrossRunner<'env> {
    ...
    pub fn step_function_and_compare(
        ...
        let move_vm = MoveVM::new(dijets_natives()).unwrap();
                ...
                Transaction::UserTransaction(signed_txn) => {
                    let (senders, script_fun, is_admin) = match signed_txn.payload() {
                        ...
                        TransactionPayload::Module(_) => {
                            ...
                            if flags.warning {
                                eprintln!(
                                    "[!] Replay stopped due to non-Dijets module compilation: {}",
                                    test_name
                                );
                            }
                            return Ok(());
