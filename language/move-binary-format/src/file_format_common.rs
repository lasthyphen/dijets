...
impl BinaryConstants {
    ...
    pub const DIJETS_MAGIC_SIZE: usize = 4;
    pub const DIJETS_MAGIC: [u8; BinaryConstants::DIJETS_MAGIC_SIZE] = [0xA1, 0x1C, 0xEB, 0x0B];
    /// The `DIJETS_MAGIC` size, 4 byte for major version and 1 byte for table count.
    pub const HEADER_SIZE: usize = BinaryConstants::DIJETS_MAGIC_SIZE + 5;
...
pub(crate) mod versioned_data {
    ...
    impl<'a> VersionedBinary<'a> {
        fn new(binary: &'a [u8]) -> BinaryLoaderResult<(Self, Cursor<&'a [u8]>)> {
            ...
            let mut magic = [0u8; BinaryConstants::DIJETS_MAGIC_SIZE];
            if let Ok(count) = cursor.read(&mut magic) {
                if count != BinaryConstants::DIJETS_MAGIC_SIZE || magic != BinaryConstants::DIJETS_MAGIC
                {
