...
const DIJETS_CORE_ADDR: AccountAddress =
...
impl<'env> FunctionContext<'env> {
    ...
    fn exec_native_function(
        ...
        match (addr, module_name.as_str(), function_name.as_str()) {
            ...
            (DIJETS_CORE_ADDR, "Vector", "length") => {
                if cfg!(debug_assertions) {
            ...
            (DIJETS_CORE_ADDR, "Vector", "borrow") => {
                if cfg!(debug_assertions) {
            ...
            (DIJETS_CORE_ADDR, "Vector", "borrow_mut") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 2);
                }
            ...
            (DIJETS_CORE_ADDR, "Vector", "push_back") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 2);
                }
                let res = self
            ...
            (DIJETS_CORE_ADDR, "Vector", "pop_back") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
            ...
            (DIJETS_CORE_ADDR, "Vector", "destroy_empty") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
            ...
            (DIJETS_CORE_ADDR, "Signer", "borrow_address") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
            ...
            (DIJETS_CORE_ADDR, "Hash", "sha2_256") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
            ...
            (DIJETS_CORE_ADDR, "Hash", "sha3_256") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
            ...
            (DIJETS_CORE_ADDR, "Event", "write_to_event_store") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 3);
                }
            ...
            (DIJETS_CORE_ADDR, "Signature", "ed25519_validate_pubkey") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
            ...
            (DIJETS_CORE_ADDR, "Signature", "ed25519_verify") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 3);
                }
            ...
            (DIJETS_CORE_ADDR, "DijetsAccount", "create_signer") => {
                if cfg!(debug_assertions) {
                    assert_eq!(srcs.len(), 1);
                }
                let res = self.native_dijets_account_create_signer(dummy_state.del_value(0));
                Ok(vec![res])
            }
            _ => unreachable!(),
        }
    ...
    fn native_dijets_account_create_signer(&self, addr: TypedValue) -> TypedValue {
        if cfg!(debug_assertions) {
            assert_eq!(self.ty_args.len(), 0);
        }
