...
const INCONSISTENCY_TEST_FLAGS: &[&str] = &[
    ...
    "--dependency=../dijets-framework/modules",
    "--check-inconsistency",
    "-a=Std=0x1",
    "-a=DijetsFramework=0x1",
    "-a=DijetsRoot=0xA550C18",
...
fn test_runner_for_feature(path: &Path, feature: &Feature) -> datatest_stable::Result<()> {
    ...
    if no_tools {
        ...
        See https://github.com/dijets/dijets/tree/main/language/move-prover/doc/user/install.md \
        for instructions."
            );
        }
    }
...
fn get_flags_and_baseline(
    ...
    let (base_flags, baseline_path) =
        if path_str.contains("dijets-framework/") || path_str.contains("move-stdlib/") {
...
fn main() {
    ...
    for feature in get_features() {
        ...
        if read_env_var(ENV_TEST_EXTENDED) == "1" {
            ...
            collect_enabled_tests(&mut reqs, "dijets", feature, "../dijets-framework");
        }
    }
