...
echo "*************** [check-pr] Assuming dijets root at $BASE"
...
while getopts "hxd" opt; do
  case $opt in
    h)
      ...
      echo "crates are run. With -g, dijets's git-check script is run".
    ...
    d)
      RUN_CRATES="
        ...
        $BASE/language/dijets-framework\
      "
      ;;
    g)
      GIT_CHECKS=1
...
if [ ! -z "$GIT_CHECKS" ]; then
   echo "*************** [check-pr] Dijets git checks"
