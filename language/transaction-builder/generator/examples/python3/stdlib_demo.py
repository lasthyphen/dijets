...
import dijets_types as dijets
import serde_types as st
import dijets_framework as stdlib


def demo_p2p_script() -> None:
    token = dijets.TypeTag__Struct(
        value=dijets.StructTag(
            address=dijets.AccountAddress.from_bytes(b"\x00" * 15 + b"\x01"),
            module=dijets.Identifier("XDX"),
            name=dijets.Identifier("XDX"),
            type_params=[],
        )
    )
    payee = dijets.AccountAddress.from_bytes(b"\x22" * 16)
