...
pub fn output(
    ...
    dijets_module_path: Option<String>,
    package_name: String,
    abis: &[ScriptABI],
) -> Result<()> {
...
struct GoEmitter<T> {
    ...
    serde_module_path: Option<String>,
    /// Go module path for Dijets types.
    ...
    T: Write,
{
    fn output_script_call_enum_with_imports(&mut self, abis: &[ScriptABI]) -> Result<()> {
        let dijets_types_package = match &self.dijets_module_path {
            Some(path) => format!("{}/dijetstypes", path),
            None => "dijetstypes".into(),
        };
        let mut external_definitions = crate::common::get_external_definitions(&dijets_types_package);
    ...
    fn output_encode_method(&mut self, abis: &[ScriptABI]) -> Result<()> {
        ...
        if !transaction_script_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build a Dijets `Script` from a structured object `ScriptCall`.
func EncodeScript(call ScriptCall) dijetstypes.Script {{"#
            )?;
            self.out.indent();
        ...
        if !script_fun_abis.is_empty() {
            writeln!(
                self.out,
                r#"
// Build a Dijets `TransactionPayload` from a structured object `ScriptFunctionCall`.
func EncodeScriptFunction(call ScriptFunctionCall) dijetstypes.TransactionPayload {{"#
            )?;
            self.out.indent();
            writeln!(self.out, "switch call := call.(type) {{")?;
    ...
    fn output_transaction_script_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize a Dijets `Script` and convert it into a structured object `ScriptCall`.
func DecodeScript(script *dijetstypes.Script) (ScriptCall, error) {{
    ...
    fn output_script_function_decode_method(&mut self) -> Result<()> {
        writeln!(
            self.out,
            r#"
// Try to recognize a Dijets `TransactionPayload` and convert it into a structured object `ScriptFunctionCall`.
func DecodeScriptFunctionPayload(script dijetstypes.TransactionPayload) (ScriptFunctionCall, error) {{
    switch script := script.(type) {{
        case *dijetstypes.TransactionPayload__ScriptFunction:
    ...
    fn output_transaction_script_encoder_function(
        ...
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}Script({}) dijetstypes.Script {{",
        ...
        writeln!(
            self.out,
            r#"return dijetstypes.Script {{
	Code: append([]byte(nil), {}_code...),
	TyArgs: []dijetstypes.TypeTag{{{}}},
	Args: []dijetstypes.TransactionArgument{{{}}},
}}"#,
            abi.name(),
    ...
    fn output_script_function_encoder_function(&mut self, abi: &ScriptFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}\nfunc Encode{}ScriptFunction({}) dijetstypes.TransactionPayload {{",
            Self::quote_doc(abi.doc()),
            abi.name().to_camel_case(),
            [
                Self::quote_type_parameters(abi.ty_args()),
        ...
        writeln!(
            self.out,
            r#"return &dijetstypes.TransactionPayload__ScriptFunction {{
            dijetstypes.ScriptFunction {{
                Module: {},
                Function: {},
                TyArgs: []dijetstypes.TypeTag{{{}}},
                Args: [][]byte{{{}}},
    ...
    fn output_transaction_script_decoder_function(
        ...
        writeln!(
            self.out,
            "\nfunc decode_{}_script(script *dijetstypes.Script) (ScriptCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
        writeln!(
...
"#,
    ...
    fn output_script_function_decoder_function(&mut self, abi: &ScriptFunctionABI) -> Result<()> {
        writeln!(
            self.out,
            "\nfunc decode_{}_script_function(script dijetstypes.TransactionPayload) (ScriptFunctionCall, error) {{",
            abi.name(),
        )?;
        self.out.indent();
...
"#,
    ...
    fn output_transaction_script_decoder_map(
        ...
        writeln!(
            self.out,
            r#"
var script_decoder_map = map[string]func(*dijetstypes.Script) (ScriptCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
    ...
    fn output_script_function_decoder_map(&mut self, abis: &[ScriptFunctionABI]) -> Result<()> {
        writeln!(
            self.out,
            r#"
var script_function_decoder_map = map[string]func(dijetstypes.TransactionPayload) (ScriptFunctionCall, error) {{"#
        )?;
        self.out.indent();
        for abi in abis {
            writeln!(
...
"#,
    ...
    fn output_decoding_helper(&mut self, type_tag: &TypeTag) -> Result<()> {
        ...
        writeln!(
            self.out,
            r#"
func decode_{0}_argument(arg dijetstypes.TransactionArgument) (value {1}, err error) {{
	if arg, ok := arg.(*dijetstypes.TransactionArgument__{2}); ok {{
		{3}
	}} else {{
...
"#,
    ...
    fn quote_module_id(module_id: &ModuleId) -> String {
        format!(
            "dijetstypes.ModuleId {{ Address: {}, Name: {} }}",
    ...
    fn quote_type_parameters(ty_args: &[TypeArgumentABI]) -> Vec<String> {
        ty_args
            .iter()
            .map(|ty_arg| format!("{} dijetstypes.TypeTag", ty_arg.name()))
            .collect()
    }
    ...
    fn quote_type(type_tag: &TypeTag) -> String {
        ...
        match type_tag {
            ...
            Address => "dijetstypes.AccountAddress".into(),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => "[]byte".into(),
                _ => common::type_not_allowed(type_tag),
            },
    ...
    fn quote_transaction_argument_for_script(type_tag: &TypeTag, name: &str) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => format!("(*dijetstypes.TransactionArgument__Bool)(&{})", name),
            U8 => format!("(*dijetstypes.TransactionArgument__U8)(&{})", name),
            U64 => format!("(*dijetstypes.TransactionArgument__U64)(&{})", name),
            U128 => format!("(*dijetstypes.TransactionArgument__U128)(&{})", name),
            Address => format!("&dijetstypes.TransactionArgument__Address{{{}}}", name),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => format!("(*dijetstypes.TransactionArgument__U8Vector)(&{})", name),
                _ => common::type_not_allowed(type_tag),
            },
            Struct(_) | Signer => common::type_not_allowed(type_tag),
        }
