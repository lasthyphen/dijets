...
struct PythonEmitter<T> {
    ...
    serde_package_name: Option<String>,
    /// Package where to find the dijets module (if any).
    dijets_package_name: Option<String>,
}
    ...
    T: Write,
{
    fn output_additional_imports(&mut self) -> Result<()> {
        let dijets_pkg_root = match &self.dijets_package_name {
            None => "".into(),
...
from {}dijets_types import (Script, ScriptFunction, TransactionPayload, TransactionPayload__ScriptFunction, Identifier, ModuleId, TypeTag, AccountAddress, TransactionArgument, TransactionArgument__Bool, TransactionArgument__U8, TransactionArgument__U64, TransactionArgument__U128, TransactionArgument__Address, TransactionArgument__U8Vector)"#,
            dijets_pkg_root, dijets_pkg_root
        )
    }
...
def encode_script_function(call: ScriptFunctionCall) -> TransactionPayload:
    """Build a Dijets `ScriptFunction` `TransactionPayload` from a structured object `ScriptFunctionCall`.
    """
...
"#
    ...
    fn output_script_call_enum_with_imports(&mut self, abis: &[ScriptABI]) -> Result<()> {
        let dijets_types_module = match &self.dijets_package_name {
            None => "dijets_types".into(),
            Some(package) => format!("{}.dijets_types", package),
        };
        let external_definitions = crate::common::get_external_definitions(&dijets_types_module);
...
impl crate::SourceInstaller for Installer {
    ...
    fn install_transaction_builders(
        ...
        output(
            ...
            self.dijets_package_name.clone(),
            abis,
        )?;
        Ok(())
    }
