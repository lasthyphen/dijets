...
struct Options {
    ...
    /// Also install the dijets types described by the given YAML file, along with the BCS runtime.
    #[structopt(long)]
    with_dijets_types: Option<PathBuf>,
    ...
    /// and `dijets_types` is assumed to be in "go_module/path/dijets_types".
    #[structopt(long)]
    ...
    /// Optional version number for the `dijets_types` module (useful in Rust).
    /// If `--with-dijets-types` is passed, this will be the version of the generated `dijets_types` module.
    #[structopt(long, default_value = "0.1.0")]
    dijets_version_number: String,

    /// Optional package name (Python) or module path (Go) of the `dijets_types` dependency.
    #[structopt(long)]
    dijets_package_name: Option<String>,

    /// Read custom code for Dijets containers from the given file paths. Containers will be matched with file stems.
    /// (e.g. `AddressAccount` <- `path/to/AddressAccount.py`)
    #[structopt(long)]
    with_custom_dijets_code: Vec<PathBuf>,
}
...
fn main() {
        ...
        None => {
            ...
            match options.language {
                Language::Python3 => buildgen::python3::output(
                    ...
                    options.dijets_package_name.clone(),
                    &abis,
                )
                .unwrap(),
                Language::Rust => {
    ...
    // Dijets types
    if let Some(registry_file) = options.with_dijets_types {
        ...
        let (dijets_package_name, dijets_package_path) = match options.language {
            Language::Rust => (
                if options.dijets_version_number == "0.1.0" {
                    "dijets-types".to_string()
                } else {
                    format!("dijets-types:{}", options.dijets_version_number)
                },
                vec!["dijets-types"],
            ),
            Language::Java => ("com.dijets.types".to_string(), vec!["com", "dijets", "types"]),
            Language::Csharp => ("Dijets.Types".to_string(), vec!["Dijets", "Types"]),
            Language::Go => ("dijetstypes".to_string(), vec!["dijetstypes"]),
            Language::TypeScript => ("dijetsTypes".to_string(), vec!["dijetsTypes"]),
            _ => ("dijets_types".to_string(), vec!["dijets_types"]),
        };
        let custom_dijets_code = buildgen::read_custom_code_from_paths(
            &dijets_package_path,
            options.with_custom_dijets_code.into_iter(),
        );
        let config = serdegen::CodeGeneratorConfig::new(dijets_package_name)
            .with_encodings(vec![serdegen::Encoding::Bcs])
            .with_custom_code(custom_dijets_code);
