...
/// If `local_types` is true, we generate a file suitable for the Dijets codebase itself
...
struct RustEmitter<T> {
    ...
    /// Whether we are targetting the Dijets repository itself (as opposed to generated Dijets types).
    local_types: bool,
}
    ...
    fn get_external_definitions(local_types: bool) -> serde_generate::ExternalDefinitions {
        let definitions = if local_types {
            vec![
                ...
                ("dijets_types::account_address", vec!["AccountAddress"]),
            ]
        } else {
            vec![(
                "dijets_types",
...
pub struct Installer {
    install_dir: PathBuf,
    dijets_types_version: String,
}

impl Installer {
    pub fn new(install_dir: PathBuf, dijets_types_version: String) -> Self {
        Installer {
            install_dir,
            dijets_types_version,
        }
    }
}
...
serde_bytes = "0.11"
dijets-types = {{ path = "../dijets-types", version = "{}" }}
"#,
            name, version, self.dijets_types_version,
        )?;
