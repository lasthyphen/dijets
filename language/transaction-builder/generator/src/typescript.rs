...
use crate::common;
use dijets_types::transaction::{ArgumentABI, ScriptABI, TransactionScriptABI, TypeArgumentABI};
...
export interface ScriptDef {{
  readonly stdlibEncodeFunction: (...args: any[]) => DijetsTypes.Script;
  readonly stdlibDecodeFunction: (script: DijetsTypes.Script) => ScriptCall;
...
fn write_script_calls(out: &mut dyn Write, abis: &[TransactionScriptABI]) -> Result<()> {
    let external_definitions = crate::common::get_external_definitions("dijetsTypes");
...
"#,
    ...
    fn output_script_encoder_function(&mut self, abi: &TransactionScriptABI) -> Result<()> {
        writeln!(
            self.out,
            "\n{}static encode{}Script({}): DijetsTypes.Script {{",
...
const tyArgs: Seq<DijetsTypes.TypeTag> = [{}];
const args: Seq<DijetsTypes.TransactionArgument> = [{}];
return new DijetsTypes.Script(code, tyArgs, args);"#,
    ...
    fn output_script_decoder_function(&mut self, abi: &TransactionScriptABI) -> Result<()> {
        writeln!(
            self.out,
            "\nstatic decode{}Script({}script: DijetsTypes.Script): ScriptCallVariant{0} {{",
            abi.name().to_camel_case(),
    ...
    fn output_script_args_callbacks(&mut self, abi: &TransactionScriptABI) -> Result<()> {
        let mut args_with_types = abi
            ...
            .map(|ty_arg| {
                format!(
                    "{}: DijetsTypes.TypeTagVariantStruct",
    ...
    fn quote_transaction_argument_type(type_tag: &TypeTag) -> String {
        use TypeTag::*;
        match type_tag {
            Bool => "DijetsTypes.TransactionArgumentVariantBool".to_string(),
            U8 => "DijetsTypes.TransactionArgumentVariantU8".to_string(),
            U64 => "DijetsTypes.TransactionArgumentVariantU64".to_string(),
            U128 => "DijetsTypes.TransactionArgumentVariantU128".to_string(),
            Address => "DijetsTypes.TransactionArgumentVariantAddress".to_string(),
            Vector(type_tag) => match type_tag.as_ref() {
                U8 => "DijetsTypes.TransactionArgumentVariantU8Vector".to_string(),
                _ => common::type_not_allowed(type_tag),
            },
