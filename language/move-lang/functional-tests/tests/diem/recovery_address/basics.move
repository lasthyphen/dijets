...
fun main(account: signer) {
    let account = &account;
    DijetsAccount::create_child_vasp_account<XUS>(account, @{{child1}}, {{child1::auth_key}}, false);
    DijetsAccount::create_child_vasp_account<XUS>(account, @{{child2}}, {{child2::auth_key}}, false)
}
}
// check: "Keep(EXECUTED)"
...
fun main(account: signer) {
    ...
    RecoveryAddress::add_rotation_capability(
        DijetsAccount::extract_key_rotation_capability(account), @{{child1}}
    );
}
}
// check: "Keep(EXECUTED)"
...
fun main(account: signer) {
    let account = &account;
    RecoveryAddress::add_rotation_capability(
        DijetsAccount::extract_key_rotation_capability(account), @0x3333
    )
}
}
...
fun main(account: signer) {
    let account = &account;
    Holder::hold(account, DijetsAccount::extract_key_rotation_capability(account));
}
}
// check: "Keep(EXECUTED)"
...
fun main(account: signer) {
    let account = &account;
    let cap = Holder::get<DijetsAccount::KeyRotationCapability>(@{{vasp1}});
