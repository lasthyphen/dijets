...
module {{default}}::ColdWallet {
    ...
    // Dijets account, without assuming there's a cold wallet module
    // under that account
    public fun withdraw_from_payer(
        ...
        let account_balance = Dijets::value(&payer_ref.balance);
        ...
        if (has_authority) {
            ...
            let withdraw_amount = Dijets::withdraw(&mut payer_ref.balance, amount);
            // DijetsAccount no longer has this API
            //DijetsAccount::deposit(payer_, payee, withdraw_amount);
            Dijets::destroy_zero(withdraw_amount);
        };
    }
}
