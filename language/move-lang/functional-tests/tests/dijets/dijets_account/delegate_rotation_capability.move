...
module {{alice}}::SharedKeyRotation {
    ...
    struct T has key {
        ...
        cap: DijetsAccount::KeyRotationCapability,
    ...
    public fun publish(account: &signer, cap: DijetsAccount::KeyRotationCapability, master_key_address: address) {
    ...
    public fun rotate(account: &signer, wallet_address: address, new_key: vector<u8>) acquires T {
        ...
        let cap_addr = *DijetsAccount::key_rotation_capability_address(&wallet_ref.cap);
        assert((wallet_ref.master_key_address == sender) || (cap_addr == sender), 77);
        DijetsAccount::rotate_authentication_key(&wallet_ref.cap, new_key);
    }
}
...
fun main(account: signer) {
    let account = &account;
    assert(DijetsAccount::sequence_number(@{{alice}}) == 1, 77);
    SharedKeyRotation::publish(account, DijetsAccount::extract_key_rotation_capability(account), @{{bob}});
}
}
...
fun main(account: signer) {
    let account = &account;
    assert(DijetsAccount::sequence_number(@{{alice}}) == 2, 78);
...
fun main(account: signer) {
    let account = &account;
    assert(DijetsAccount::sequence_number(@{{alice}}) == 3, 78);
    SharedKeyRotation::rotate(
        account,
        @{{alice}},
...
fun main(account: signer) {
    ...
    // details to be found in DijetsAccount::make_account
...
use DijetsFramework::DijetsAccount;
fun main() {
    assert(DijetsAccount::sequence_number(@{{alice}}) == 3, 79);
}
}
