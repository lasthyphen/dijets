...
fun main(account: signer, amount: u64) {
    ...
    DijetsAccount::pay_from<XUS>(&with_cap, @{{carol}}, amount, x"", x"");
...
fun main() {
    assert(DijetsAccount::balance<XUS>(@{{carol}}) == 1000, 42);
    assert(DijetsAccount::balance<XUS>(@{{alice}}) == DijetsAccount::balance<XUS>(@{{bob}}) + 1000, 43)
}
}
// check: "Keep(EXECUTED)"
