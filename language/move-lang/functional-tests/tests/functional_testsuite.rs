...
use dijets_types::account_address::AccountAddress as DijetsAddress;
...
pub const STD_LIB_DIR: &str = "../../dijets-framework/modules";
...
impl<'a> MoveSourceCompiler<'a> {
    ...
    fn move_compile_with_stdlib(
        ...
        let (files, comments_and_compiler_res) = MoveCompiler::new(targets, &self.deps)
            ...
            .set_named_address_values(dijets_framework_named_addresses())
...
static DIJETS_PRECOMPILED_STDLIB: Lazy<FullyCompiledProgram> = Lazy::new(|| {
    let program_res = move_lang::construct_pre_compiled_lib(
        ...
        dijets_framework_named_addresses(),
    )
    .unwrap();
    match program_res {
        Ok(stdlib) => stdlib,
...
fn functional_testsuite(path: &Path) -> datatest_stable::Result<()> {
    testsuite::functional_tests(MoveSourceCompiler::new(&*DIJETS_PRECOMPILED_STDLIB), path)
}
