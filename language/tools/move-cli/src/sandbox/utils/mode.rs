...
/// Content for dijets framework directory
const DIR_DIJETS_FRAMEWORK: Dir = include_dir!("../../dijets-framework/modules");
...
static PACKAGE_DIJETS_FRAMEWORK: Lazy<MovePackage> = Lazy::new(|| {
    ...
    MovePackage::new(
        "dijets".to_string(),
        vec![SourceFilter {
            source_dir: &DIR_DIJETS_FRAMEWORK,
            inclusion: None, // include everything
            exclusion: HashSet::new(),
        }],
        vec![&PACKAGE_MOVE_STDLIB],
        named_addresses,
        Some("DijetsFramework".to_owned()),
    )
});
...
pub enum ModeType {
    ...
    Stdlib,
    /// Include stdlib and all Dijets dependencies
    Dijets,
}
...
impl Mode {
    pub fn new(t: ModeType) -> Self {
        match t {
            ...
            ModeType::Dijets => Mode(vec![&*PACKAGE_DIJETS_FRAMEWORK]),
        }
    }
