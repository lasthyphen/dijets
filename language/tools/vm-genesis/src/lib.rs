...
use dijets_types::{
    ...
    on_chain_config::{VMPublishingOption, DIJETS_MAX_KNOWN_VERSION},
    transaction::{
        authenticator::AuthenticationKey, ChangeSet, ScriptFunction, Transaction, WriteSetPayload,
    },
};
use dijets_vm::{convert_changeset_and_events, data_cache::StateViewCache};
...
pub fn encode_genesis_transaction(
    dijets_root_key: Ed25519PublicKey,
...
fn create_and_initialize_main_accounts(
    ...
    let dijets_root_auth_key = AuthenticationKey::ed25519(dijets_root_key);
    let treasury_compliance_auth_key = AuthenticationKey::ed25519(treasury_compliance_key);

    let root_dijets_root_address = account_config::dijets_root_address();
    ...
    exec_function(
        ...
        serialize_values(&vec![
            MoveValue::Signer(root_dijets_root_address),
            MoveValue::Signer(tc_account_address),
            MoveValue::vector_u8(dijets_root_auth_key.to_vec()),
            ...
            MoveValue::U64(DIJETS_MAX_KNOWN_VERSION.major),
        ]),
    );
...
fn create_and_initialize_owners_operators(
    ...
    let dijets_root_address = account_config::dijets_root_address();
    let mut owners = vec![];
    ...
    exec_function(
        ...
        vec![],
        serialize_values(&vec![
            MoveValue::Signer(dijets_root_address),
...
fn verify_genesis_write_set(events: &[ContractEvent]) {
    // (1) first event is account creation event for DijetsRoot
    let create_dijets_root_event = &events[0];
    assert_eq!(
        *create_dijets_root_event.key(),
...
pub fn generate_genesis_change_set_for_testing(genesis_options: GenesisOptions) -> ChangeSet {
    let modules = match genesis_options {
        GenesisOptions::Compiled => dijets_framework_releases::current_module_blobs(),
        GenesisOptions::Fresh => dijets_framework::module_blobs(),
    };
...
pub struct Validator {
    /// The Dijets account address of the validator
    ...
    /// The Dijets account address of the validator's operator (same as `address` if the validator is
    /// its own operator)
...
impl TestValidator {
    ...
    fn gen(index: usize, rng: &mut rand::rngs::StdRng) -> TestValidator {
        ...
        let address = dijets_config::utils::validator_owner_account_from_name(&name);
