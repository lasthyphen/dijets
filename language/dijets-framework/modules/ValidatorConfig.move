...
/// is published and updated by Dijets root in a `Self::ValidatorConfig` in preparation for
/// later inclusion (by functions in DijetsConfig) in a `DijetsConfig::DijetsConfig<DijetsSystem>`
/// struct (the `Self::ValidatorConfig` in a `DijetsConfig::ValidatorInfo` which is a member
/// of the `DijetsSystem::DijetsSystem.validators` vector).
module DijetsFramework::ValidatorConfig {
    ...
    /// See comment on `ValidatorConfig::set_config` -- DijetsSystem depends on this.
    spec module {
    ...
    spec module {
        ...
        /// in DijetsSystem so we don't have to check whether every validator address has a validator role.
        invariant forall addr: address where exists_config(addr):
            Roles::spec_has_validator_role_addr(addr);
