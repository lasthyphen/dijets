...
module DijetsFramework::SlidingNonce {
    use Std::Signer;
    use Std::Errors;
    ...
    spec module {
        ...
        /// Sliding nonces are initialized at Dijets root and treasury compliance addresses
        invariant DijetsTimestamp::is_operating()
            ==> exists<SlidingNonce>(@DijetsRoot);

        invariant DijetsTimestamp::is_operating()
            ==> exists<SlidingNonce>(@TreasuryCompliance);

        // In the current code, only Dijets root and Treasury compliance have sliding nonces.
        // That is a difficult cross-module invariant to prove (it depends on Genesis and
        // DijetsAccount). Additional modules could be added that call the publish functions
