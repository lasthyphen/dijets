/// This module defines role-based access control for the Dijets framework.
///
/// Roles are associated with accounts and govern what operations are permitted by those accounts. A role
/// is typically asserted on function entry using a statement like `Self::assert_dijets_root(account)`. This
...
module DijetsFramework::Roles {
    use DijetsFramework::CoreAddresses;
    use DijetsFramework::DijetsTimestamp;
    use Std::Errors;
    use Std::Signer;
    friend DijetsFramework::DijetsAccount;

    /// A `RoleId` resource was in an unexpected state
    const EROLE_ID: u64 = 0;
    /// The signer didn't have the required Dijets Root role
    const EDIJETS_ROOT: u64 = 1;
    ...
    const DIJETS_ROOT_ROLE_ID: u64 = 0;
    ...
    /// Publishes dijets root role. Granted only in genesis.
    public(friend) fun grant_dijets_root_role(
        dr_account: &signer,
    ) {
        DijetsTimestamp::assert_genesis();
        // Checks actual Dijets root because Dijets root role is not set
        // until next line of code.
        CoreAddresses::assert_dijets_root(dr_account);
        // Grant the role to the dijets root account
        grant_role(dr_account, DIJETS_ROOT_ROLE_ID);
    }
    spec grant_dijets_root_role {
        include DijetsTimestamp::AbortsIfNotGenesis;
        include CoreAddresses::AbortsIfNotDijetsRoot{account: dr_account};
        include GrantRole{addr: Signer::address_of(dr_account), role_id: DIJETS_ROOT_ROLE_ID};
    }
    ...
    spec grant_treasury_compliance_role {
        ...
        include AbortsIfNotDijetsRoot{account: dr_account};
    ...
    /// The `creating_account` must be dijets root.
    public(friend) fun new_validator_role(
        creating_account: &signer,
        new_account: &signer
    ) acquires RoleId {
        assert_dijets_root(creating_account);
        grant_role(new_account, VALIDATOR_ROLE_ID);
    }
    spec new_validator_role {
        include AbortsIfNotDijetsRoot{account: creating_account};
    ...
    /// The `creating_account` must be DijetsRoot
    ...
    spec grant_role {
        ...
        requires role_id == DIJETS_ROOT_ROLE_ID ==> addr == @DijetsRoot;
    ...
    public fun has_dijets_root_role(account: &signer): bool acquires RoleId {
        has_role(account, DIJETS_ROOT_ROLE_ID)
    }
    ...
    /// Return true if `addr` is allowed to receive and send `Dijets<T>` for any T
    ...
    /// Assert that the account is dijets root.
    public fun assert_dijets_root(account: &signer) acquires RoleId {
        CoreAddresses::assert_dijets_root(account);
        let addr = Signer::address_of(account);
        assert(exists<RoleId>(addr), Errors::not_published(EROLE_ID));
        assert(borrow_global<RoleId>(addr).role_id == DIJETS_ROOT_ROLE_ID, Errors::requires_role(EDIJETS_ROOT));
    }
    spec assert_dijets_root {
        pragma opaque;
        include CoreAddresses::AbortsIfNotDijetsRoot;
    ...
    spec module {
        /// The DijetsRoot role is only granted in genesis [[A1]][ROLE]. A new `RoleId` with `DIJETS_ROOT_ROLE_ID` is only
        /// published through `grant_dijets_root_role` which aborts if it is not invoked in genesis.
        apply ThisRoleIsNotNewlyPublished{this: DIJETS_ROOT_ROLE_ID} to * except grant_dijets_root_role, grant_role;
        apply DijetsTimestamp::AbortsIfNotGenesis to grant_dijets_root_role;
        ...
        apply DijetsTimestamp::AbortsIfNotGenesis to grant_treasury_compliance_role;

        /// Validator roles are only granted by DijetsRoot [[A3]][ROLE]. A new `RoleId` with `VALIDATOR_ROLE_ID` is only
        /// published through `new_validator_role` which aborts if `creating_account` does not have the DijetsRoot role.
        apply ThisRoleIsNotNewlyPublished{this: VALIDATOR_ROLE_ID} to * except new_validator_role, grant_role;
        apply AbortsIfNotDijetsRoot{account: creating_account} to new_validator_role;

        /// ValidatorOperator roles are only granted by DijetsRoot [[A4]][ROLE]. A new `RoleId` with `VALIDATOR_OPERATOR_ROLE_ID` is only
        /// published through `new_validator_operator_role` which aborts if `creating_account` does not have the DijetsRoot role.
        apply ThisRoleIsNotNewlyPublished{this: VALIDATOR_OPERATOR_ROLE_ID} to *
            except new_validator_operator_role, grant_role;
        apply AbortsIfNotDijetsRoot{account: creating_account} to new_validator_operator_role;
        ...
        /// The DijetsRoot role is globally unique [[B1]][ROLE], and is published at DIJETS_ROOT_ADDRESS [[C1]][ROLE].
        /// In other words, a `RoleId` with `DIJETS_ROOT_ROLE_ID` uniquely exists at `DIJETS_ROOT_ADDRESS`.
        invariant [global, isolated] forall addr: address where spec_has_dijets_root_role_addr(addr):
          addr == @DijetsRoot;
        invariant [global, isolated]
            DijetsTimestamp::is_operating() ==> spec_has_dijets_root_role_addr(@DijetsRoot);
        ...
        /// DijetsRoot cannot have balances [[D1]][ROLE].
        invariant [global, isolated] forall addr: address where spec_has_dijets_root_role_addr(addr):
    ...
    spec module {
        ...
        fun spec_has_dijets_root_role_addr(addr: address): bool {
            spec_has_role_id_addr(addr, DIJETS_ROOT_ROLE_ID)
        }
    ...
    spec schema AbortsIfNotDijetsRoot {
        ...
        aborts_if global<RoleId>(addr).role_id != DIJETS_ROOT_ROLE_ID with Errors::REQUIRES_ROLE;
    }
