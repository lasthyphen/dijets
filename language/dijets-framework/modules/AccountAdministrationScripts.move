/// This module holds transactions that can be used to administer accounts in the Dijets Framework.
module DijetsFramework::AccountAdministrationScripts {
    use DijetsFramework::DijetsAccount;
    use DijetsFramework::RecoveryAddress;
    use DijetsFramework::SharedEd25519PublicKey;
    ...
    /// send, receive, and hold `Dijets::Dijets<Currency>` coins. This transaction can be
    /// successfully sent by any account that is allowed to hold balances
    /// (e.g., VASP, Designated Dealer).
    ///
    /// # Technical Description
    /// After the successful execution of this transaction the sending account will have a
    /// `DijetsAccount::Balance<Currency>` resource with zero balance published under it. Only
    /// accounts that can hold balances can send this transaction, the sending account cannot
    /// already have a `DijetsAccount::Balance<Currency>` published under it.
    ///
    /// # Parameters
    ...
    /// | ----------------            | --------------                           | -------------                                                              |
    /// | `Errors::NOT_PUBLISHED`     | `Dijets::ECURRENCY_INFO`                  | The `Currency` is not a registered currency on-chain.                      |
    /// | `Errors::INVALID_ARGUMENT`  | `DijetsAccount::EROLE_CANT_STORE_BALANCE` | The sending `account`'s role does not permit balances.                     |
    /// | `Errors::ALREADY_PUBLISHED` | `DijetsAccount::EADD_EXISTING_CURRENCY`   | A balance for `Currency` is already published under the sending `account`. |
    ///
    /// # Related Scripts
    ...
    public(script) fun add_currency_to_account<Currency>(account: signer) {
        DijetsAccount::add_currency<Currency>(&account);
    }
    spec add_currency_to_account {
        ...
        include DijetsAccount::AddCurrencyAbortsIf<Currency>;
        include DijetsAccount::AddCurrencyEnsures<Currency>{addr: Signer::spec_address_of(account)};
    ...
    ///
    /// # Technical Description
    /// Adds the `DijetsAccount::KeyRotationCapability` for the sending account
    ...
    /// | `recovery_address`   | `address` | The account address where the `to_recover_account`'s `DijetsAccount::KeyRotationCapability` will be stored. |
    ///
    /// # Common Abort Conditions
    /// | Error Category             | Error Reason                                              | Description                                                                                       |
    /// | ----------------           | --------------                                            | -------------                                                                                     |
    /// | `Errors::INVALID_STATE`    | `DijetsAccount::EKEY_ROTATION_CAPABILITY_ALREADY_EXTRACTED` | `to_recover_account` has already delegated/extracted its `DijetsAccount::KeyRotationCapability`.    |
    ...
    public(script) fun add_recovery_rotation_capability(to_recover_account: signer, recovery_address: address) {
        RecoveryAddress::add_rotation_capability(
            DijetsAccount::extract_key_rotation_capability(&to_recover_account), recovery_address
        )
    }
    spec add_recovery_rotation_capability {
        ...
        include DijetsAccount::TransactionChecks{sender: to_recover_account}; // properties checked by the prologue.
        include DijetsAccount::ExtractKeyRotationCapabilityAbortsIf{account: to_recover_account};
        include DijetsAccount::ExtractKeyRotationCapabilityEnsures{account: to_recover_account};

        let addr = Signer::spec_address_of(to_recover_account);
        let rotation_cap = DijetsAccount::spec_get_key_rotation_cap(addr);
    ...
    /// [authentication key derived from `public_key`](https://developers.dijets.com/docs/core/accounts/#addresses-authentication-keys-and-cryptographic-keys)
    /// and publishes a `SharedEd25519PublicKey::SharedEd25519PublicKey` resource
    /// containing the 32-byte ed25519 `public_key` and the `DijetsAccount::KeyRotationCapability` for
    ...
    /// | Error Category              | Error Reason                                               | Description                                                                                         |
    /// | ----------------            | --------------                                             | -------------                                                                                       |
    /// | `Errors::INVALID_STATE`     | `DijetsAccount::EKEY_ROTATION_CAPABILITY_ALREADY_EXTRACTED` | `account` has already delegated/extracted its `DijetsAccount::KeyRotationCapability` resource.       |
    ...
    ///
    /// # Technical Description
    /// Rotate the `account`'s `DijetsAccount::DijetsAccount` `authentication_key`
    /// field to `new_key`. `new_key` must be a valid authentication key that
    /// corresponds to an ed25519 public key as described [here](https://developers.dijets.com/docs/core/accounts/#addresses-authentication-keys-and-cryptographic-keys),
    /// and `account` must not have previously delegated its `DijetsAccount::KeyRotationCapability`.
    ///
    /// # Parameters
    /// | Name      | Type         | Description                                       |
    /// | ------    | ------       | -------------                                     |
    ...
    /// | Error Category             | Error Reason                                              | Description                                                                         |
    /// | ----------------           | --------------                                            | -------------                                                                       |
    /// | `Errors::INVALID_STATE`    | `DijetsAccount::EKEY_ROTATION_CAPABILITY_ALREADY_EXTRACTED` | `account` has already delegated/extracted its `DijetsAccount::KeyRotationCapability`. |
    /// | `Errors::INVALID_ARGUMENT` | `DijetsAccount::EMALFORMED_AUTHENTICATION_KEY`              | `new_key` was an invalid length.                                                    |
    ///
    /// # Related Scripts
    ...
    public(script) fun rotate_authentication_key(account: signer, new_key: vector<u8>) {
        let key_rotation_capability = DijetsAccount::extract_key_rotation_capability(&account);
        DijetsAccount::rotate_authentication_key(&key_rotation_capability, new_key);
        DijetsAccount::restore_key_rotation_capability(key_rotation_capability);
    }
    spec rotate_authentication_key {
        ...
        include DijetsAccount::ExtractKeyRotationCapabilityAbortsIf;
        let key_rotation_capability = DijetsAccount::spec_get_key_rotation_cap(account_addr);
        include DijetsAccount::RotateAuthenticationKeyAbortsIf{cap: key_rotation_capability, new_authentication_key: new_key};

        /// This rotates the authentication key of `account` to `new_key`
        include DijetsAccount::RotateAuthenticationKeyEnsures{addr: account_addr, new_authentication_key: new_key};
        ...
        include DijetsAccount::AbortsIfDelegatedKeyRotationCapability;
    }
    ...
    /// Compliance or Dijets Root accounts).
    ///
    /// # Technical Description
    /// Rotates the `account`'s `DijetsAccount::DijetsAccount` `authentication_key`
    /// field to `new_key`. `new_key` must be a valid authentication key that
    /// corresponds to an ed25519 public key as described [here](https://developers.dijets.com/docs/core/accounts/#addresses-authentication-keys-and-cryptographic-keys),
    /// and `account` must not have previously delegated its `DijetsAccount::KeyRotationCapability`.
    ///
    ...
    /// only be sent by the Dijets Root account as a write set transaction.
    ///
    /// # Technical Description
    /// Rotate the `account`'s `DijetsAccount::DijetsAccount` `authentication_key` field to `new_key`.
    /// `new_key` must be a valid authentication key that corresponds to an ed25519
    /// public key as described [here](https://developers.dijets.com/docs/core/accounts/#addresses-authentication-keys-and-cryptographic-keys),
    /// and `account` must not have previously delegated its `DijetsAccount::KeyRotationCapability`.
    ///
    /// # Parameters
    /// | Name            | Type         | Description                                                                                       |
    /// | ------          | ------       | -------------                                                                                     |
    /// | `dr_account`    | `signer`     | The signer of the sending account of the write set transaction. May only be the Dijets Root signer. |
    /// | `account`       | `signer`     | Signer of account specified in the `execute_as` field of the write set transaction.               |
    /// | `sliding_nonce` | `u64`        | The `sliding_nonce` (see: `SlidingNonce`) to be used for this transaction for Dijets Root.          |
    /// | `new_key`       | `vector<u8>` | New authentication key to be used for `account`.                                                  |
    ///
    /// # Common Abort Conditions
    /// | Error Category             | Error Reason                                              | Description                                                                                                |
    ...
    spec rotate_authentication_key_with_nonce_admin {
        ...
        /// **Access Control:**
        /// Only the Dijets Root account can process the admin scripts [[H9]][PERMISSION].
        requires Roles::has_dijets_root_role(dr_account); /// This is ensured by DijetsAccount::writeset_prologue.
        /// The account can rotate its own authentication key unless
        /// it has delegrated the capability [[H18]][PERMISSION][[J18]][PERMISSION].
        include DijetsAccount::AbortsIfDelegatedKeyRotationCapability{account: account};
    }
    ...
    /// `DijetsAccount::KeyRotationCapability` stored in the `RecoveryAddress::RecoveryAddress` resource
    /// published under `recovery_address`. `new_key` must be a valide authentication key as described
    /// [here](https://developers.dijets.com/docs/core/accounts/#addresses-authentication-keys-and-cryptographic-keys).
    /// This transaction can be sent either by the `to_recover` account, or by the account where the
    /// `RecoveryAddress::RecoveryAddress` resource is published that contains `to_recover`'s `DijetsAccount::KeyRotationCapability`.
    ///
    /// # Parameters
    /// | Name               | Type         | Description                                                                                                                   |
    /// | ------             | ------       | -------------                                                                                                                 |
    /// | `account`          | `signer`     | Signer of the sending account of the transaction.                                                                             |
    /// | `recovery_address` | `address`    | Address where `RecoveryAddress::RecoveryAddress` that holds `to_recover`'s `DijetsAccount::KeyRotationCapability` is published. |
    ...
    /// | `Errors::INVALID_ARGUMENT` | `RecoveryAddress::EACCOUNT_NOT_RECOVERABLE`  | `to_recover`'s `DijetsAccount::KeyRotationCapability`  is not in the `RecoveryAddress::RecoveryAddress`  resource published under `recovery_address`. |
    /// | `Errors::INVALID_ARGUMENT` | `DijetsAccount::EMALFORMED_AUTHENTICATION_KEY` | `new_key` was an invalid length.                                                                                                                    |
    ///
    /// # Related Scripts
    /// * `AccountAdministrationScripts::rotate_authentication_key`
    ...
    /// [here](https://developers.dijets.com/docs/core/accounts/#addresses-authentication-keys-and-cryptographic-keys)
    /// using the `DijetsAccount::KeyRotationCapability` stored in `account`'s `SharedEd25519PublicKey::SharedEd25519PublicKey`.
    ///
    /// # Parameters
    /// | Name         | Type         | Description                                           |
    /// | ------       | ------       | -------------                                         |
    ...
    /// extracts the `DijetsAccount::KeyRotationCapability` for `account` and adds
    ...
    public(script) fun create_recovery_address(account: signer) {
        RecoveryAddress::publish(&account, DijetsAccount::extract_key_rotation_capability(&account))
    }
    ...
    spec create_recovery_address {
        ...
        include DijetsAccount::TransactionChecks{sender: account}; // properties checked by the prologue.
        include DijetsAccount::ExtractKeyRotationCapabilityAbortsIf;
        include DijetsAccount::ExtractKeyRotationCapabilityEnsures;

        let account_addr = Signer::spec_address_of(account);
        let rotation_cap = DijetsAccount::spec_get_key_rotation_cap(account_addr);
