...
/// * DijetsSystem, DijetsAccount, DijetsConfig: to check if the current state is in the genesis state
/// * DijetsBlock: to reach consensus on the global wall clock time
...
module DijetsFramework::DijetsTimestamp {
    ...
    spec set_time_has_started {
        ...
        /// After time has started, all invariants guarded by `DijetsTimestamp::is_operating`
        /// will become activated and need to hold.
        pragma friend = DijetsFramework::Genesis::initialize;
    ...
    public fun update_global_time(
        ...
        assert_operating();
        // Can only be invoked by DijetsVM signer.
        CoreAddresses::assert_vm(account);

        let global_timer = borrow_global_mut<CurrentTimeMicroseconds>(@DijetsRoot);
    ...
    spec update_global_time {
        pragma opaque;
        modifies global<CurrentTimeMicroseconds>(@DijetsRoot);
    ...
    public fun now_microseconds(): u64 acquires CurrentTimeMicroseconds {
        assert_operating();
        borrow_global<CurrentTimeMicroseconds>(@DijetsRoot).microseconds
    }
    ...
    spec fun spec_now_microseconds(): u64 {
        global<CurrentTimeMicroseconds>(@DijetsRoot).microseconds
    }
    ...
    spec fun spec_now_seconds(): u64 {
        global<CurrentTimeMicroseconds>(@DijetsRoot).microseconds / MICRO_CONVERSION_FACTOR
    }

    /// Helper function to determine if Dijets is in genesis state.
    public fun is_genesis(): bool {
        !exists<CurrentTimeMicroseconds>(@DijetsRoot)
    }
    ...
    /// Helper function to determine if Dijets is operating. This is the same as `!is_genesis()` and is provided
    ...
    spec module {
        ...
        invariant is_operating() ==> exists<CurrentTimeMicroseconds>(@DijetsRoot);
