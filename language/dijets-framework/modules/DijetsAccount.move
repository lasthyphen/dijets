...
module DijetsFramework::DijetsAccount {
    ...
    /// An `address` is a Dijets Account iff it has a published DijetsAccount resource.
    struct DijetsAccount has key {
    ...
    /// The holder of WithdrawCapability for account_address can withdraw Dijets from
    /// account_address/DijetsAccount/balance.
    ...
    /// account_address (i.e., write to account_address/DijetsAccount/authentication_key).
    ...
    struct DijetsWriteSetManager has key {
    ...
    struct SentPaymentEvent has drop, store {
        /// The amount of Dijets<Token> sent
        amount: u64,
    ...
    struct ReceivedPaymentEvent has drop, store {
        /// The amount of Dijets<Token> received
        amount: u64,
    ...
    /// The `DijetsAccount` resource is not in the required state
    ...
    /// e.g., `Dijets<XDX>` to an account that exists, but does not have a `Balance<XDX>` resource
    ...
    /// The `DijetsWriteSetManager` was not in the required state
    ...
    spec initialize {
        ...
        include CreateDijetsRootAccountAbortsIf{auth_key_prefix: dummy_auth_key_prefix};
        ...
        include CreateDijetsRootAccountModifies;
        include CreateDijetsRootAccountEnsures;
    ...
    fun deposit<Token>(
        payer: address,
        payee: address,
        to_deposit: Dijets<Token>,
        metadata: vector<u8>,
        metadata_signature: vector<u8>,
        dual_attestation: bool,
    ) acquires DijetsAccount, Balance, AccountOperationsCapability {
        DijetsTimestamp::assert_operating();
        AccountFreezing::assert_not_frozen(payee);

        // Check that the `to_deposit` coin is non-zero
        let deposit_value = Dijets::value(&to_deposit);
        ...
        if (should_track_limits_for_account<Token>(payer, payee, false)) {
            assert(
                AccountLimits::update_deposit_limits<Token>(
                    ...
                    &borrow_global<AccountOperationsCapability>(@DijetsRoot).limits_cap
                ),
        ...
        Dijets::deposit(&mut borrow_global_mut<Balance<Token>>(payee).coin, to_deposit);

        // Log a received event
        Event::emit_event<ReceivedPaymentEvent>(
            &mut borrow_global_mut<DijetsAccount>(payee).received_events,
            ReceivedPaymentEvent {
                amount: deposit_value,
                currency_code: Dijets::currency_code<Token>(),
                payer,
                metadata
            }
        );
    ...
    spec deposit {
        ...
        modifies global<DijetsAccount>(payee);
        ...
        ensures exists<DijetsAccount>(payee);
        ensures exists<Balance<Token>>(payee);
        ensures global<DijetsAccount>(payee).withdraw_capability
            == old(global<DijetsAccount>(payee).withdraw_capability);
        ensures global<DijetsAccount>(payee).authentication_key
            == old(global<DijetsAccount>(payee).authentication_key);
        ensures Event::spec_guid_eq(global<DijetsAccount>(payee).sent_events,
                                    old(global<DijetsAccount>(payee).sent_events));
        ensures Event::spec_guid_eq(global<DijetsAccount>(payee).received_events,
                                    old(global<DijetsAccount>(payee).received_events));
    ...
    spec schema DepositAbortsIfRestricted<Token> {
        ...
        include Dijets::AbortsIfNoCurrency<Token>;
    }
    ...
    spec schema DepositEmits<Token> {
        ...
        amount: u64;
        metadata: vector<u8>;
        let handle = global<DijetsAccount>(payee).received_events;
        let msg = ReceivedPaymentEvent {
            amount,
            currency_code: Dijets::spec_currency_code<Token>(),
            payer,
            metadata
        };
        emits msg to handle;
    ...
    spec tiered_mint {
        pragma opaque;
        modifies global<DijetsAccount>(designated_dealer_address);
        ...
        modifies global<Dijets::CurrencyInfo<Token>>(@CurrencyInfo);
    ...
    spec schema TieredMintEnsures<Token> {
        ...
        let currency_info = global<Dijets::CurrencyInfo<Token>>(@CurrencyInfo);
        let post post_currency_info = global<Dijets::CurrencyInfo<Token>>(@CurrencyInfo);
    ...
    public fun cancel_burn<Token>(
        ...
        let coin = Dijets::cancel_burn<Token>(account, preburn_address, amount);
    ...
    spec cancel_burn {
        ...
        include Dijets::CancelBurnWithCapEmits<Token>;
        include Dijets::CancelBurnWithCapEnsures<Token>;
    ...
    spec schema CancelBurnAbortsIf<Token> {
        ...
        preburn_address: address;
        amount: u64;
        include Dijets::CancelBurnAbortsIf<Token>;
    ...
    /// Helper to withdraw `amount` from the given account balance and return the withdrawn Dijets<Token>
    fun withdraw_from_balance<Token>(
        payer: address,
        payee: address,
        balance: &mut Balance<Token>,
        amount: u64
    ): Dijets<Token> acquires AccountOperationsCapability {
        DijetsTimestamp::assert_operating();
        ...
        assert(Dijets::value(coin) >= amount, Errors::limit_exceeded(EINSUFFICIENT_BALANCE));
        Dijets::withdraw(coin, amount)
    }
    ...
    /// Withdraw `amount` `Dijets<Token>`'s from the account balance under
    /// `cap.account_address`
    fun withdraw_from<Token>(
        ...
        amount: u64,
        metadata: vector<u8>,
    ): Dijets<Token> acquires Balance, AccountOperationsCapability, DijetsAccount {
        DijetsTimestamp::assert_operating();
        ...
        Event::emit_event<SentPaymentEvent>(
            &mut borrow_global_mut<DijetsAccount>(payer).sent_events,
            SentPaymentEvent {
                amount,
                currency_code: Dijets::currency_code<Token>(),
                payee,
    ...
    spec withdraw_from {
        ...
        modifies global<DijetsAccount>(payer);
        modifies global<AccountLimits::Window<Token>>(VASP::spec_parent_address(payer));
        ensures exists<DijetsAccount>(payer);
        ensures global<DijetsAccount>(payer).withdraw_capability
                    == old(global<DijetsAccount>(payer).withdraw_capability);
        ensures Event::spec_guid_eq(global<DijetsAccount>(payer).sent_events,
                                    old(global<DijetsAccount>(payer).sent_events));
        ensures Event::spec_guid_eq(global<DijetsAccount>(payer).received_events,
                                    old(global<DijetsAccount>(payer).received_events));
    ...
    spec schema WithdrawFromEmits<Token> {
        ...
        metadata: vector<u8>;
        let payer = cap.account_address;
        let handle = global<DijetsAccount>(payer).sent_events;
    ...
    /// Withdraw `amount` `Dijets<Token>`'s from `cap.address` and send them to the `Preburn`
    /// resource under `dd`.
    public fun preburn<Token>(
        dd: &signer,
        cap: &WithdrawCapability,
        amount: u64
    ) acquires Balance, AccountOperationsCapability, DijetsAccount {
        Roles::assert_designated_dealer(dd);
        DijetsTimestamp::assert_operating();
        Dijets::preburn_to<Token>(dd, withdraw_from(cap, Signer::address_of(dd), amount, x""))
    }
    spec preburn {
        pragma opaque;
        ...
        ensures Event::spec_guid_eq(global<DijetsAccount>(payer).received_events,
                                    old(global<DijetsAccount>(payer).received_events));
        ensures Event::spec_guid_eq(global<DijetsAccount>(dd_addr).sent_events,
                                    old(global<DijetsAccount>(dd_addr).sent_events));
        ensures Event::spec_guid_eq(global<DijetsAccount>(dd_addr).received_events,
                                    old(global<DijetsAccount>(dd_addr).received_events));
    ...
    spec schema PreburnAbortsIf<Token> {
        ...
        include Dijets::PreburnToAbortsIf<Token>{account: dd};
    }
    spec schema PreburnEnsures<Token> {
        ...
        include Dijets::PreburnToEnsures<Token>{amount, account: dd};
    }
    spec schema PreburnEmits<Token> {
        ...
        amount: u64;
        let dd_addr = Signer::spec_address_of(dd);
        include Dijets::PreburnWithResourceEmits<Token>{preburn_address: dd_addr};
    ...
    public fun extract_withdraw_capability(
        sender: &signer
    ): WithdrawCapability acquires DijetsAccount {
        ...
        let account = borrow_global_mut<DijetsAccount>(sender_addr);
    ...
    spec extract_withdraw_capability {
        ...
        modifies global<DijetsAccount>(sender_addr);
        include ExtractWithdrawCapAbortsIf{sender_addr};
        ensures exists<DijetsAccount>(sender_addr);
        ensures result == old(spec_get_withdraw_cap(sender_addr));
        ensures global<DijetsAccount>(sender_addr) == update_field(old(global<DijetsAccount>(sender_addr)),
    ...
    acquires DijetsAccount {
        ...
        let account = borrow_global_mut<DijetsAccount>(cap.account_address);
    ...
    spec restore_withdraw_capability {
        ...
        modifies global<DijetsAccount>(cap_addr);
        ensures global<DijetsAccount>(cap_addr) == update_field(old(global<DijetsAccount>(cap_addr)),
    ...
    /// Withdraw `amount` Dijets<Token> from the address embedded in `WithdrawCapability` and
    ...
    public fun rotate_authentication_key(
        ...
        assert(exists_at(cap.account_address), Errors::not_published(EACCOUNT));
        let sender_account_resource = borrow_global_mut<DijetsAccount>(cap.account_address);
    ...
    spec schema RotateAuthenticationKeyEnsures {
        addr: address;
        new_authentication_key: vector<u8>;
        ensures global<DijetsAccount>(addr).authentication_key == new_authentication_key;
    }
    ...
    spec schema RotateOnlyKeyOfCapAddress {
        ...
        ensures forall addr: address where addr != cap.account_address && old(exists_at(addr)):
            global<DijetsAccount>(addr).authentication_key == old(global<DijetsAccount>(addr).authentication_key);
    }
    ...
    acquires DijetsAccount {
        ...
        let account = borrow_global_mut<DijetsAccount>(account_address);
    ...
    spec schema AddCurrencyForAccountAbortsIf<Token> {
        ...
        include add_all_currencies && !exists<Balance<XUS>>(addr)
            ==> Dijets::AbortsIfNoCurrency<XUS>;
        include add_all_currencies && !exists<Balance<XDX>>(addr)
            ==> Dijets::AbortsIfNoCurrency<XDX>;
    }
    ...
    fun make_account(
        ...
        assert(
            new_account_addr != @DijetsFramework,
        ...
        assert(
            exists<AccountOperationsCapability>(@DijetsRoot),
            Errors::not_published(EACCOUNT_OPERATIONS_CAPABILITY)
        );
        // Emit the CreateAccountEvent
        Event::emit_event(
            &mut borrow_global_mut<AccountOperationsCapability>(@DijetsRoot).creation_events,
    ...
    spec make_account {
        ...
        let new_account_addr = Signer::address_of(new_account);
        modifies global<DijetsAccount>(new_account_addr);
        modifies global<Event::EventHandleGenerator>(new_account_addr);
        modifies global<AccountFreezing::FreezingBit>(new_account_addr);
        modifies global<AccountOperationsCapability>(@DijetsRoot);
        ensures exists<AccountOperationsCapability>(@DijetsRoot);
        ...
        let account_ops_cap = global<AccountOperationsCapability>(@DijetsRoot);
        let post post_account_ops_cap = global<AccountOperationsCapability>(@DijetsRoot);
    ...
    spec schema MakeAccountAbortsIf {
        ...
        aborts_if addr == @DijetsFramework with Errors::INVALID_ARGUMENT;
        ...
        aborts_if DijetsTimestamp::is_genesis()
            && !exists<AccountOperationsCapability>(@DijetsRoot)
    ...
    spec schema MakeAccountEmits {
        ...
        let post handle = global<AccountOperationsCapability>(@DijetsRoot).creation_events;
    ...
    /// Creates the dijets root account (during genesis). Publishes the Dijets root role,
    ...
    fun create_dijets_root_account(
        auth_key_prefix: vector<u8>,
    ) acquires AccountOperationsCapability {
        DijetsTimestamp::assert_genesis();
        let dr_account = create_signer(@DijetsRoot);
        CoreAddresses::assert_dijets_root(&dr_account);
        Roles::grant_dijets_root_role(&dr_account);
        ...
        assert(
            !exists<DijetsWriteSetManager>(@DijetsRoot),
            Errors::already_published(EWRITESET_MANAGER)
        );
        move_to(
            &dr_account,
            DijetsWriteSetManager {
    ...
    spec create_dijets_root_account {
        pragma disable_invariants_in_body;
        pragma opaque;
        include CreateDijetsRootAccountModifies;
        include CreateDijetsRootAccountAbortsIf;
        include CreateDijetsRootAccountEnsures;
        include MakeAccountEmits{new_account_address: @DijetsRoot};
    }

    spec schema CreateDijetsRootAccountModifies {
        let dr_addr = @DijetsRoot;
        modifies global<Event::EventHandleGenerator>(dr_addr);
        modifies global<DijetsAccount>(dr_addr);
        modifies global<AccountOperationsCapability>(dr_addr);
        modifies global<DijetsWriteSetManager>(dr_addr);
    ...
    spec schema CreateDijetsRootAccountAbortsIf {
        auth_key_prefix: vector<u8>;
        include DijetsTimestamp::AbortsIfNotGenesis;
        include Roles::GrantRole{addr: @DijetsRoot, role_id: Roles::DIJETS_ROOT_ROLE_ID};
        aborts_if exists<SlidingNonce::SlidingNonce>(@DijetsRoot)
            with Errors::ALREADY_PUBLISHED;
        aborts_if exists<AccountOperationsCapability>(@DijetsRoot)
            with Errors::ALREADY_PUBLISHED;
        aborts_if exists<DijetsWriteSetManager>(@DijetsRoot)
            with Errors::ALREADY_PUBLISHED;
        aborts_if exists<AccountFreezing::FreezingBit>(@DijetsRoot)
            with Errors::ALREADY_PUBLISHED;
        include CreateAuthenticationKeyAbortsIf;
    }
    spec schema CreateDijetsRootAccountEnsures {
        let dr_addr = @DijetsRoot;
        ensures exists<AccountOperationsCapability>(dr_addr);
        ensures exists<DijetsWriteSetManager>(dr_addr);
        ensures exists<SlidingNonce::SlidingNonce>(dr_addr);
        ensures Roles::spec_has_dijets_root_role_addr(dr_addr);
    ...
    spec schema CreateTreasuryComplianceAccountModifies {
        let tc_addr = @TreasuryCompliance;
        modifies global<DijetsAccount>(tc_addr);
    ...
    fun balance_for<Token>(balance: &Balance<Token>): u64 {
        Dijets::value<Token>(&balance.coin)
    }
    ...
    public fun add_currency<Token>(account: &signer) {
        ...
        Dijets::assert_is_currency<Token>();
        ...
        move_to(account, Balance<Token>{ coin: Dijets::zero<Token>() })
    }
    spec add_currency {
    ...
    spec schema AddCurrencyEnsures<Token> {
        ...
        ensures global<Balance<Token>>(addr)
            == Balance<Token>{ coin: Dijets<Token> { value: 0 } };
    }
    ...
    fun sequence_number_for_account(account: &DijetsAccount): u64 {
        account.sequence_number
    }

    /// Return the current sequence number at `addr`
    public fun sequence_number(addr: address): u64 acquires DijetsAccount {
        assert(exists_at(addr), Errors::not_published(EACCOUNT));
        sequence_number_for_account(borrow_global<DijetsAccount>(addr))
    }

    /// Return the authentication key for this account
    public fun authentication_key(addr: address): vector<u8> acquires DijetsAccount {
        assert(exists_at(addr), Errors::not_published(EACCOUNT));
        *&borrow_global<DijetsAccount>(addr).authentication_key
    }
    ...
    acquires DijetsAccount {
        assert(exists_at(addr), Errors::not_published(EACCOUNT));
        Option::is_none(&borrow_global<DijetsAccount>(addr).key_rotation_capability)
    }
    ...
    acquires DijetsAccount {
        assert(exists_at(addr), Errors::not_published(EACCOUNT));
        Option::is_none(&borrow_global<DijetsAccount>(addr).withdraw_capability)
    }
    ...
    public fun exists_at(check_addr: address): bool {
        exists<DijetsAccount>(check_addr)
    }
    ...
    fun module_prologue<Token>(
        ...
        chain_id: u8,
    ) acquires DijetsAccount, Balance {
        assert(
            DijetsTransactionPublishingOption::is_module_allowed(&sender),
    ...
    spec schema ModulePrologueAbortsIf<Token> {
        ...
        include DijetsTransactionPublishingOption::AbortsIfNoTransactionPublishingOption;
        /// Covered: L75 (Match 9)
        aborts_if !DijetsTransactionPublishingOption::spec_is_module_allowed(sender) with Errors::INVALID_STATE;
    }
    ...
    fun script_prologue<Token>(
        ...
        assert(
            DijetsTransactionPublishingOption::is_script_allowed(&sender, &script_hash),
    ...
    spec schema ScriptPrologueAbortsIf<Token> {
        ...
        include DijetsTransactionPublishingOption::AbortsIfNoTransactionPublishingOption;
        /// Covered: L74 (Match 8)
        aborts_if !DijetsTransactionPublishingOption::spec_is_script_allowed(sender, script_hash) with Errors::INVALID_STATE;
    }
    ...
    fun writeset_prologue(
        ...
        assert(
            Signer::address_of(&sender) == @DijetsRoot,
            Errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER)
        );
        assert(Roles::has_dijets_root_role(&sender), Errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER));
    ...
    spec writeset_prologue {
        ...
        ensures prologue_guarantees(sender);
        ensures Roles::has_dijets_root_role(sender);
    }
    ...
    spec schema WritesetPrologueAbortsIf {
        ...
        aborts_if transaction_sender != @DijetsRoot with Errors::INVALID_ARGUMENT;
        /// Must abort if the signer does not have the DijetsRoot role [[H9]][PERMISSION].
        /// Covered: L146 (Match 0)
        aborts_if !Roles::spec_has_dijets_root_role_addr(transaction_sender) with Errors::INVALID_ARGUMENT;
    ...
    fun multi_agent_script_prologue<Token>(
        ...
        while ({
            spec {
                ...
                invariant forall j in 0..i: secondary_signer_public_key_hashes[j]
                    == global<DijetsAccount>(secondary_signer_addresses[j]).authentication_key;
            };
        ...
        })
        {
            ...
            let signer_account = borrow_global<DijetsAccount>(secondary_address);
    ...
    spec schema MultiAgentScriptPrologueAbortsIf<Token> {
        ...
        aborts_if exists i in 0..num_secondary_signers:
            secondary_signer_public_key_hashes[i] != global<DijetsAccount>(secondary_signer_addresses[i]).authentication_key
        with Errors::INVALID_ARGUMENT;
    }
    ...
    fun prologue_common<Token>(
        ...
        let sender_account = borrow_global<DijetsAccount>(transaction_sender);
        ...
        assert(
            DijetsTimestamp::now_seconds() < txn_expiration_time_seconds,
    ...
    spec schema PrologueCommonAbortsIf<Token> {
        ...
        aborts_if Hash::sha3_256(txn_public_key) != global<DijetsAccount>(transaction_sender).authentication_key with Errors::INVALID_ARGUMENT;
        ...
        aborts_if DijetsTimestamp::spec_now_seconds() >= txn_expiration_time_seconds with Errors::INVALID_ARGUMENT;
        /// [PCA10] Covered: L81 (match 11)
        aborts_if txn_sequence_number >= MAX_U64 with Errors::LIMIT_EXCEEDED;
        /// [PCA11] Covered: L61 (Match 2)
        aborts_if txn_sequence_number < global<DijetsAccount>(transaction_sender).sequence_number with Errors::INVALID_ARGUMENT;
    ...
    fun epilogue_common<Token>(
        ...
        let sender_account = borrow_global_mut<DijetsAccount>(sender);
        ...
        if (transaction_fee_amount > 0) {
            ...
            assert(
                transaction_fee_amount <= Dijets::value(coin),
            ...
            TransactionFee::pay_fee(Dijets::withdraw(coin, transaction_fee_amount))
        }
    }
    ...
    ) acquires DijetsWriteSetManager, DijetsAccount, Balance {
        let dr_account = &dr_account;
        let writeset_events_ref = borrow_global_mut<DijetsWriteSetManager>(@DijetsRoot);
        Event::emit_event<AdminTransactionEvent>(
            &mut writeset_events_ref.upgrade_events,
            AdminTransactionEvent { committed_timestamp_secs: DijetsTimestamp::now_seconds() },
        );

        // Double check that the sender is the DijetsRoot account at the `@DijetsRoot`
        assert(
            Signer::address_of(dr_account) == @DijetsRoot,
            Errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER)
        );
        assert(Roles::has_dijets_root_role(dr_account), Errors::invalid_argument(PROLOGUE_EINVALID_WRITESET_SENDER));

        // Currency code don't matter here as it won't be charged anyway.
        epilogue_common<XUS>(dr_account, txn_sequence_number, 0, 0, 0);
        if (should_trigger_reconfiguration) DijetsConfig::reconfigure(dr_account)
    }
    ...
    spec schema WritesetEpiloguEmits {
        should_trigger_reconfiguration: bool;
        let handle = global<DijetsWriteSetManager>(@DijetsRoot).upgrade_events;
        let msg = AdminTransactionEvent {
            committed_timestamp_secs: DijetsTimestamp::spec_now_seconds()
        };
        emits msg to handle;
        include should_trigger_reconfiguration ==> DijetsConfig::ReconfigureEmits;
    }
    ...
    public fun create_validator_account(
        ...
        Roles::assert_dijets_root(dr_account);
        let new_account = create_signer(new_account_address);
        // The dr_account account is verified to have the dijets root role in `Roles::new_validator_role`
    ...
    public fun create_validator_operator_account(
        ...
        Roles::assert_dijets_root(dr_account);
        let new_account = create_signer(new_account_address);
        // The dr_account is verified to have the dijets root role in `Roles::new_validator_operator_role`
    ...
    spec schema PreserveKeyRotationCapAbsence {
        ...
        ensures forall addr: address:
            old(!exists<DijetsAccount>(addr) || !spec_has_key_rotation_cap(addr)) ==>
                (!exists<DijetsAccount>(addr) || !spec_has_key_rotation_cap(addr));
    }
    ...
    spec schema PreserveWithdrawCapAbsence {
        ...
        ensures forall addr: address:
            old(!exists<DijetsAccount>(addr) || Option::is_none(global<DijetsAccount>(addr).withdraw_capability)) ==>
                (!exists<DijetsAccount>(addr) || Option::is_none(global<DijetsAccount>(addr).withdraw_capability));
    }
    ...
    spec module {
        ...
        invariant DijetsTimestamp::is_operating() ==> exists<AccountOperationsCapability>(@DijetsRoot);

        /// After genesis, the `DijetsWriteSetManager` exists.
        invariant DijetsTimestamp::is_operating() ==> exists<DijetsWriteSetManager>(@DijetsRoot);
        ...
        invariant update old(exists<AccountOperationsCapability>(@DijetsRoot))
                ==> exists<AccountOperationsCapability>(@DijetsRoot);

        /// resource struct `AccountOperationsCapability` is persistent
        invariant update
            old(exists<DijetsWriteSetManager>(@DijetsRoot)) ==> exists<DijetsWriteSetManager>(@DijetsRoot);
    }
    ...
    spec module {
        ...
        /// There is a published AccountOperationsCapability iff there is an account and it's at Dijets root address
        invariant [suspendable] forall addr: address:
            exists<AccountOperationsCapability>(addr) <==> (addr == @DijetsRoot && exists_at(addr));

        /// An account has a WriteSetManager iff if it is Dijets root
        invariant [suspendable] forall addr: address:
           exists<DijetsWriteSetManager>(addr) <==> (addr == @DijetsRoot && exists_at(addr));

        /// There is a VASPDomainManager at an address iff the address is a dijets treasury compliance account
        invariant [suspendable] forall addr: address:
            exists<VASPDomain::VASPDomainManager>(addr) <==> Roles::spec_has_treasury_compliance_role_addr(addr);

        /// There is a VASPDomains at an address iff the address is a Dijets treasury compliance account
        invariant [suspendable] forall addr: address:
        ...
        /// Account has SlidingNonce only if it's Dijets Root or Treasury Compliance
        invariant [suspendable] forall addr: address: exists<SlidingNonce::SlidingNonce>(addr)
            <==> (Roles::spec_has_dijets_root_role_addr(addr) || Roles::spec_has_treasury_compliance_role_addr(addr));
    ...
    spec module {
        /// Returns field `key_rotation_capability` of the DijetsAccount under `addr`.
        fun spec_get_key_rotation_cap_field(addr: address): Option<KeyRotationCapability> {
            global<DijetsAccount>(addr).key_rotation_capability
        }
        ...
        /// Returns true if the DijetsAccount at `addr` holds
        ...
        /// Returns field `withdraw_capability` of DijetsAccount under `addr`.
        fun spec_get_withdraw_cap_field(addr: address): Option<WithdrawCapability> {
            global<DijetsAccount>(addr).withdraw_capability
        }
        ...
        /// Returns true if the DijetsAccount at `addr` holds a `WithdrawCapability`.
        fun spec_has_withdraw_cap(addr: address): bool {
            Option::is_some(spec_get_withdraw_cap_field(addr))
        }

        /// Returns true if the DijetsAccount at `addr` holds `WithdrawCapability` for itself.
        ...
        fun spec_holds_delegated_withdraw_capability(addr: address): bool {
            exists_at(addr) && Option::is_none(global<DijetsAccount>(addr).withdraw_capability)
        }
    ...
    spec fun prologue_guarantees(sender: signer) : bool {
        ...
        DijetsTimestamp::is_operating() && exists_at(addr) && !AccountFreezing::account_is_frozen(addr)
    }
