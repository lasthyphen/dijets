...
module DijetsFramework::TransactionFee {
    use DijetsFramework::XUS::XUS;
    use DijetsFramework::XDX;
    use DijetsFramework::Dijets::{Self, Dijets, Preburn};
    ...
    public fun add_txn_fee_currency<CoinType>(tc_account: &signer) {
        Roles::assert_treasury_compliance(tc_account);
        Dijets::assert_is_currency<CoinType>();
        assert(
        ...
        move_to(
            ...
            TransactionFee<CoinType> {
                balance: Dijets::zero(),
                preburn: Dijets::create_preburn(tc_account)
            }
        )
    }

    /// Deposit `coin` into the transaction fees bucket
    public fun pay_fee<CoinType>(coin: Dijets<CoinType>) acquires TransactionFee {
        ...
        Dijets::deposit(&mut fees.balance, coin)
    }
    ...
    spec pay_fee {
        ...
        include Dijets::DepositAbortsIf<CoinType>{coin: fees, check: coin};
    ...
    public fun burn_fees<CoinType>(
        ...
        if (XDX::is_xdx<CoinType>()) {
            ...
            // extract fees
            let fees = borrow_global_mut<TransactionFee<CoinType>>(@TreasuryCompliance);
            let coin = Dijets::withdraw_all(&mut fees.balance);
            let burn_cap = Dijets::remove_burn_capability<CoinType>(tc_account);
            // burn
            Dijets::burn_now(
                coin,
                &mut fees.preburn,
            ...
            Dijets::publish_burn_capability(tc_account, burn_cap);
        }
    }
    ...
    spec burn_fees {
        ...
        ensures Dijets::spec_market_cap<CoinType>()
            == old(Dijets::spec_market_cap<CoinType>()) - old(spec_transaction_fee<CoinType>().balance.value);
    ...
    spec schema BurnFeesNotXDX<CoinType> {
        ...
        include Dijets::AbortsIfNoBurnCapability<CoinType>{account: tc_account};

        let fees = spec_transaction_fee<CoinType>();
        include Dijets::BurnNowAbortsIf<CoinType>{coin: fees.balance, preburn: fees.preburn};

        /// tc_account retrieves BurnCapability [[H3]][PERMISSION].
        /// BurnCapability is not transferrable [[J3]][PERMISSION].
        ensures exists<Dijets::BurnCapability<CoinType>>(Signer::spec_address_of(tc_account));
    }
    ...
    spec module {
        ...
        invariant DijetsTimestamp::is_operating() ==> is_initialized();
    }
