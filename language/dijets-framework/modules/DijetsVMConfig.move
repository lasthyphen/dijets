...
module DijetsFramework::DijetsVMConfig {
    ...
    /// The struct to hold config data needed to operate the DijetsVM.
    struct DijetsVMConfig has copy, drop, store {
    ...
    /// 2. The initialization of the module will publish the instruction table to the dijets root account
    ...
    /// Initialize the table under the dijets root account
    public fun initialize(
        ...
        // The permission "UpdateVMConfig" is granted to DijetsRoot [[H11]][PERMISSION].
    ...
    spec initialize {
        ...
        include DijetsConfig::PublishNewConfigAbortsIf<DijetsVMConfig>;
        include DijetsConfig::PublishNewConfigEnsures<DijetsVMConfig> {
            payload: DijetsVMConfig {
                gas_schedule: GasSchedule {
                    instruction_schedule,
                    native_schedule,
                    gas_constants,
    ...
    public fun set_gas_constants(
        ...
        let config = DijetsConfig::get<DijetsVMConfig>();
        ...
        DijetsConfig::set(dr_account, config);
    }
    spec set_gas_constants {
        include DijetsTimestamp::AbortsIfNotOperating;
        /// No one can update DijetsVMConfig except for the Dijets Root account [[H11]][PERMISSION].
        include Roles::AbortsIfNotDijetsRoot{account: dr_account};
        include DijetsConfig::SetAbortsIf<DijetsVMConfig>{account: dr_account };
        ...
        let config = DijetsConfig::spec_get_config<DijetsVMConfig>();
        ensures DijetsConfig::spec_is_published<DijetsVMConfig>();
        ensures DijetsConfig::get<DijetsVMConfig>() == DijetsVMConfig {
            gas_schedule: GasSchedule {
        ...
        ensures old(DijetsConfig::spec_has_config()) == DijetsConfig::spec_has_config();
    }
    ...
    spec module {
        invariant DijetsTimestamp::is_operating() ==> DijetsConfig::spec_is_published<DijetsVMConfig>();
    }

    /// # Access Control

    /// No one can update DijetsVMConfig except for the Dijets Root account [[H11]][PERMISSION].
    spec schema DijetsVMConfigRemainsSame {
        ensures old(DijetsConfig::spec_is_published<DijetsVMConfig>()) ==>
            global<DijetsConfig<DijetsVMConfig>>(@DijetsRoot) ==
                old(global<DijetsConfig<DijetsVMConfig>>(@DijetsRoot));
    }
    spec module {
        apply DijetsVMConfigRemainsSame to * except set_gas_constants;
    }
}
