...
module ValidatorAdministrationScripts {
    ...
    /// transaction can only be successfully called by the Dijets Root account.
    ///
    /// # Technical Description
    /// This script adds the account at `validator_address` to the validator set.
    /// This transaction emits a `DijetsConfig::NewEpochEvent` event and triggers a
    ...
    /// | `Errors::INVALID_ARGUMENT` | `DijetsSystem::EINVALID_PROSPECTIVE_VALIDATOR` | The validator to be added does not have a `ValidatorConfig::ValidatorConfig` resource published under it, or its `config` field is empty. |
    /// | `Errors::INVALID_ARGUMENT` | `DijetsSystem::EALREADY_A_VALIDATOR`           | The `validator_address` account is already a registered validator.                                                                        |
    /// | `Errors::INVALID_STATE`    | `DijetsConfig::EINVALID_BLOCK_TIME`            | An invalid time value was encountered in reconfiguration. Unlikely to occur.                                                              |
    /// | `Errors::LIMIT_EXCEEDED`   | `DijetsSystem::EMAX_VALIDATORS`                | The validator set is already at its maximum size. The validator could not be added.                                                       |
    ///
    /// # Related Scripts
    ...
    public(script) fun add_validator_and_reconfigure(
        ...
        DijetsSystem::add_validator(&dr_account, validator_address);
    }
    ...
    spec add_validator_and_reconfigure {
        ...
        include DijetsSystem::AddValidatorAbortsIf{validator_addr: validator_address};
        include DijetsSystem::AddValidatorEnsures{validator_addr: validator_address};

        /// Reports INVALID_STATE because of is_operating() and !exists<DijetsSystem::CapabilityHolder>.
        /// is_operating() is always true during transactions, and CapabilityHolder is published
        /// during initialization (Genesis).
        /// Reports REQUIRES_ROLE if dr_account is not Dijets root, but that can't happen
        ...
        /// **Access Control:**
        /// Only the Dijets Root account can add Validators [[H14]][PERMISSION].
    ...
    /// config resource held under `validator_account`. It does not emit a `DijetsConfig::NewEpochEvent`
    ...
    spec register_validator_config {
        ...
        include DijetsAccount::TransactionChecks{sender: validator_operator_account}; // properties checked by the prologue.
    ...
    /// successfully called by the Dijets Root account.
    ///
    /// # Technical Description
    /// This script removes the account at `validator_address` from the validator set. This transaction
    /// emits a `DijetsConfig::NewEpochEvent` event. Once the reconfiguration triggered by this event
    ...
    /// | `Errors::INVALID_ARGUMENT` | `SlidingNonce::ENONCE_TOO_NEW`          | The `sliding_nonce` is too far in the future.                                                   |
    /// | `Errors::INVALID_ARGUMENT` | `SlidingNonce::ENONCE_ALREADY_RECORDED` | The `sliding_nonce` has been previously recorded.                                               |
    /// | `Errors::NOT_PUBLISHED`    | `SlidingNonce::ESLIDING_NONCE`          | The sending account is not the Dijets Root account or Treasury Compliance account                |
    /// | 0                          | 0                                       | The provided `validator_name` does not match the already-recorded human name for the validator. |
    /// | `Errors::INVALID_ARGUMENT` | `DijetsSystem::ENOT_AN_ACTIVE_VALIDATOR` | The validator to be removed is not in the validator set.                                        |
    /// | `Errors::REQUIRES_ADDRESS` | `CoreAddresses::EDIJETS_ROOT`            | The sending account is not the Dijets Root account.                                              |
    /// | `Errors::REQUIRES_ROLE`    | `Roles::EDIJETS_ROOT`                    | The sending account is not the Dijets Root account.                                              |
    /// | `Errors::INVALID_STATE`    | `DijetsConfig::EINVALID_BLOCK_TIME`      | An invalid time value was encountered in reconfiguration. Unlikely to occur.                    |
    ///
    ...
    public(script) fun remove_validator_and_reconfigure(
        ...
        assert(ValidatorConfig::get_human_name(validator_address) == validator_name, 0);
        DijetsSystem::remove_validator(&dr_account, validator_address);
    }
    ...
    spec remove_validator_and_reconfigure {
        ...
        // TODO: use an error code from Errors.move instead of 0.
        aborts_if ValidatorConfig::get_human_name(validator_address) != validator_name with 0;
        include DijetsSystem::RemoveValidatorAbortsIf{validator_addr: validator_address};
        include DijetsSystem::RemoveValidatorEnsures{validator_addr: validator_address};
        ...
        /// **Access Control:**
        /// Only the Dijets Root account can remove Validators [[H14]][PERMISSION].
        include Roles::AbortsIfNotDijetsRoot{account: dr_account};
    }
    ...
    /// # Technical Description
    /// This updates the fields with corresponding names held in the `ValidatorConfig::ValidatorConfig`
    /// config resource held under `validator_account`. It then emits a `DijetsConfig::NewEpochEvent` to
    ...
    public(script) fun set_validator_config_and_reconfigure(
        ...
        DijetsSystem::update_config_and_reconfigure(&validator_operator_account, validator_account);
     }
    ...
    spec set_validator_config_and_reconfigure {
       ...
       include DijetsAccount::TransactionChecks{sender: validator_operator_account};
        ...
        // These properties are important for access control. See notes in DijetsSystem.
        include DijetsSystem::UpdateConfigAndReconfigureEnsures{validator_addr: validator_account};
        ensures ValidatorConfig::is_valid(validator_account);
        ...
        include ValidatorConfig::SetConfigAbortsIf{validator_addr: validator_account};
        include DijetsSystem::UpdateConfigAndReconfigureAbortsIf{validator_addr: validator_account};
        ...
        let is_validator_info_updated =
            (exists v_info in DijetsSystem::spec_get_validators():
        ...
        let validator_index = DijetsSystem::spec_index_of_validator(DijetsSystem::spec_get_validators(), validator_account);
        let last_config_time = DijetsSystem::spec_get_validators()[validator_index].last_config_update_time;
        aborts_if is_validator_info_updated && last_config_time > DijetsSystem::MAX_U64 - DijetsSystem::FIVE_MINUTES
            with Errors::LIMIT_EXCEEDED;
        aborts_if is_validator_info_updated && DijetsTimestamp::spec_now_microseconds() <= last_config_time + DijetsSystem::FIVE_MINUTES
            with Errors::LIMIT_EXCEEDED;

        /// This reports a possible INVALID_STATE abort, which comes from an assert in DijetsConfig::reconfigure_
    ...
    /// `DijetsConfig::NewEpochEvent` and no reconfiguration of the system is initiated by this script.
    ///
    /// # Parameters
    /// | Name               | Type         | Description                                                                                  |
    /// | ------             | ------       | -------------                                                                                |
    ...
    spec set_validator_operator {
        ...
        /// Reports INVALID_STATE because of !exists<DijetsSystem::CapabilityHolder>, but that can't happen
    ...
    /// Sets the validator operator for a validator in the validator's configuration resource "locally"
    /// and does not reconfigure the system. Changes from this transaction will not picked up by the
    /// system until a reconfiguration of the system is triggered. May only be sent by the Dijets Root
    ...
    /// | Name               | Type         | Description                                                                                   |
    /// | ------             | ------       | -------------                                                                                 |
    /// | `dr_account`       | `signer`     | Signer of the sending account of the write set transaction. May only be the Dijets Root signer. |
