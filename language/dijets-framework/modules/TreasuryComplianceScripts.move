...
/// activities in the Dijets Framework.
///
...
module DijetsFramework::TreasuryComplianceScripts {
    ...
    /// Cancels and returns all coins held in the `Dijets::Preburn<Token>` resource under the `preburn_address` and
    /// return the funds to the `preburn_address` account's `DijetsAccount::Balance<Token>`.
    /// The transaction must be sent by an `account` with a `Dijets::BurnCapability<Token>`
    /// resource published under it. The account at `preburn_address` must have a
    /// `Dijets::Preburn<Token>` resource published under it, and its value must be nonzero. The transaction removes
    /// the entire balance held in the `Dijets::Preburn<Token>` resource, and returns it back to the account's
    /// `DijetsAccount::Balance<Token>` under `preburn_address`. Due to this, the account at
    ...
    /// * A `Dijets::CancelBurnEvent` on the event handle held in the `Dijets::CurrencyInfo<Token>`
    /// resource's `burn_events` published under `0xA550C18`.
    /// * A `DijetsAccount::ReceivedPaymentEvent` on the `preburn_address`'s
    /// `DijetsAccount::DijetsAccount` `received_events` event handle with both the `payer` and `payee`
    ...
    /// | ----------------              | --------------                                   | -------------                                                                                                                       |
    /// | `Errors::REQUIRES_CAPABILITY` | `Dijets::EBURN_CAPABILITY`                         | The sending `account` does not have a `Dijets::BurnCapability<Token>` published under it.                                             |
    /// | `Errors::INVALID_STATE`       | `Dijets::EPREBURN_NOT_FOUND`                       | The `Dijets::PreburnQueue<Token>` resource under `preburn_address` does not contain a preburn request with a value matching `amount`. |
    /// | `Errors::NOT_PUBLISHED`       | `Dijets::EPREBURN_QUEUE`                           | The account at `preburn_address` does not have a `Dijets::PreburnQueue<Token>` resource published under it.                           |
    /// | `Errors::NOT_PUBLISHED`       | `Dijets::ECURRENCY_INFO`                           | The specified `Token` is not a registered currency on-chain.                                                                        |
    /// | `Errors::INVALID_ARGUMENT`    | `DijetsAccount::EPAYEE_CANT_ACCEPT_CURRENCY_TYPE`  | The account at `preburn_address` doesn't have a balance resource for `Token`.                                                       |
    /// | `Errors::LIMIT_EXCEEDED`      | `DijetsAccount::EDEPOSIT_EXCEEDS_LIMITS`           | The depositing of the funds held in the prebun area would exceed the `account`'s account limits.                                    |
    ...
    public(script) fun cancel_burn_with_amount<Token>(account: signer, preburn_address: address, amount: u64) {
        DijetsAccount::cancel_burn<Token>(&account, preburn_address, amount)
    }
    ...
    spec cancel_burn_with_amount {
        ...
        include DijetsAccount::TransactionChecks{sender: account}; // properties checked by the prologue.
        include DijetsAccount::CancelBurnAbortsIf<Token>;
        include Dijets::CancelBurnWithCapEnsures<Token>;
        include DijetsAccount::DepositEnsures<Token>{payee: preburn_address};

        let total_preburn_value = global<Dijets::CurrencyInfo<Token>>(
            @CurrencyInfo
        ).preburn_value;
        let post post_total_preburn_value = global<Dijets::CurrencyInfo<Token>>(
            @CurrencyInfo
        ).preburn_value;

        let balance_at_addr = DijetsAccount::balance<Token>(preburn_address);
        let post post_balance_at_addr = DijetsAccount::balance<Token>(preburn_address);
        ...
        include DijetsAccount::DepositEmits<Token>{
        ...
        include Dijets::AbortsIfNoBurnCapability<Token>{account: account};
    }
    ...
    /// stored in the `Dijets::Preburn<Token>` resource published under the
    /// `preburn_address` account address.
    ///
    /// This transaction will only succeed if the sending `account` has a
    /// `Dijets::BurnCapability<Token>`, and a `Dijets::Preburn<Token>` resource
    ...
    /// `Dijets::CurrencyInfo<Token>` resource published under `0xA550C18` will be
    ...
    ///
    /// # Events
    /// The successful execution of this transaction will emit a `Dijets::BurnEvent` on the event handle
    /// held in the `Dijets::CurrencyInfo<Token>` resource's `burn_events` published under
    /// `0xA550C18`.
    ///
    /// # Parameters
    /// | Name              | Type      | Description                                                                                                        |
    ...
    public(script) fun burn_with_amount<Token>(account: signer, sliding_nonce: u64, preburn_address: address, amount: u64) {
        ...
        Dijets::burn<Token>(&account, preburn_address, amount)
    }
    spec burn_with_amount {
        ...
        include Dijets::BurnAbortsIf<Token>;
        include Dijets::BurnEnsures<Token>;
        ...
        include Dijets::BurnWithResourceCapEmits<Token>{preburn: Dijets::spec_make_preburn(amount)};
    ...
    /// `DijetsAccount::Balance<Token>` to the `Dijets::Preburn<Token>` published under the same
    ...
    /// * `DijetsAccount::SentPaymentEvent ` on `account`'s `DijetsAccount::DijetsAccount` `sent_events`
    /// handle with the `payee` and `payer` fields being `account`'s address; and
    /// * A `Dijets::PreburnEvent` with `Token`'s currency code on the
    /// `Dijets::CurrencyInfo<Token`'s `preburn_events` handle for `Token` and with
    ...
    /// | Error Category           | Error Reason                                             | Description                                                                             |
    /// | ----------------         | --------------                                           | -------------                                                                           |
    /// | `Errors::NOT_PUBLISHED`  | `Dijets::ECURRENCY_INFO`                                  | The `Token` is not a registered currency on-chain.                                      |
    /// | `Errors::INVALID_STATE`  | `DijetsAccount::EWITHDRAWAL_CAPABILITY_ALREADY_EXTRACTED` | The withdrawal capability for `account` has already been extracted.                     |
    /// | `Errors::LIMIT_EXCEEDED` | `DijetsAccount::EINSUFFICIENT_BALANCE`                    | `amount` is greater than `payer`'s balance in `Token`.                                  |
    /// | `Errors::NOT_PUBLISHED`  | `DijetsAccount::EPAYER_DOESNT_HOLD_CURRENCY`              | `account` doesn't hold a balance in `Token`.                                            |
    /// | `Errors::NOT_PUBLISHED`  | `Dijets::EPREBURN`                                        | `account` doesn't have a `Dijets::Preburn<Token>` resource published under it.           |
    /// | `Errors::INVALID_STATE`  | `Dijets::EPREBURN_OCCUPIED`                               | The `value` field in the `Dijets::Preburn<Token>` resource under the sender is non-zero. |
    ...
    public(script) fun preburn<Token>(account: signer, amount: u64) {
        let withdraw_cap = DijetsAccount::extract_withdraw_capability(&account);
        DijetsAccount::preburn<Token>(&account, &withdraw_cap, amount);
        DijetsAccount::restore_withdraw_capability(withdraw_cap);
    }
    ...
    spec preburn {
        ...
        let cap = DijetsAccount::spec_get_withdraw_cap(account_addr);
        include DijetsAccount::ExtractWithdrawCapAbortsIf{sender_addr: account_addr};
        include DijetsAccount::PreburnAbortsIf<Token>{dd: account, cap: cap};
        include DijetsAccount::PreburnEnsures<Token>{dd: account, payer: account_addr};

        include DijetsAccount::PreburnEmits<Token>{dd: account, cap: cap};
        ...
        aborts_if !(exists<Dijets::Preburn<Token>>(account_addr) || exists<Dijets::PreburnQueue<Token>>(account_addr));
    }

    /// # Summary
    /// Burns the transaction fees collected in the `CoinType` currency so that the
    /// Dijets association may reclaim the backing coins off-chain. May only be sent
    ...
    /// # Events
    /// The successful execution of this transaction will emit a `Dijets::BurnEvent` on the event handle
    /// held in the `Dijets::CurrencyInfo<CoinType>` resource's `burn_events` published under
    /// `0xA550C18`.
    ///
    /// # Parameters
    /// | Name         | Type     | Description                                                                                                                                         |
    ...
    /// | `Errors::INVALID_ARGUMENT` | `Dijets::ECOIN`                        | The collected fees in `CoinType` are zero.                  |
    ///
    /// # Related Scripts
    /// * `TreasuryComplianceScripts::burn_with_amount`
    /// * `TreasuryComplianceScripts::cancel_burn_with_amount`
    ...
    /// * A `Dijets::MintEvent` with the amount and currency code minted is emitted on the
    /// `mint_event_handle` in the stored `Dijets::CurrencyInfo<CoinType>` resource stored under
    /// `0xA550C18`; and
    ...
    /// | `Errors::REQUIRES_CAPABILITY` | `Dijets::EMINT_CAPABILITY`                    | `tc_account` does not have a `Dijets::MintCapability<CoinType>` resource published under it.                                  |
    /// | `Errors::INVALID_STATE`       | `Dijets::EMINTING_NOT_ALLOWED`                | Minting is not currently allowed for `CoinType` coins.                                                                       |
    /// | `Errors::LIMIT_EXCEEDED`      | `DijetsAccount::EDEPOSIT_EXCEEDS_LIMITS`      | The depositing of the funds would exceed the `account`'s account limits.                                                     |
    ///
    /// # Related Scripts
    ...
    public(script) fun tiered_mint<CoinType>(
        ...
        DijetsAccount::tiered_mint<CoinType>(
    ...
    spec tiered_mint {
        ...
        include DijetsAccount::TransactionChecks{sender: tc_account}; // properties checked by the prologue.
        include SlidingNonce::RecordNonceAbortsIf{account: tc_account, seq_nonce: sliding_nonce};
        include DijetsAccount::TieredMintAbortsIf<CoinType>;
        include DijetsAccount::TieredMintEnsures<CoinType>;
        ...
        include DijetsAccount::TieredMintEmits<CoinType>;
    ...
    /// the Dijets Root or Treasury Compliance account. After the successful
    ...
    /// not be either `0xA550C18` (the Dijets Root address), or `0xB1E55ED` (the
    ...
    /// | `Errors::INVALID_ARGUMENT` | `AccountFreezing::ECANNOT_FREEZE_DIJETS_ROOT` | `to_freeze_account` was the Dijets Root account (`0xA550C18`).                              |
    ///
    /// # Related Scripts
    ...
    public(script) fun update_dual_attestation_limit(
        ...
        SlidingNonce::record_nonce_or_abort(&tc_account, sliding_nonce);
        DualAttestation::set_microdijets_limit(&tc_account, new_micro_xdx_limit);
    }
    ...
    public(script) fun update_exchange_rate<Currency>(
        ...
        Dijets::update_xdx_exchange_rate<Currency>(&tc_account, rate);
    }
    spec update_exchange_rate {
        ...
        include Dijets::UpdateXDXExchangeRateAbortsIf<Currency>;
        include Dijets::UpdateXDXExchangeRateEnsures<Currency>{xdx_exchange_rate: rate};
        include Dijets::UpdateXDXExchangeRateEmits<Currency>{xdx_exchange_rate: rate};
    ...
    ///
    /// # Technical Description
    /// This transaction sets the `can_mint` field of the `Dijets::CurrencyInfo<Currency>` resource
    ...
    /// | `account`       | `signer` | Signer of the sending account. Must be the Dijets Root account.                                                                        |
    ...
    /// | ----------------           | --------------                        | -------------                                        |
    /// | `Errors::REQUIRES_ADDRESS` | `CoreAddresses::ETREASURY_COMPLIANCE` | `tc_account` is not the Treasury Compliance account. |
    /// | `Errors::NOT_PUBLISHED`    | `Dijets::ECURRENCY_INFO`               | `Currency` is not a registered currency on-chain.    |
    ///
    /// # Related Scripts
    /// * `TreasuryComplianceScripts::update_dual_attestation_limit`
    /// * `TreasuryComplianceScripts::update_exchange_rate`
    ...
    public(script) fun update_minting_ability<Currency>(
        ...
        Dijets::update_minting_ability<Currency>(&tc_account, allow_minting);
    }
