...
/// a given time period.
module DijetsFramework::AccountLimits {
    ...
    /// "unlimited" `LimitsDefinition` resource for accounts published at`@DijetsRoot`, but other
    ...
    spec publish_unrestricted_limits {
        ...
        /// in DijetsConfig, this function violates an invariant in
    ...
    spec module {
        fun spec_window_expired<CoinType>(
            ...
            DijetsTimestamp::spec_now_microseconds() > window.window_start + limits_definition.time_period
        }
        fun spec_window_reset_with_limits<CoinType>(
            ...
            if (spec_window_expired<CoinType>(window, limits_definition)) {
                Window<CoinType>{
                    ...
                    window_start: DijetsTimestamp::spec_now_microseconds(),
                    window_inflow: 0,
                    window_outflow: 0
                }
            } else {
    ...
    fun current_time(): u64 {
        if (DijetsTimestamp::is_genesis()) 0 else DijetsTimestamp::now_microseconds()
    }
