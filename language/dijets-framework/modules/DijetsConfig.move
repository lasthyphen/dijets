...
module DijetsFramework::DijetsConfig {
    ...
    struct DijetsConfig<Config: copy + drop + store> has key, store {
        /// Holds specific info for instance of `Config` type.
        payload: Config
    }

    /// Event that signals DijetsBFT algorithm to start a new epoch,
    ...
    /// Accounts with this privilege can modify DijetsConfig<TypeName> under Dijets root address.
    ...
    /// A `DijetsConfig` resource is in an invalid state
    const EDIJETS_CONFIG: u64 = 1;
    ...
    /// Publishes `Configuration` resource. Can only be invoked by Dijets root, and only a single time in Genesis.
    public fun initialize(
        dr_account: &signer,
    ) {
        DijetsTimestamp::assert_genesis();
        CoreAddresses::assert_dijets_root(dr_account);
        assert(!exists<Configuration>(@DijetsRoot), Errors::already_published(ECONFIGURATION));
    ...
    spec initialize {
        ...
        modifies global<Configuration>(@DijetsRoot);
    ...
    spec schema InitializeEnsures {
        ...
        let post post_config = global<Configuration>(@DijetsRoot);
    ...
    acquires DijetsConfig {
        let addr = @DijetsRoot;
        assert(exists<DijetsConfig<Config>>(addr), Errors::not_published(EDIJETS_CONFIG));
        *&borrow_global<DijetsConfig<Config>>(addr).payload
    }
    spec get {
        pragma opaque;
    ...
    spec schema AbortsIfNotPublished<Config> {
        aborts_if !exists<DijetsConfig<Config>>(@DijetsRoot) with Errors::NOT_PUBLISHED;
    }
    ...
    acquires DijetsConfig, Configuration {
        ...
        let addr = @DijetsRoot;
        assert(exists<DijetsConfig<Config>>(addr), Errors::not_published(EDIJETS_CONFIG));
        let config = borrow_global_mut<DijetsConfig<Config>>(addr);
    ...
    spec set {
        pragma opaque;
        modifies global<Configuration>(@DijetsRoot);
        modifies global<DijetsConfig<Config>>(@DijetsRoot);
    ...
    /// It is called by `DijetsSystem::update_config_and_reconfigure`, which allows
    /// validator operators to change the validator set.  All other config changes require
    /// a Dijets root signer.
    ...
    fun disable_reconfiguration(dr_account: &signer) {
        assert(
            ...
            Errors::requires_address(EDIJETS_CONFIG)
        );
    ...
    fun reconfiguration_enabled(): bool {
        !exists<DisableReconfiguration>(@DijetsRoot)
    }
    ...
    public fun publish_new_config_and_get_capability<Config: copy + drop + store>(
        ...
        Roles::assert_dijets_root(dr_account);
        assert(
            !exists<DijetsConfig<Config>>(Signer::address_of(dr_account)),
            Errors::already_published(EDIJETS_CONFIG)
        );
        move_to(dr_account, DijetsConfig { payload });
        ModifyConfigCapability<Config> {}
    }
    ...
    spec schema AbortsIfPublished<Config> {
        aborts_if exists<DijetsConfig<Config>>(@DijetsRoot) with Errors::ALREADY_PUBLISHED;
    }

    /// Publish a new config item. Only Dijets root can modify such config.
    /// Publishes the capability to modify this config under the Dijets root account.
    /// Does not trigger a reconfiguration.
    ...
    spec publish_new_config {
        pragma opaque;
        modifies global<DijetsConfig<Config>>(@DijetsRoot);
        modifies global<ModifyConfigCapability<Config>>(@DijetsRoot);
    ...
    /// Signal validators to start using new configuration. Must be called by Dijets root.
    ...
    fun reconfigure_() acquires Configuration {
        ...
        if (DijetsTimestamp::is_genesis() || DijetsTimestamp::now_microseconds() == 0 || !reconfiguration_enabled()) {
            return ()
        };

        let config_ref = borrow_global_mut<Configuration>(@DijetsRoot);
        let current_time = DijetsTimestamp::now_microseconds();
        ...
        //   DijetsVM spec that all transactions comming after a reconfiguration transaction will be returned as Retry
        //   status.
    ...
    spec fun spec_reconfigure_omitted(): bool {
       DijetsTimestamp::is_genesis() || DijetsTimestamp::spec_now_microseconds() == 0 || !reconfiguration_enabled()
    }
    spec reconfigure_ {
        pragma opaque;
        modifies global<Configuration>(@DijetsRoot);
        ensures old(spec_has_config()) == spec_has_config();
        let config = global<Configuration>(@DijetsRoot);
        let post post_config = global<Configuration>(@DijetsRoot);
        let now = DijetsTimestamp::spec_now_microseconds();
        let post post_now = DijetsTimestamp::spec_now_microseconds();
    ...
    spec schema InternalReconfigureAbortsIf {
        let config = global<Configuration>(@DijetsRoot);
        let current_time = DijetsTimestamp::spec_now_microseconds();
    ...
    spec schema ReconfigureAbortsIf {
        let config = global<Configuration>(@DijetsRoot);
        let current_time = DijetsTimestamp::spec_now_microseconds();
        aborts_if DijetsTimestamp::is_operating()
            && reconfiguration_enabled()
            && DijetsTimestamp::spec_now_microseconds() > 0
    ...
    spec schema ReconfigureEmits {
        let config = global<Configuration>(@DijetsRoot);
        let post post_config = global<Configuration>(@DijetsRoot);
        let post now = DijetsTimestamp::spec_now_microseconds();
    ...
    fun emit_genesis_reconfiguration_event() acquires Configuration {
        assert(exists<Configuration>(@DijetsRoot), Errors::not_published(ECONFIGURATION));
        let config_ref = borrow_global_mut<Configuration>(@DijetsRoot);
    ...
    spec emit_genesis_reconfiguration_event {
        let post config = global<Configuration>(@DijetsRoot);
    ...
    spec module {
        ...
        invariant DijetsTimestamp::is_operating() ==> spec_has_config();
    }

    /// # Invariants
    spec module {
        /// Configurations are only stored at the dijets root address.
        invariant<ConfigType>
            forall config_address: address where exists<DijetsConfig<ConfigType>>(config_address):
                config_address == @DijetsRoot;
        ...
        invariant<ConfigType> update
            old(exists<ModifyConfigCapability<ConfigType>>(@DijetsRoot)) ==>
                exists<ModifyConfigCapability<ConfigType>>(@DijetsRoot);
    }

    /// # Helper Functions
    spec module {
        fun spec_has_config(): bool {
            exists<Configuration>(@DijetsRoot)
        }

        fun spec_is_published<Config>(): bool {
            exists<DijetsConfig<Config>>(@DijetsRoot)
        }

        fun spec_get_config<Config>(): Config {
            global<DijetsConfig<Config>>(@DijetsRoot).payload
        }
    }
