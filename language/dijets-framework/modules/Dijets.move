/// The `Dijets` module describes the concept of a coin in the Dijets framework. It introduces the
/// resource `Dijets::Dijets<CoinType>`, representing a coin of given coin type.
/// The module defines functions operating on coins as well as functionality like
/// minting and burning of coins.
module DijetsFramework::Dijets {
    ...
    /// The `Dijets` resource defines the Dijets coin for each currency in
    /// Dijets. Each "coin" is coupled with a type `CoinType` specifying the
    ...
    struct Dijets<phantom CoinType> has store {
    ...
    /// account or the `DijetsFramework::XDX` module (and `@DijetsRoot` in testnet).
    ...
    /// A `MintEvent` is emitted every time a Dijets coin is minted. This
    ...
    /// A `BurnEvent` is emitted every time a non-synthetic Dijets coin
    /// (i.e., a Dijets coin with false `is_synthetic` field) is
    ...
    struct ToXDXExchangeRateUpdateEvent has drop, store {
        ...
        /// between the (on-chain) base-units for the currency and microdijets.
    ...
    /// Initialization of the `Dijets` module. Initializes the set of
    /// registered currencies in the `DijetsFramework::RegisteredCurrencies` on-chain
    /// config, and publishes the `CurrencyRegistrationCapability` under the
    /// `@DijetsRoot`. This can only be called from genesis.
    ...
    /// to be successful.
    public fun mint<CoinType>(account: &signer, value: u64): Dijets<CoinType>
    ...
    public fun cancel_burn<CoinType>(
        ...
        preburn_address: address,
        amount: u64,
    ): Dijets<CoinType> acquires BurnCapability, CurrencyInfo, PreburnQueue {
        assert_is_currency<CoinType>();
        let addr = Signer::address_of(account);
        assert(exists<BurnCapability<CoinType>>(addr), Errors::requires_capability(EBURN_CAPABILITY));
    ...
    /// Mint a new `Dijets` coin of `CoinType` currency worth `value`. The
    /// caller must have a reference to a `MintCapability<CoinType>`. Only
    /// the treasury compliance account or the `DijetsFramework::XDX` module can acquire such a
    /// reference.
    public fun mint_with_capability<CoinType>(
        value: u64,
        _capability: &MintCapability<CoinType>
    ): Dijets<CoinType> acquires CurrencyInfo {
        assert_is_currency<CoinType>();
    ...
    /// > See https://github.com/dijets/dijets/issues/7615 for more information.
    spec schema RemovePreburnFromQueueEnsures<CoinType> {
        ...
        // let preburn_queue = global<PreburnQueue<CoinType>>(preburn_address).preburns;
        // let preburn = Preburn { to_burn: Dijets { value: amount }};
    ...
    fun burn_with_resource_cap<CoinType>(
        ...
        let Dijets { value } = withdraw_all<CoinType>(&mut preburn.to_burn);
    ...
    public fun cancel_burn_with_capability<CoinType>(
        ...
        _capability: &BurnCapability<CoinType>,
        amount: u64,
    ): Dijets<CoinType> acquires CurrencyInfo, PreburnQueue {
    ...
    /// Returns the total value of `Dijets<CoinType>` that is waiting to be
    ...
    /// with value equal to `amount`. Calls will fail if `amount > Dijets::value(&coin)`.
    public fun split<CoinType>(coin: Dijets<CoinType>, amount: u64): (Dijets<CoinType>, Dijets<CoinType>) {
    ...
    public fun withdraw<CoinType>(coin: &mut Dijets<CoinType>, amount: u64): Dijets<CoinType> {
    ...
    /// Return a `Dijets<CoinType>` worth `coin.value` and reduces the `value` of the input `coin` to
    /// zero. Does not abort.
    public fun withdraw_all<CoinType>(coin: &mut Dijets<CoinType>): Dijets<CoinType> {
        let val = coin.value;
        withdraw(coin, val)
    }
    spec withdraw_all {
    ...
    public fun join<CoinType>(coin1: Dijets<CoinType>, coin2: Dijets<CoinType>): Dijets<CoinType>  {
    ...
    public fun deposit<CoinType>(coin: &mut Dijets<CoinType>, check: Dijets<CoinType>) {
        let Dijets { value } = check;
    ...
    /// so it is impossible to "burn" any non-zero amount of `Dijets` without having
    /// a `BurnCapability` for the specific `CoinType`.
    public fun destroy_zero<CoinType>(coin: Dijets<CoinType>) {
        let Dijets { value } = coin;
    ...
    /// registered as a currency it cannot be used as a coin/currency unit in Dijets.
    /// The passed-in `dr_account` must be a specific address (`@CurrencyInfo`) and
    /// `dr_account` must also have the correct `DijetsRoot` account role.
    ...
    spec schema RegisterCurrencyAbortsIf<CoinType> {
        ...
        /// Must abort if the signer does not have the DijetsRoot role [[H8]][PERMISSION].
    ...
    /// accounts: The `CoinInfo` is published on the Dijets root account, and the mint and
    ...
    /// rate is needed.
    public fun approx_xdx_for_coin<FromCoinType>(coin: &Dijets<FromCoinType>): u64
    acquires CurrencyInfo {
    ...
    spec module {
        ...
        fun spec_approx_xdx_for_value<CoinType>(value: num):  num {
        ...
        fun spec_make_preburn<CoinType>(amount: u64): Preburn<CoinType> {
            Preburn { to_burn: Dijets { value: amount }}
        }
    }
