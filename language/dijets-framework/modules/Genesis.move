/// The `Genesis` module defines the Move initialization entry point of the Dijets framework
...
module DijetsFramework::Genesis {
    ...
    /// Initializes the Dijets framework.
    fun initialize(
        dr_account: signer,
        tc_account: signer,
        ...
        initialize_internal(
            ...
            native_schedule,
            chain_id,
            initial_dijets_version,
        )
    }

    /// Initializes the Dijets Framework. Internal so it can be used by both genesis code, and for testing purposes
    fun initialize_internal(
        ...
        // Rotate auth keys for DijetsRoot and TreasuryCompliance accounts to the given
        // values
        ...
        // `DijetsTimestamp::is_operating() ==> ...` will become active and a verification condition.
        // See also discussion at function specification.
        DijetsTimestamp::set_time_has_started(dr_account);
    }

    /// Sets up the initial validator set for the Dijets network.
    ...
    /// (see dijets/types/src/network_address/mod.rs) for itself and its full nodes.
    fun create_initialize_owners_operators(
        ...
        while (i < num_owners) {
            ...
            let rotation_cap = DijetsAccount::extract_key_rotation_capability(owner);
            DijetsAccount::rotate_authentication_key(&rotation_cap, owner_auth_key);
            DijetsAccount::restore_key_rotation_capability(rotation_cap);
            ...
            if (!DijetsAccount::exists_at(operator_address)) {
                ...
                let rotation_cap = DijetsAccount::extract_key_rotation_capability(operator);
                DijetsAccount::rotate_authentication_key(&rotation_cap, operator_auth_key);
                DijetsAccount::restore_key_rotation_capability(rotation_cap);
            };
            ...
            DijetsSystem::add_validator(&dr_account, owner_address);
    ...
    spec initialize {
        ...
        requires DijetsTimestamp::is_genesis();
    }
