/// This module contains Dijets Framework script functions to administer the
/// network outside of validators and validator operators.
module DijetsFramework::SystemAdministrationScripts {
    use DijetsFramework::DijetsConsensusConfig;
    use DijetsFramework::DijetsVersion;
    use DijetsFramework::DijetsVMConfig;
    use DijetsFramework::SlidingNonce;

    ///  # Summary
    /// Updates the Dijets major version that is stored on-chain and is used by the VM.  This
    /// transaction can only be sent from the Dijets Root account.
    ///
    /// # Technical Description
    /// Updates the `DijetsVersion` on-chain config and emits a `DijetsConfig::NewEpochEvent` to trigger
    ...
    /// | `Errors::REQUIRES_ADDRESS` | `CoreAddresses::EDIJETS_ROOT`                   | `account` is not the Dijets Root account.                                                    |
    /// | `Errors::INVALID_ARGUMENT` | `DijetsVersion::EINVALID_MAJOR_VERSION_NUMBER`  | `major` is less-than or equal to the current major version stored on-chain.                |

    public(script) fun update_dijets_version(account: signer, sliding_nonce: u64, major: u64) {
        SlidingNonce::record_nonce_or_abort(&account, sliding_nonce);
        DijetsVersion::set(&account, major)
    }

    /// # Summary
    /// Updates the gas constants stored on chain and used by the VM for gas
    /// metering. This transaction can only be sent from the Dijets Root account.
    ///
    /// # Technical Description
    /// Updates the on-chain config holding the `DijetsVMConfig` and emits a
    /// `DijetsConfig::NewEpochEvent` to trigger a reconfiguration of the system.
    ///
    /// # Parameters
    /// | Name                                | Type     | Description                                                                                            |
    /// | ------                              | ------   | -------------                                                                                          |
    ...
    /// | Error Category             | Error Reason                                | Description                                                                                |
    /// | ----------------           | --------------                              | -------------                                                                              |
    /// | `Errors::INVALID_ARGUMENT` | `DijetsVMConfig::EGAS_CONSTANT_INCONSISTENCY` | The provided gas constants are inconsistent.                                               |
    /// | `Errors::NOT_PUBLISHED`    | `SlidingNonce::ESLIDING_NONCE`              | A `SlidingNonce` resource is not published under `account`.                                |
    /// | `Errors::INVALID_ARGUMENT` | `SlidingNonce::ENONCE_TOO_OLD`              | The `sliding_nonce` is too old and it's impossible to determine if it's duplicated or not. |
    /// | `Errors::INVALID_ARGUMENT` | `SlidingNonce::ENONCE_TOO_NEW`              | The `sliding_nonce` is too far in the future.                                              |
    /// | `Errors::INVALID_ARGUMENT` | `SlidingNonce::ENONCE_ALREADY_RECORDED`     | The `sliding_nonce` has been previously recorded.                                          |
    ...
    public(script) fun set_gas_constants(
        ...
        DijetsVMConfig::set_gas_constants(
                &dr_account,
    ...
    ///  # Summary
    /// Initializes the Dijets consensus config that is stored on-chain.  This
    /// transaction can only be sent from the Dijets Root account.
    ///
    /// # Technical Description
    /// Initializes the `DijetsConsensusConfig` on-chain config to empty and allows future updates from DijetsRoot via
    /// `update_dijets_consensus_config`. This doesn't emit a `DijetsConfig::NewEpochEvent`.
    ///
    /// # Parameters
    /// | Name            | Type      | Description                                                                |
    /// | ------          | ------    | -------------                                                              |
    ...
    public(script) fun initialize_dijets_consensus_config(account: signer, sliding_nonce: u64) {
        SlidingNonce::record_nonce_or_abort(&account, sliding_nonce);
        DijetsConsensusConfig::initialize(&account);
    }

    ///  # Summary
    /// Updates the Dijets consensus config that is stored on-chain and is used by the Consensus.  This
    /// transaction can only be sent from the Dijets Root account.
    ///
    /// # Technical Description
    /// Updates the `DijetsConsensusConfig` on-chain config and emits a `DijetsConfig::NewEpochEvent` to trigger
    ...
    public(script) fun update_dijets_consensus_config(account: signer, sliding_nonce: u64, config: vector<u8>) {
        SlidingNonce::record_nonce_or_abort(&account, sliding_nonce);
        DijetsConsensusConfig::set(&account, config)
    }
}
