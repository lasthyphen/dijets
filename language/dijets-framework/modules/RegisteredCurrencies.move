/// Module for registering currencies in Dijets. Basically, this means adding a
/// string (vector<u8>) for the currency name to vector of names in DijetsConfig.
module DijetsFramework::RegisteredCurrencies {
    ...
    /// A DijetsConfig config holding all of the currency codes for registered
    ...
    spec schema InitializeAbortsIf {
        ...
        include DijetsConfig::PublishNewConfigAbortsIf<RegisteredCurrencies>;
    }
    spec schema InitializeEnsures {
        include DijetsConfig::PublishNewConfigEnsures<RegisteredCurrencies>{
    ...
    public fun add_currency_code(
        ...
        let config = DijetsConfig::get<RegisteredCurrencies>();
        assert(
    ...
    spec schema AddCurrencyCodeAbortsIf {
        dr_account: &signer;
        currency_code: vector<u8>;
        include DijetsConfig::SetAbortsIf<RegisteredCurrencies>{ account: dr_account };
        /// The same currency code can be only added once.
        aborts_if contains(
            DijetsConfig::get<RegisteredCurrencies>().currency_codes,
            currency_code
    ...
    spec schema AddCurrencyCodeEnsures {
        ...
        include DijetsConfig::SetEnsures<RegisteredCurrencies> {payload: DijetsConfig::get<RegisteredCurrencies>()};
    }
    ...
    spec module {
        ...
        invariant DijetsTimestamp::is_operating() ==> DijetsConfig::spec_is_published<RegisteredCurrencies>();
    }
