...
module DijetsFramework::DesignatedDealer {
    ...
    // PreburnQueue published at top level in Dijets.move
    ...
    const EDEALER: u64 = 0;
    // Error codes 1 -- 3 were deprecated along with tiered minting in Dijets Framework version 2
    ...
    spec publish_designated_dealer_credential {
        ...
        modifies global<Dijets::PreburnQueue<CoinType>>(dd_addr);
        modifies global<Dijets::PreburnQueue<XUS>>(dd_addr);
    }
    ...
    public fun add_currency<CoinType>(dd: &signer, tc_account: &signer) {
        ...
        Dijets::publish_preburn_queue_to_account<CoinType>(dd, tc_account);
    }
    spec add_currency {
        pragma opaque;
    ...
    spec schema AddCurrencyAbortsIf<CoinType> {
        dd_addr: address;
        include Dijets::AbortsIfNoCurrency<CoinType>;
        aborts_if Dijets::is_synthetic_currency<CoinType>() with Errors::INVALID_ARGUMENT;
        aborts_if exists<Dijets::PreburnQueue<CoinType>>(dd_addr) with Errors::ALREADY_PUBLISHED;
        aborts_if exists<Dijets::Preburn<CoinType>>(dd_addr) with Errors::INVALID_STATE;
    }
    ...
    public fun tiered_mint<CoinType>(
        ...
        _tier_index: u64,
    ): Dijets::Dijets<CoinType> acquires Dealer, TierInfo {
        Roles::assert_treasury_compliance(tc_account);
        ...
        Event::emit_event<ReceivedMintEvent>(
            ...
            ReceivedMintEvent {
                currency_code: Dijets::currency_code<CoinType>(),
                destination_address: dd_addr,
                amount
            },
        );
        Dijets::mint<CoinType>(tc_account, amount)
    }
    spec tiered_mint {
        pragma opaque;
        ...
        modifies global<Dealer>(dd_addr);
        modifies global<Dijets::CurrencyInfo<CoinType>>(@CurrencyInfo);
        ensures exists<Dijets::CurrencyInfo<CoinType>>(@CurrencyInfo);
        modifies global<TierInfo<CoinType>>(dd_addr);
        ensures !exists<TierInfo<CoinType>>(dd_addr);
        let currency_info = global<Dijets::CurrencyInfo<CoinType>>(@CurrencyInfo);
        let post post_currency_info = global<Dijets::CurrencyInfo<CoinType>>(@CurrencyInfo);
    ...
    spec schema TieredMintAbortsIf<CoinType> {
        ...
        aborts_if !exists<Dijets::MintCapability<CoinType>>(Signer::spec_address_of(tc_account)) with Errors::REQUIRES_CAPABILITY;
        include Dijets::MintAbortsIf<CoinType>{value: amount};
    }
    spec schema TieredMintEmits<CoinType> {
        ...
        let msg = ReceivedMintEvent {
            currency_code: Dijets::spec_currency_code<CoinType>(),
            destination_address: dd_addr,
            amount,
        };
        emits msg to handle;
        include Dijets::MintEmits<CoinType>{value: amount};
    }
