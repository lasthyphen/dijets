...
module DijetsFramework::DualAttestation {
    ...
    struct Credential has key {
        ...
        ///     protocol)
        /// Note that this is different than `authentication_key` used in DijetsAccount, which is
        ...
        /// U64_MAX. Mutable, but only by DijetsRoot.
        expiration_date: u64,
    ...
    const DOMAIN_SEPARATOR: vector<u8> = b"@@$$DIJETS_ATTEST$$@@";
    ...
    public fun rotate_base_url(account: &signer, new_url: vector<u8>) acquires Credential {
        ...
        Event::emit_event(&mut credential.base_url_rotation_events, BaseUrlRotationEvent {
            ...
            time_rotated_seconds: DijetsTimestamp::now_seconds(),
        });
    }
    ...
    spec schema RotateBaseUrlEmits {
        ...
        let msg = BaseUrlRotationEvent {
            new_base_url: new_url,
            time_rotated_seconds: DijetsTimestamp::spec_now_seconds(),
        };
        emits msg to handle;
    }
    ...
    fun dual_attestation_required<Token>(
        ...
        let travel_rule_limit_microdijets = get_cur_microdijets_limit();
        let approx_xdx_microdijets_value = Dijets::approx_xdx_for_value<Token>(deposit_value);
        let above_limit = approx_xdx_microdijets_value >= travel_rule_limit_microdijets;
        if (!above_limit) {
            return false
        };
    ...
    spec schema DualAttestationRequiredAbortsIf<Token> {
        deposit_value: num;
        include Dijets::ApproxXdmForValueAbortsIf<Token>{from_value: deposit_value};
    ...
    spec fun spec_dual_attestation_required<Token>(
        ...
        Dijets::spec_approx_xdx_for_value<Token>(deposit_value)
                    >= spec_get_cur_microdijets_limit() &&
        payer != payee &&
    ...
    public fun initialize(dr_account: &signer) {
        ...
        CoreAddresses::assert_dijets_root(dr_account); // operational constraint.
        assert(!exists<Limit>(@DijetsRoot), Errors::already_published(ELIMIT));
        let initial_limit = (INITIAL_DUAL_ATTESTATION_LIMIT as u128) * (Dijets::scaling_factor<XDX>() as u128);
    ...
    spec initialize {
        ...
        aborts_if exists<Limit>(@DijetsRoot) with Errors::ALREADY_PUBLISHED;
        let initial_limit = INITIAL_DUAL_ATTESTATION_LIMIT * Dijets::spec_scaling_factor<XDX>();
        aborts_if initial_limit > MAX_U64 with Errors::LIMIT_EXCEEDED;
        include Dijets::AbortsIfNoCurrency<XDX>; // for scaling_factor.
    }

    /// Return the current dual attestation limit in microdijets
    public fun get_cur_microdijets_limit(): u64 acquires Limit {
        assert(exists<Limit>(@DijetsRoot), Errors::not_published(ELIMIT));
        borrow_global<Limit>(@DijetsRoot).micro_xdx_limit
    }
    spec get_cur_microdijets_limit {
        pragma opaque;
        aborts_if !spec_is_published() with Errors::NOT_PUBLISHED;
        ensures result == spec_get_cur_microdijets_limit();
    }

    /// Set the dual attestation limit to `micro_dijets_limit`.
    /// Aborts if `tc_account` does not have the TreasuryCompliance role
    public fun set_microdijets_limit(tc_account: &signer, micro_xdx_limit: u64) acquires Limit {
        Roles::assert_treasury_compliance(tc_account);
        assert(exists<Limit>(@DijetsRoot), Errors::not_published(ELIMIT));
        borrow_global_mut<Limit>(@DijetsRoot).micro_xdx_limit = micro_xdx_limit;
    }
    spec set_microdijets_limit {
        ...
        aborts_if !spec_is_published() with Errors::NOT_PUBLISHED;
        ensures global<Limit>(@DijetsRoot).micro_xdx_limit == micro_xdx_limit;
    }
    ...
    spec module {
        invariant DijetsTimestamp::is_operating() ==> spec_is_published();
    }
    ...
    spec module {
        ...
        fun spec_is_published(): bool {
            exists<Limit>(@DijetsRoot)
        }

        /// Mirrors `Self::get_cur_microdijets_limit`.
        fun spec_get_cur_microdijets_limit(): u64 {
            global<Limit>(@DijetsRoot).micro_xdx_limit
        }
    }
    ...
    /// Only set_microdijets_limit can change the limit [[H6]][PERMISSION].
    spec schema DualAttestationLimitRemainsSame {
        /// The DualAttestation limit stays constant.
        ensures old(spec_is_published())
            ==> spec_get_cur_microdijets_limit() == old(spec_get_cur_microdijets_limit());
    }
    spec module {
        apply DualAttestationLimitRemainsSame to * except set_microdijets_limit;
    }
