...
module DijetsFramework::DijetsBlock {
    ...
    fun is_initialized(): bool {
        exists<BlockMetadata>(@DijetsRoot)
    }
    ...
    fun block_prologue(
        ...
        // Authorization
        assert(
            proposer == @VMReserved || DijetsSystem::is_validator(proposer),
            Errors::requires_address(EVM_OR_VALIDATOR)
        );

        let block_metadata_ref = borrow_global_mut<BlockMetadata>(@DijetsRoot);
        DijetsTimestamp::update_global_time(&vm, proposer, timestamp);
    ...
    spec block_prologue {
        ...
        aborts_if proposer != @VMReserved && !DijetsSystem::spec_is_validator(proposer)
            with Errors::REQUIRES_ADDRESS;
        ensures DijetsTimestamp::spec_now_microseconds() == timestamp;
    ...
    spec schema BlockPrologueEmits {
        ...
        previous_block_votes: vector<address>;
        proposer: address;
        let handle = global<BlockMetadata>(@DijetsRoot).new_block_events;
    ...
    public fun get_current_block_height(): u64 acquires BlockMetadata {
        ...
        borrow_global<BlockMetadata>(@DijetsRoot).height
    }
