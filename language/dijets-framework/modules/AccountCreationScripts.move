module DijetsFramework::AccountCreationScripts {
    ...
    /// * A `DijetsAccount::CreateAccountEvent` with the `created` field being `child_address`,
    /// and the `rold_id` field being `Roles::CHILD_VASP_ROLE_ID`. This is emitted on the
    /// `DijetsAccount::AccountOperationsCapability` `creation_events` handle.
    ///
    /// Successful execution with a `child_initial_balance` greater than zero will additionaly emit:
    /// * A `DijetsAccount::SentPaymentEvent` with the `payee` field being `child_address`.
    /// This is emitted on the Parent VASP's `DijetsAccount::DijetsAccount` `sent_events` handle.
    /// * A `DijetsAccount::ReceivedPaymentEvent` with the  `payer` field being the Parent VASP's address.
    /// This is emitted on the new Child VASPS's `DijetsAccount::DijetsAccount` `received_events` handle.
    ///
    /// # Parameters
    ...
    /// | ----------------            | --------------                                           | -------------                                                                            |
    /// | `Errors::INVALID_ARGUMENT`  | `DijetsAccount::EMALFORMED_AUTHENTICATION_KEY`            | The `auth_key_prefix` was not of length 32.                                              |
    ...
    /// | `Errors::NOT_PUBLISHED`     | `Dijets::ECURRENCY_INFO`                                  | The `CoinType` is not a registered currency on-chain.                                    |
    /// | `Errors::INVALID_STATE`     | `DijetsAccount::EWITHDRAWAL_CAPABILITY_ALREADY_EXTRACTED` | The withdrawal capability for the sending account has already been extracted.            |
    /// | `Errors::NOT_PUBLISHED`     | `DijetsAccount::EPAYER_DOESNT_HOLD_CURRENCY`              | The sending account doesn't have a balance in `CoinType`.                                |
    /// | `Errors::LIMIT_EXCEEDED`    | `DijetsAccount::EINSUFFICIENT_BALANCE`                    | The sending account doesn't have at least `child_initial_balance` of `CoinType` balance. |
    /// | `Errors::INVALID_ARGUMENT`  | `DijetsAccount::ECANNOT_CREATE_AT_VM_RESERVED`            | The `child_address` is the reserved address 0x0.                                         |
    ///
    /// # Related Scripts
    ...
    public(script) fun create_child_vasp_account<CoinType>(
        ...
        DijetsAccount::create_child_vasp_account<CoinType>(
            &parent_vasp,
            child_address,
            auth_key_prefix,
            add_all_currencies,
        ...
        if (child_initial_balance > 0) {
            let vasp_withdrawal_cap = DijetsAccount::extract_withdraw_capability(&parent_vasp);
            DijetsAccount::pay_from<CoinType>(
                &vasp_withdrawal_cap, child_address, child_initial_balance, x"", x""
            );
            DijetsAccount::restore_withdraw_capability(vasp_withdrawal_cap);
        };
    }
    ...
    spec create_child_vasp_account {
        ...
        include DijetsAccount::TransactionChecks{sender: parent_vasp}; // properties checked by the prologue.
        let parent_addr = Signer::spec_address_of(parent_vasp);
        let parent_cap = DijetsAccount::spec_get_withdraw_cap(parent_addr);
        include DijetsAccount::CreateChildVASPAccountAbortsIf<CoinType>{
        ...
        include (child_initial_balance > 0) ==>
            DijetsAccount::ExtractWithdrawCapAbortsIf{sender_addr: parent_addr};
        ...
        include (child_initial_balance) > 0 ==>
            DijetsAccount::PayFromAbortsIfRestricted<CoinType>{
                cap: parent_cap,
                payee: child_address,
                amount: child_initial_balance,
                metadata: x"",
            };
        include DijetsAccount::CreateChildVASPAccountEnsures<CoinType>{
            parent_addr: parent_addr,
            child_addr: child_address,
        };
        ensures DijetsAccount::balance<CoinType>(child_address) == child_initial_balance;
        ensures DijetsAccount::balance<CoinType>(parent_addr)
            == old(DijetsAccount::balance<CoinType>(parent_addr)) - child_initial_balance;
        ...
        /*
        include DijetsAccount::MakeAccountEmits{new_account_address: child_address};
        include child_initial_balance > 0 ==>
            DijetsAccount::PayFromEmits<CoinType>{
                cap: parent_cap,
                payee: child_address,
                amount: child_initial_balance,
                metadata: x"",
    ...
    /// # Summary
    /// Creates a Validator Operator account. This transaction can only be sent by the Dijets
    /// Root account.
    ///
    /// # Technical Description
    ...
    /// # Events
    /// Successful execution will emit:
    /// * A `DijetsAccount::CreateAccountEvent` with the `created` field being `new_account_address`,
    ...
    /// | Name                  | Type         | Description                                                                              |
    /// | ------                | ------       | -------------                                                                            |
    /// | `dr_account`          | `signer`     | The signer of the sending account of this transaction. Must be the Dijets Root signer.     |
    ...
    /// | `Errors::REQUIRES_ADDRESS`  | `CoreAddresses::EDIJETS_ROOT`            | The sending account is not the Dijets Root account.                                         |
    /// | `Errors::REQUIRES_ROLE`     | `Roles::EDIJETS_ROOT`                    | The sending account is not the Dijets Root account.                                         |
    ...
    /// Only Dijets root may create Validator Operator accounts
    /// Authentication: ValidatorAccountAbortsIf includes AbortsIfNotDijetsRoot.
    ...
    /// * REQUIRES_ROLE comes from `Roles::assert_dijets_root`. However, assert_dijets_root checks the literal
    ///   Dijets root address before checking the role, and the role abort is unreachable in practice, since
    ///   only Dijets root has the Dijets root role.
    spec create_validator_operator_account {
        use Std::Errors;
        use DijetsFramework::Roles;

        include DijetsAccount::TransactionChecks{sender: dr_account}; // properties checked by the prologue.
        include SlidingNonce::RecordNonceAbortsIf{seq_nonce: sliding_nonce, account: dr_account};
        include DijetsAccount::CreateValidatorOperatorAccountAbortsIf;
        include DijetsAccount::CreateValidatorOperatorAccountEnsures;
        ...
        include DijetsAccount::MakeAccountEmits;

        /// **Access Control:**
        /// Only the Dijets Root account can create Validator Operator accounts [[A4]][ROLE].
        include Roles::AbortsIfNotDijetsRoot{account: dr_account};
    }

    /// # Summary
    /// Creates a Validator account. This transaction can only be sent by the Dijets
    /// Root account.
    ///
    /// # Technical Description
    ...
    /// Only Dijets root may create Validator accounts
    /// Authentication: ValidatorAccountAbortsIf includes AbortsIfNotDijetsRoot.
    /// Checks that above table includes all error categories.
    /// The verifier finds an abort that is not documented, and cannot occur in practice:
    /// * REQUIRES_ROLE comes from `Roles::assert_dijets_root`. However, assert_dijets_root checks the literal
    ...
    spec create_validator_account {
        ...
        include DijetsAccount::TransactionChecks{sender: dr_account}; // properties checked by the prologue.
        include SlidingNonce::RecordNonceAbortsIf{seq_nonce: sliding_nonce, account: dr_account};
        include DijetsAccount::CreateValidatorAccountAbortsIf;
        include DijetsAccount::CreateValidatorAccountEnsures;
        ...
        /// **Access Control:**
        /// Only the Dijets Root account can create Validator accounts [[A3]][ROLE].
        include Roles::AbortsIfNotDijetsRoot{account: dr_account};
    }
    ...
    public(script) fun create_parent_vasp_account<CoinType>(
        ...
        DijetsAccount::create_parent_vasp_account<CoinType>(
            &tc_account,
            new_account_address,
            auth_key_prefix,
            human_name,
    ...
    spec create_parent_vasp_account {
        ...
        include DijetsAccount::CreateParentVASPAccountAbortsIf<CoinType>{creator_account: tc_account};
        include DijetsAccount::CreateParentVASPAccountEnsures<CoinType>;
    ...
    /// # Events
    /// Successful execution will emit:
    /// * A `DijetsAccount::CreateAccountEvent` with the `created` field being `addr`,
    ...
    public(script) fun create_designated_dealer<Currency>(
        ...
        SlidingNonce::record_nonce_or_abort(&tc_account, sliding_nonce);
        DijetsAccount::create_designated_dealer<Currency>(
            &tc_account,
            addr,
            auth_key_prefix,
            human_name,
    ...
    spec create_designated_dealer {
        ...
        include DijetsAccount::TransactionChecks{sender: tc_account}; // properties checked by the prologue.
        include SlidingNonce::RecordNonceAbortsIf{account: tc_account, seq_nonce: sliding_nonce};
        include DijetsAccount::CreateDesignatedDealerAbortsIf<Currency>{
            creator_account: tc_account, new_account_address: addr};
        include DijetsAccount::CreateDesignatedDealerEnsures<Currency>{new_account_address: addr};
        ...
        include DijetsAccount::MakeAccountEmits{new_account_address: addr};
