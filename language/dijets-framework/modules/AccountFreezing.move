...
module DijetsFramework::AccountFreezing {
    ...
    /// An attempt to freeze the Dijets Root account was attempted
    const ECANNOT_FREEZE_DIJETS_ROOT: u64 = 3;
    ...
    acquires FreezingBit, FreezeEventsHolder {
        ...
        // The dijets root account and TC cannot be frozen
        assert(frozen_address != @DijetsRoot, Errors::invalid_argument(ECANNOT_FREEZE_DIJETS_ROOT));
        ...
        Event::emit_event<FreezeAccountEvent>(
            &mut borrow_global_mut<FreezeEventsHolder>(@DijetsRoot).freeze_event_handle,
            FreezeAccountEvent {
                initiator_address,
                frozen_address
            },
    ...
    spec freeze_account {
        ...
        aborts_if frozen_address == @DijetsRoot with Errors::INVALID_ARGUMENT;
    ...
    spec schema FreezeAccountEmits {
        account: &signer;
        frozen_address: address;
        let handle = global<FreezeEventsHolder>(@DijetsRoot).freeze_event_handle;
    ...
    acquires FreezingBit, FreezeEventsHolder {
        ...
        Event::emit_event<UnfreezeAccountEvent>(
            &mut borrow_global_mut<FreezeEventsHolder>(@DijetsRoot).unfreeze_event_handle,
    ...
    spec schema UnfreezeAccountEmits {
        account: &signer;
        unfrozen_address: address;
        let handle = global<FreezeEventsHolder>(@DijetsRoot).unfreeze_event_handle;
    ...
    spec module {
        ...
        invariant DijetsTimestamp::is_operating() ==>
            exists<FreezeEventsHolder>(@DijetsRoot);
    }

    /// # Access Control
    spec module {
        /// The account of DijetsRoot is not freezable [[F1]][ROLE].
        /// After genesis, FreezingBit of DijetsRoot is always false.
        invariant DijetsTimestamp::is_operating() ==>
            spec_account_is_not_frozen(@DijetsRoot);
        ...
        invariant update DijetsTimestamp::is_operating() ==> exists<FreezeEventsHolder>(@DijetsRoot);
