...
pub fn dijets_stdlib_modules_full_path() -> String {
    format!("{}/{}", env!("CARGO_MANIFEST_DIR"), MODULES_DIR)
}

pub fn dijets_stdlib_files_no_dependencies() -> Vec<String> {
    let path = path_in_crate(MODULES_DIR);
    find_filenames(&[path], |p| extension_equals(p, MOVE_EXTENSION)).unwrap()
}

pub fn dijets_stdlib_files() -> Vec<String> {
    let mut files = move_stdlib::move_stdlib_files();
    files.extend(dijets_stdlib_files_no_dependencies());
    files
}

// TODO: This will be declared in the package once those are in
pub fn dijets_framework_named_addresses() -> BTreeMap<String, AddressBytes> {
    let mapping = [
        ("Std", "0x1"),
        ("DijetsFramework", "0x1"),
        ("DijetsRoot", "0xA550C18"),
...
pub(crate) fn build_stdlib() -> BTreeMap<Symbol, CompiledModule> {
    let (_files, compiled_units) = Compiler::new(&dijets_stdlib_files(), &[])
