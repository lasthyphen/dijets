...
fn main() {
    let cli = App::new("stdlib")
        ...
        .author("The Dijets Core Contributors")
    ...
    // Make sure that the current directory is `language/dijets-framework` from now on.
    let exec_path = std::env::args().next().expect("path of the executable");
    let base_path = std::path::Path::new(&exec_path)
        .parent()
        .unwrap()
        .join("../../language/dijets-framework");
