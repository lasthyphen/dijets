...
//! Rust representation of a Move transaction script that can be executed on the Dijets blockchain.
//! Dijets does not allow arbitrary transaction scripts; only scripts whose hashes are present in
...
use dijets_types::transaction::{ScriptABI, TransactionScriptABI};
...
/// All of the Move transaction scripts that can be executed on the Dijets blockchain
...
impl LegacyStdlibScript {
    ...
    /// be executed on the Dijets blockchain
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        write!(
            ...
            match self {
                ...
                UpdateDijetsVersion => "update_dijets_version",
    ...
    fn test_names() {
        ...
        for script in LegacyStdlibScript::all() {
            assert_eq!(
                script.name(),
                script.abi().name(),
                "The main function in language/dijets-framework/transaction_scripts/{}.move is named `{}` instead of `{}`.",
                script.name(),
                script.abi().name(),
                script.name(),
            );
    ...
    fn test_docs() {
        ...
        for script in LegacyStdlibScript::all() {
            assert!(
                ...
                "The main function in language/dijets-framework/transaction_scripts/{}.move does not have a `///` inline doc comment.",
                script.name(),
            );
        }
    }
