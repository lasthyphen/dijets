...
use crate::{
    ...
    dijets_transaction_validator::validate_signature_checked_transaction,
    dijets_vm::{
        charge_global_write_gas_usage, convert_changeset_and_events, get_transaction_output,
        DijetsVMImpl, DijetsVMInternals,
    },
...
use dijets_types::{
    ...
    on_chain_config::DIJETS_VERSION_3,
    transaction::{
...
pub struct DijetsVM(DijetsVMImpl);

impl DijetsVM {
    pub fn new<S: StateView>(state: &S) -> Self {
        Self(DijetsVMImpl::new(state))
    }

    pub fn internals(&self) -> DijetsVMInternals {
        DijetsVMInternals::new(&self.0)
    }
        ...
        // Run the execution logic
        {
            ...
            match payload {
                TransactionPayload::Script(script) => {
                    let dijets_version = self.0.get_dijets_version()?;
                    let remapped_script =
                        if dijets_version < dijets_types::on_chain_config::DIJETS_VERSION_2 {
                            None
                        } else {
                            script_to_script_function::remapping(script.code())
                        };
                    let mut senders = vec![txn_data.sender()];
                    if dijets_version >= DIJETS_VERSION_3 {
    ...
    fn execute_writeset<S: MoveResolver>(
        ...
        Ok(match writeset_payload {
            ...
            WriteSetPayload::Script { script, execute_as } => {
                ...
                let dijets_version = self.0.get_dijets_version().map_err(Err)?;
                let senders = match txn_sender {
                    None => vec![*execute_as],
                    Some(sender) => vec![sender, *execute_as],
                };
                let remapped_script = if dijets_version < dijets_types::on_chain_config::DIJETS_VERSION_2
                {
                    None
                } else {
                    script_to_script_function::remapping(script.code())
        ...
        self.execute_writeset_transaction(
            ...
            match txn.payload() {
                ...
                | TransactionPayload::ScriptFunction(_) => {
                    log_context.alert();
                    error!(*log_context, "[dijets_vm] UNREACHABLE");
    ...
    pub(crate) fn execute_block_impl(
        ...
        rayon::scope(|scope| {
            scope.spawn(|_| {
                DijetsVM::preload_cache(&signature_verified_block, data_cache);
            });
        });
    ...
    pub fn execute_block_and_keep_vm_status(
        ...
        let vm = DijetsVM::new(&state_view_cache);
...
impl AsRef<DijetsVMImpl> for DijetsVM {
    fn as_ref(&self) -> &DijetsVMImpl {
        &self.0
    }
}

impl AsMut<DijetsVMImpl> for DijetsVM {
    fn as_mut(&mut self) -> &mut DijetsVMImpl {
        &mut self.0
    }
}
