...
use crate::{
    counters::*, create_access_path, data_cache::StateViewCache, dijets_vm::DijetsVMImpl,
...
use dijets_types::{
    ...
    on_chain_config::{DijetsVersion, VMConfig, VMPublishingOption, DIJETS_VERSION_2, DIJETS_VERSION_3},
    transaction::{
...
#[derive(Clone)]
pub struct DijetsVMValidator(DijetsVMImpl);

impl DijetsVMValidator {
    ...
    pub fn init_with_config(
        ...
        DijetsVMValidator(DijetsVMImpl::init_with_config(
            version,
            on_chain_config,
            publishing_option,
        ))
...
impl VMValidator for DijetsVMValidator {
    ...
    /// 3. Invokes `DijetsAccount.prologue`, which checks properties such as the transaction has the
...
pub(crate) fn validate_signature_checked_transaction<S: MoveResolver>(
    ...
    if transaction.is_multi_agent() && vm.get_dijets_version()? < DIJETS_VERSION_3 {
    ...
    let prologue_status = match transaction.payload() {
        ...
        TransactionPayload::ScriptFunction(_) => {
            // gate the behavior until the Dijets version is ready
            if vm.get_dijets_version()? < DIJETS_VERSION_2 {
                return Err(VMStatus::Error(StatusCode::FEATURE_UNDER_GATING));
            }
