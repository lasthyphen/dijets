...
use dijets_types::{
    ...
    event::EventKey,
    on_chain_config::{
        ConfigStorage, DijetsVersion, OnChainConfig, VMConfig, VMPublishingOption, DIJETS_VERSION_3,
    },
...
pub struct DijetsVMImpl {
    move_vm: Arc<MoveVM>,
    on_chain_config: Option<VMConfig>,
    version: Option<DijetsVersion>,
...
impl DijetsVMImpl {
    ...
    pub fn new<S: StateView>(state: &S) -> Self {
        let inner = MoveVM::new(dijets_natives())
    ...
    /// Provides access to some internal APIs of the Dijets VM.
    pub fn internals(&self) -> DijetsVMInternals {
        DijetsVMInternals(self)
    }
    ...
    fn load_configs_impl<S: ConfigStorage>(&mut self, data_cache: &S) {
        ...
        self.version = DijetsVersion::fetch_config(data_cache);
    ...
    pub fn get_dijets_version(&self) -> Result<DijetsVersion, VMStatus> {
        self.version.clone().ok_or_else(|| {
            CRITICAL_ERRORS.inc();
            error!("VM Startup Failed. Dijets Version Not Found");
            VMStatus::Error(StatusCode::VM_STARTUP_FAILURE)
        })
    }
    ...
    pub(crate) fn run_script_prologue<S: MoveResolver>(
        ...
        let secondary_public_key_hashes: Vec<MoveValue> = txn_data
            ...
            .collect();
        let args = if self.get_dijets_version()? >= DIJETS_VERSION_3 && txn_data.is_multi_agent() {
            vec![
        ...
        let prologue_function_name =
            if self.get_dijets_version()? >= DIJETS_VERSION_3 && txn_data.is_multi_agent() {
...
/// Internal APIs for the Dijets VM, primarily used for testing.
#[derive(Clone, Copy)]
pub struct DijetsVMInternals<'a>(&'a DijetsVMImpl);

impl<'a> DijetsVMInternals<'a> {
    pub fn new(internal: &'a DijetsVMImpl) -> Self {
        Self(internal)
    }
    ...
    pub fn dijets_version(self) -> Result<DijetsVersion, VMStatus> {
        self.0.get_dijets_version()
    }
...
fn vm_thread_safe() {
    ...
    use crate::{DijetsVM, DijetsVMValidator};

    assert_send::<DijetsVM>();
    assert_sync::<DijetsVM>();
    assert_send::<DijetsVMValidator>();
    assert_sync::<DijetsVMValidator>();
    assert_send::<MoveVM>();
    assert_sync::<MoveVM>();
}
