...
pub mod known_locations {
    ...
    pub const ACCOUNT_MODULE_IDENTIFIER: &IdentStr = ident_str!("DijetsAccount");
    ...
    /// The Identifier for the Dijets module.
    pub const DIJETS_MODULE_IDENTIFIER: &IdentStr = ident_str!("Dijets");
    /// The ModuleId for the Dijets module.
    pub static DIJETS_MODULE: Lazy<ModuleId> =
        Lazy::new(|| ModuleId::new(CORE_CODE_ADDRESS, DIJETS_MODULE_IDENTIFIER.to_owned()));
    pub fn dijets_module_abort() -> AbortLocation {
        AbortLocation::Module(DIJETS_MODULE.clone())
    }
...
pub enum StatusCode {
    ...
    // The feature requested is intended for a future Dijets version instead of the current one
