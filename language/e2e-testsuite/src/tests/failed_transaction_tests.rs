...
use dijets_types::vm_status::{KeptVMStatus, StatusCode, VMStatus};
use dijets_vm::{
    data_cache::StateViewCache, logging::AdapterLogSchema,
    transaction_metadata::TransactionMetadata, DijetsVM,
};
...
fn failed_transaction_cleanup_test() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        let dijets_vm = DijetsVM::new(executor.get_state_view());
        ...
        let out1 = dijets_vm.failed_transaction_cleanup(
        ...
        let out2 = dijets_vm.failed_transaction_cleanup(
