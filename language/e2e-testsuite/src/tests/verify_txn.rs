...
use dijets_transaction_builder::{
...
#[test]
pub fn test_publish_from_dijets_root() {
    // create a FakeExecutor with a genesis from file
    let mut executor =
        FakeExecutor::from_genesis_with_options(VMPublishingOption::custom_scripts());
    executor.set_golden_file(current_function_name!());
...
#[test]
pub fn test_no_publishing_dijets_root_sender() {
    // create a FakeExecutor with a genesis from file
    let mut executor =
        FakeExecutor::from_genesis_with_options(VMPublishingOption::custom_scripts());
    executor.set_golden_file(current_function_name!());
    ...
    // create a transaction trying to publish a new module.
    let sender = Account::new_dijets_root();
...
fn good_module_uses_bad(
    ...
    let compiler = Compiler {
        address,
        deps: dijets_framework_releases::current_modules()
            .iter()
...
fn test_module_dependency_fails_verification() {
    ...
    let good_module = {
        ...
        dijets_types::transaction::Module::new(serialized_module)
    };
...
fn test_module_transitive_dependency_fails_verification() {
    ...
    let module = {
        ...
        dijets_types::transaction::Module::new(
            compiler
    ...
    let module = r#"
        module COIN {
            ...
            public initialize(dr_account: &signer, tc_account: &signer) {
                Dijets.register_SCS_currency<Self.COIN>(
                    move(dr_account),
                    move(tc_account),
