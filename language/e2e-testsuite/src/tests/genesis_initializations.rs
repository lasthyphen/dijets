...
fn test_dijets_initialize() {
    ...
    let output = executor.try_exec(
        ...
        serialize_values(&vec![
            MoveValue::Signer(account_config::dijets_root_address()),
        ]),
    );
...
#[test]
fn test_dijets_initialize_tc_account() {
    let mut executor = FakeExecutor::stdlib_only_genesis();
...
#[test]
fn test_dijets_timestamp_time_has_started() {
    let mut executor = FakeExecutor::stdlib_only_genesis();
    let account_address = AccountAddress::random();

    // Invalid address used to call `DijetsTimestamp::set_time_has_started`
    let output = executor.try_exec(
        "DijetsTimestamp",
...
#[test]
fn test_dijets_block_double_init() {
    let mut executor = FakeExecutor::stdlib_only_genesis();
