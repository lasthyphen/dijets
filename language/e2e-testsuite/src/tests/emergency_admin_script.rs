...
fn validator_batch_remove() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        let dijets_root_account = test_env.dr_account;
        ...
        assert!(executor
            .try_exec(
                ...
                serialize_values(&vec![
                    MoveValue::Signer(*dijets_root_account.address()),
...
fn halt_network() {
    ...
    // `DijetsTransactionPublishingOption::halt_all_transactions` is not available in version 1.
    test_with_different_versions! {&[2], |test_env| {
        ...
        let auth_key = dijets_root_account.auth_key();
        // DijetsRoot can still send transaction
        executor.execute_and_apply(
            dijets_root_account
                .transaction()
