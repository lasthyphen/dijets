...
use dijets_transaction_builder::stdlib::encode_update_dual_attestation_limit_script;
use dijets_types::{
...
#[test]
fn initial_dijets_version() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        let mut executor = test_env.executor;

        let vm = DijetsVM::new(executor.get_state_view());

        assert_eq!(
            vm.internals().dijets_version().unwrap(),
            DijetsVersion { major: test_env.version_number }
        );
        ...
        assert_eq!(
            ...
            DijetsVersion { major: test_env.version_number + 1 }
        );
    }
    }
}
...
fn update_script_allow_list() {
    ...
    let dr = Account::new_dijets_root();
    // create and publish a sender with 5_000_000 coins and a receiver with 0 coins
    let sender = executor.create_raw_account_data(5_000_000, 10);
    executor.add_account_data(&sender);
    ...
    // DIJETS_ROOT can send arbitrary txn to the network.
    let random_script = vec![];
    let txn = dr
        .transaction()
        .script(Script::new(random_script, vec![], vec![]))
...
fn update_consensus_config() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        if test_env.version_number == 2 {
            ...
            assert!(matches!(executor.execute_transaction(generate_txn(seq_num, "update_dijets_consensus_config", Some(vec![1,2,3]))).status(), &TransactionStatus::Keep(KeptVMStatus::MoveAbort(_, _))));
            assert_eq!(executor.execute_and_apply(generate_txn(seq_num, "initialize_dijets_consensus_config", None)).status(), &TransactionStatus::Keep(KeptVMStatus::Executed));
            assert_eq!(executor.execute_and_apply(generate_txn(seq_num + 1, "update_dijets_consensus_config", Some(vec![1,2,3]))).status(), &TransactionStatus::Keep(KeptVMStatus::Executed));
        }
