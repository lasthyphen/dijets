...
use dijets_crypto_derive::{CryptoHasher, DeserializeKey, SerializeKey};
...
/// Each transaction submitted to the Dijets blockchain contains a `TransactionAuthenticator`. During
...
/// Each on-chain `DijetsAccount` must store an `AuthenticationKey` (computed via a sha3 hash of an
