...
/// ## Overview
///
/// Dijets `NetworkAddress` is a compact, efficient, self-describing and
...
/// 4. Perform a DijetsNet version negotiation handshake (version 1).
///
...
/// ## Transport
///
/// In addition, `NetworkAddress` is integrated with the DijetsNet concept of a
...
///
/// use dijets_types::network_address::NetworkAddress;
/// use bcs;
...
impl NetworkAddress {
    ...
    ///
    /// ```rust
    /// use dijets_crypto::{traits::ValidCryptoMaterialStringExt, x25519};
    /// use dijets_types::network_address::NetworkAddress;
    /// use std::str::FromStr;
    ///
    ...
    /// Check that a `NetworkAddress` looks like a typical DijetsNet address with
    /// associated protocols.
    ///
    /// "typical" DijetsNet addresses begin with a transport protocol:
    ///
    ...
    /// assert!(addr.is_dijetsnet_addr());
    /// ```
    pub fn is_dijetsnet_addr(&self) -> bool {
        parse_dijetsnet_protos(self.as_slice()).is_some()
    }
...
#[cfg(any(test, feature = "fuzzing"))]
pub fn arb_dijetsnet_addr() -> impl Strategy<Value = NetworkAddress> {
    ...
    let arb_dijetsnet_protos = any::<(x25519::PublicKey, u8)>()
        .prop_map(|(pubkey, hs)| vec![Protocol::NoiseIK(pubkey), Protocol::Handshake(hs)]);

    (arb_transport_protos, arb_dijetsnet_protos).prop_map(
        |(mut transport_protos, mut dijetsnet_protos)| {
            transport_protos.append(&mut dijetsnet_protos);
...
/// parse canonical dijetsnet protocols
///
/// See: [`NetworkAddress::is_dijetsnet_addr`]
fn parse_dijetsnet_protos(protos: &[Protocol]) -> Option<&[Protocol]> {
...
mod test {
    ...
    proptest! {
        ...
        #[test]
        fn test_is_dijetsnet_addr(addr in arb_dijetsnet_addr()) {
            assert!(addr.is_dijetsnet_addr(), "addr.is_dijetsnet_addr() = false; addr: '{}'", addr);
        }

        #[test]
        fn test_is_not_dijetsnet_addr_with_trailing(
            addr in arb_dijetsnet_addr(),
            addr_suffix in any::<NetworkAddress>(),
        ) {
            // A valid DijetsNet addr w/ unexpected trailing protocols should not parse.
            let addr = addr.extend_from_slice(addr_suffix.as_slice());
            assert!(!addr.is_dijetsnet_addr(), "addr.is_dijetsnet_addr() = true; addr: '{}'", addr);
        }
    }
}
