...
fn test_role_ordering() {
    use GovernanceRole::*;
    assert!(DijetsRoot.priority() > TreasuryCompliance.priority());
    assert!(DijetsRoot.priority() > Validator.priority());
    assert!(DijetsRoot.priority() > ValidatorOperator.priority());
    assert!(DijetsRoot.priority() > DesignatedDealer.priority());
