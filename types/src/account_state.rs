...
use crate::{
    ...
    block_metadata::DijetsBlockResource,
    dijets_timestamp::DijetsTimestampResource,
    on_chain_config::{
        ConfigurationResource, DijetsVersion, OnChainConfig, RegisteredCurrencies,
...
impl AccountState {
    ...
    pub fn get_dijets_timestamp_resource(&self) -> Result<Option<DijetsTimestampResource>> {
        self.get_resource::<DijetsTimestampResource>()
    }
    ...
    pub fn get_dijets_version(&self) -> Result<Option<DijetsVersion>> {
        self.get_config::<DijetsVersion>()
    }
    ...
    pub fn get_dijets_block_resource(&self) -> Result<Option<DijetsBlockResource>> {
        self.get_resource::<DijetsBlockResource>()
    }
...
impl fmt::Debug for AccountState {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        let dijets_timestamp_str = self
            .get_dijets_timestamp_resource()
            .map(|dijets_timestamp_opt| format!("{:#?}", dijets_timestamp_opt))
        ...
        write!(
            ...
            account_resource_str, dijets_timestamp_str, validator_config_str, validator_set_str,
        )
    }
}
