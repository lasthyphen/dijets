...
#[structopt(
    name = "swiss-knife",
    about = "Tool for generating, serializing (BCS), hashing and signing Dijets transactions. Additionally, contains tools for testing. Please refer to README.md for examples."
)]
struct Opt {
...
struct TxnParams {
    ...
    // Chain ID of the Dijets network this transaction is intended for
    pub chain_id: String,
...
struct GenerateKeypairResponse {
    ...
    pub public_key: String,
    pub dijets_auth_key: String,
    pub dijets_account_address: String,
}
...
fn generate_key_pair(seed: Option<u64>) -> GenerateKeypairResponse {
    ...
    let dijets_auth_key = AuthenticationKey::ed25519(&keypair.public_key);
    let dijets_account_address: String = dijets_auth_key.derived_address().to_string();
    let dijets_auth_key: String = dijets_auth_key.to_string();
