...
use dijets_faucet::mint;
use dijets_logger::prelude::info;
use dijets_sdk::types::chain_id::ChainId;
use std::fmt;
...
#[structopt(
    name = "Dijets Faucet",
    author = "The Dijets Association",
    about = "Dijets Testnet utitlty service for creating test account and minting test coins"
)]
struct Args {
    ...
    pub port: u16,
    /// Dijets fullnode/validator server URL
    #[structopt(short = "s", long, default_value = "https://testnet.dijets.com/v1")]
    ...
    #[structopt(short = "m", long, default_value = "/opt/dijets/etc/mint.key")]
...
fn routes(
    ...
    let health = warp::path!("-" / "healthy").map(|| "dijets-faucet:ok");
...
mod tests {
    ...
    fn setup(
        ...
        let port = dijets_config::utils::get_available_port();
    ...
    async fn test_healthy() {
        ...
        assert_eq!(resp.body(), "dijets-faucet:ok");
    }
    ...
    async fn test_vasp_domain() {
        ...
        // times.
        let auth_key = "459c77a38803bd53f3adee52703810e3a74fd7c46952c497e75afb0a7932586d";
        let vasp_domain = DijetsIdVaspDomainIdentifier::new("dijets").unwrap();
        ...
        {
            let vasp_domain_to_remove = "dijets";
            let resp = warp::test::request()
                .method("POST")
                .path(
                    format!(
                ...
                if let Some(script_function) = ScriptFunctionCall::decode(txn.payload()) {
                    match script_function {
                        ScriptFunctionCall::AddVaspDomain {
                            ...
                            let domain = DijetsIdVaspDomainIdentifier::new(
    ...
    fn create_response(
        ...
        serde_json::json!({
            "id": id,
            "jsonrpc": "2.0",
            "dijets_chain_id": chain_id,
            "dijets_ledger_timestampusec": 1599670083580598u64,
            "dijets_ledger_version": 2052770,
            "result": result
        })
    }
