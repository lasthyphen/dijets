...
//!
//! ```sh
//! $ cargo run -p dijets-assets-proof -- collect \
//!     --json-server 'https://testnet.dijets.com/v1' \
...
//! {
//!   "dijets_chain_id": 2,
//!   "dijets_ledger_version": 88497519,
//!   "dijets_ledger_timestampusec": 1616457122815922,
...
use dijets_types::{
    ...
    dijets_id_identifier::DijetsIdVaspDomainIdentifier, transaction::Version,
};
...
#[derive(Debug, StructOpt)]
pub enum Command {
    /// Collect a Proof-of-Assets receipt for a Dijets VASP's on-chain accounts.
...
pub struct CommonOptions {
    ...
    /// The URL of the fullnode json-rpc service used to interface with Dijets.
...
pub struct ParentVASPView {
    ...
    num_children: u64,
    vasp_domains: Option<Vec<DijetsIdVaspDomainIdentifier>>,
}
...
impl TryFrom<AccountView> for ParentVASPView {
    ...
    fn try_from(account: AccountView) -> Result<Self> {
            ...
            !account.is_frozen,
            "account is currently frozen by Dijets treasury compliance"
        );
...
impl From<MetadataView> for SimpleMetadataView {
    fn from(metadata: MetadataView) -> Self {
        SimpleMetadataView {
            dijets_chain_id: ChainId::new(metadata.chain_id),
            dijets_ledger_version: metadata.version,
            dijets_ledger_timestampusec: metadata.timestamp,
...
impl CollectOptions {
    fn exec(&self) -> Result<AssetsProof> {
        let client = dijets_client::BlockingClient::new(self.common.json_server.clone());
...
/// A small trait abstracting over the Dijets json-rpc client so we can mock during
/// testing.
pub trait Client {
...
impl Client for dijets_client::BlockingClient {
    fn get_last_version_before_timestamp(
        &self,
    ...
    fn get_account_by_version(
        ...
        // HACK: until the follwing PR lands and hits release (https://github.com/dijets/dijets/pull/7983),
