...
// https://dip.dijets.com/dip-1/#list-of-error-codes
#[derive(Debug, PartialEq, Eq, Deserialize, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum ErrorCode {
    //
    ...
    ///    address.
    /// * Could not find Dijets's onchain account by the `X-REQUEST-SENDER-ADDRESS` header value.
    ...
    /// * Currency field value is not a valid Dijets currency code for the connected network.
    InvalidFieldValue,
    ...
    /// Field payment.action.currency value is a valid Dijets currency code, but it is not supported
...
pub struct PaymentActorObject {
    ...
    /// than 80 characters. Mandatory and immutable. For Dijets addresses, refer to the "account
...
pub struct PaymentActionObject {
    ...
    /// currency. For example, if DijetsUSD is represented on-chain where “1” equals 1e-6 dollars,
    ...
    // TODO Should be an enum per https://dip.dijets.com/dip-1/#paymentactionobject
    pub currency: String,
