...
use dijets_sdk::{
    ...
    types::{
        ...
        account_config::{dijets_root_address, events::new_block::NewBlockEvent},
...
fn metadata() -> Result<()> {
    ...
    metadata.dijets_version.unwrap();
...
fn metadata_by_version() -> Result<()> {
    ...
    assert!(metadata.dijets_version.is_none());
...
fn script_hash_allow_list() {
    ...
    let allow_list: Vec<Script> = [
        ...
        stdlib::encode_update_dijets_version_script(0, 0),
...
fn get_event_by_version_with_proof() -> Result<()> {
    ...
    let batch = vec![
        ...
        MethodRequest::get_account_state_with_proof(dijets_root_address(), None, None),
    ...
    let block_height = account_state.get_dijets_block_resource()?.unwrap().height();
...
fn dual_attestation() -> Result<()> {
    use dijets_sdk::crypto::ed25519::ed25519_dalek;
    ...
    assert!(matches!(
        client
            ...
            .unwrap_err(),
        dijets_sdk::client::WaitForTransactionError::TransactionExecutionFailed(_),
    ));
