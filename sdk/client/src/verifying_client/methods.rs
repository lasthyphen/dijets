...
fn get_latest_metadata() -> VerifyingRequest {
    ...
    let callback: RequestCallback = Box::new(move |ctxt, subresponses| {
        let dijets_root = match subresponses {
        ...
        let dijets_root = verify_account_state(ctxt, dijets_root, dijets_root_address(), None)?
            .ok_or_else(|| Error::rpc_response("DijetsRoot account is missing"))?;
...
fn get_historical_metadata(version: Version) -> VerifyingRequest {
    ...
    let callback: RequestCallback = Box::new(move |ctxt, subresponses| {
        ...
        // NewBlockEvent can be special cased so we don't need to lookup the dijets_root::DijetsBlock->height
...
fn get_transactions(start_version: Version, limit: u64, include_events: bool) -> VerifyingRequest {
    ...
    let callback: RequestCallback = Box::new(move |ctxt, subresponses| {
        ...
        // Deserialize the dijets-types from the json-rpc-types view.
    ...
    let callback: RequestCallback = Box::new(move |ctxt, subresponses| {
        ...
        let event_views = event_with_proof_views
            ...
            .map(|(offset, event_with_proof_view)| {
                // Deserialize the dijets-core type from the json-rpc view type.
...
fn get_currencies() -> VerifyingRequest {
    ...
    let callback: RequestCallback = Box::new(|ctxt, subresponses| {
        let dijets_root = match subresponses {
            [MethodResponse::GetAccountStateWithProof(ref dijets_root)] => dijets_root,
            subresponses => {
                return Err(Error::rpc_response(format!(
                    "expected [GetAccountStateWithProof] subresponses, received: {:?}",
                    subresponses,
        ...
        let dijets_root = verify_account_state(ctxt, dijets_root, dijets_root_address(), None)?
            .ok_or_else(|| Error::rpc_response("DijetsRoot account is missing"))?;
        let currency_infos = dijets_root
