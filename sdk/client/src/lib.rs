...
cfg_async_or_blocking! {
    const USER_AGENT: &str = concat!("dijets-client-sdk-rust / ", env!("CARGO_PKG_VERSION"));
}
...
cfg_async_or_blocking! {
    fn validate(
        ...
        resp: &dijets_json_rpc_types::response::JsonRpcResponse,
        ignore_stale: bool,
    ...
    fn validate_batch(
        ...
        raw_responses: Vec<dijets_json_rpc_types::response::JsonRpcResponse>,
    ...
    fn get_id(resp: &dijets_json_rpc_types::response::JsonRpcResponse) -> Result<u64> {
    ...
    #[serde(untagged)]
    enum BatchResponse {
        Success(Vec<dijets_json_rpc_types::response::JsonRpcResponse>),
        Error(Box<dijets_json_rpc_types::response::JsonRpcResponse>),
    }

    impl BatchResponse {
        pub fn success(self) -> Result<Vec<dijets_json_rpc_types::response::JsonRpcResponse>> {
            match self {
