...
pub use dijets_transaction_builder::stdlib;
use dijets_types::transaction::Script;
...
impl TransactionFactory {
    ...
    pub fn with_dijets_version(mut self, dijets_version: u64) -> Self {
        self.dijets_version = dijets_version;
        self
    }
    ...
    pub fn update_dijets_version(&self, sliding_nonce: u64, major: u64) -> TransactionBuilder {
        if self.is_script_function_enabled() {
            self.payload(stdlib::encode_update_dijets_version_script_function(
                sliding_nonce,
                major,
            ))
        } else {
            self.script(stdlib::encode_update_dijets_version_script(
                sliding_nonce,
                major,
            ))
        }
...
impl DualAttestationMessage {
    pub fn new<M: Into<Vec<u8>>>(metadata: M, reciever: AccountAddress, amount: u64) -> Self {
        ...
        message.extend(b"@@$$DIJETS_ATTEST$$@@");
