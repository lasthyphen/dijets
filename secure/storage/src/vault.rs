...
use dijets_vault_client::Client;
...
use dijets_vault_client::ReadResponse;

const TRANSIT_NAMESPACE_SEPARATOR: &str = "__";

/// VaultStorage utilizes Vault for maintaining encrypted, authenticated data for Dijets. This
...
/// Version 2 (https://www.vaultproject.io/api/secret/kv/kv-v2.html). So while Dijets Secure Storage
...
impl VaultStorage {
        ...
        if self.renew_ttl_secs.is_some() {
            ...
            if now >= next_renewal {
                ...
                if let Ok(ttl) = result {
                    ...
                    dijets_logger::error!("Unable to renew lease: {}", e.to_string());
                }
            }
        }
        &self.client
    ...
    fn reset_crypto(&self) -> Result<(), Error> {
        let keys = match self.client().list_keys() {
            ...
            Err(dijets_vault_client::Error::NotFound(_, _)) => return Ok(()),
...
impl CryptoStorage for VaultStorage {
    ...
    fn sign<T: CryptoHash + Serialize>(
        ...
        let name = self.crypto_name(name);
        let mut bytes = <T::Hasher as dijets_crypto::hash::CryptoHasher>::seed().to_vec();
...
pub mod policy {
    ...
    use dijets_vault_client as vault;

    const DIJETS_DEFAULT: &str = "dijets_default";
        ...
        pub fn set_policies(
            ...
            for perm in &policy.permissions {
                match &perm.id {
                    ...
                    Identity::Anyone => {
                        self.set_policy(DIJETS_DEFAULT, engine, name, &perm.capabilities)?
                    }
                    Identity::NoOne => (),
                };
            }
