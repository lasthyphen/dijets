...
/// This defines a generic trait used to interact with the Dijets blockchain. In production, this
/// will be talking to a JSON-RPC service. For tests, this may be an executor and storage directly.
pub trait DijetsInterface {
...
/// This implements the DijetsInterface by talking directly to the JSON RPC API.
///
...
#[derive(Clone)]
pub struct JsonRpcDijetsInterface {
    client: BlockingClient,
}

impl JsonRpcDijetsInterface {
...
impl DijetsInterface for JsonRpcDijetsInterface {
    fn dijets_timestamp(&self) -> Result<u64, Error> {
        let account = account_config::dijets_root_address();
        let dijets_timestamp_resource = self
            .retrieve_account_state(account)?
            .get_dijets_timestamp_resource();

        match dijets_timestamp_resource {
            Ok(timestamp_resource) => timestamp_resource
                .map(|timestamp_resource| timestamp_resource.dijets_timestamp.microseconds)
                .ok_or_else(|| {
                    Error::DataDoesNotExist(format!(
                        "DijetsTimestampResource not found for account: {:?}",
                        account
                    ))
                }),
    ...
    fn submit_transaction(&self, transaction: Transaction) -> Result<(), Error> {
        if let Transaction::UserTransaction(signed_txn) = transaction {
            self.client
                .submit(&signed_txn)
                .map(dijets_client::Response::into_inner)
                .map_err(|e| {
