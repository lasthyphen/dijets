...
fn publish_metrics_test() {
    let counter_opts = Opts::new("dijets_test_counter", "dijets test counter help");
    ...
    assert_eq!("dijets test counter help", m.get_help());
    assert_eq!("dijets_test_counter", m.get_name());
