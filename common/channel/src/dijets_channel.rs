...
//! dijets_channel provides an mpsc channel which has two ends `dijets_channel::Receiver`
//! and `dijets_channel::Sender` similar to existing mpsc data structures.
...
use dijets_infallible::{Mutex, NonZeroUsize};
...
/// The sending end of the dijets_channel.
#[derive(Debug)]
pub struct Sender<K: Eq + Hash + Clone, M> {
    shared_state: Arc<Mutex<SharedState<K, M>>>,
}

/// The status of an element inserted into a dijets_channel. If the element is successfully
...
/// The receiving end of the dijets_channel.
...
/// Create a new Dijets Channel and returns the two ends of the channel.
pub fn new<K: Eq + Hash + Clone, M>(
    ...
    let max_queue_size_per_key =
        NonZeroUsize!(max_queue_size_per_key, "dijets_channel cannot be of size 0");
