...
//! [`IntGauge`](dijets_metrics::IntGauge) that counts the number of currently
//! queued items. While there is only one [`channel::Receiver`], there can be
//! many [`channel::Sender`]s, which are also cheap to clone.
//!
//! This channel differs from our other channel implementation, [`channel::dijets_channel`],
...
pub mod dijets_channel;
#[cfg(test)]
mod dijets_channel_test;
