...
/// A layer that translates tracing events into dijets-logger events.
pub struct TracingToDijetsLoggerLayer;
...
impl<S: tr::Subscriber> Layer<S> for TracingToDijetsLoggerLayer {
    fn on_event(&self, event: &tr::Event, _ctx: Context<S>) {
        let metadata = match translate_metadata(event.metadata()) {
            Some(metadata) => metadata,
            None => {
                dl::warn!(
                    "[tracing-to-dijets-logger] failed to translate event due to unknown level {:?}",
