...
/// A builder for a `DijetsLogger`, configures what, where, and how to write logs.
pub struct DijetsLoggerBuilder {
    channel_size: usize,
    level: Level,
    remote_level: Level,
...
impl DijetsLoggerBuilder {
    ...
    pub fn build(&mut self) -> Arc<DijetsLogger> {
        let filter = {
            let local_filter = {
        ...
        let logger = if self.is_async {
            ...
            let logger = Arc::new(DijetsLogger {
                sender: Some(sender),
                printer: None,
...
pub struct DijetsLogger {
    sender: Option<SyncSender<LoggerServiceEvent>>,
    printer: Option<Box<dyn Writer>>,
    filter: RwLock<DijetsFilter>,
    pub(crate) formatter: fn(&LogEntry) -> Result<String, fmt::Error>,
}

impl DijetsLogger {
    pub fn builder() -> DijetsLoggerBuilder {
        DijetsLoggerBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> DijetsLoggerBuilder {
        Self::builder()
    }
...
/// or to a `DijetsLogger` for external logging.
struct LoggerService {
    receiver: Receiver<LoggerServiceEvent>,
    address: Option<String>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<DijetsLogger>,
}
...
/// Example:
/// 2020-03-07 05:03:03 INFO [thread_name] common/dijets-logger/src/lib.rs:261 Hello { "world": true }
