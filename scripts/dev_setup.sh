...
# This script sets up the environment for the Dijets build by installing necessary dependencies.
#
...
SCCACHE_GIT='https://github.com/dijets/sccache.git@ef50d87a58260c30767520045e242ccdbdb965af'
...
function usage {
  echo "Usage:"
  echo "Installs or updates necessary dev tools for dijets/dijets."
  ...
  echo "This command must be called from the root folder of the Dijets project."
}
...
function install_allure {
    ...
    if [[ "$VERSION" != "${ALLURE_VERSION}" ]]; then
      if [[ "$PACKAGE_MANAGER" == "apt-get" ]]; then
        ...
        curl -sL -o "$ALLURE" "https://github.com/dijets/allure2/releases/download/${ALLURE_VERSION}/allure_${ALLURE_VERSION}-1_all.deb"
        dpkg -i "$ALLURE"
        rm "$ALLURE"
      elif [[ "$PACKAGE_MANAGER" == "apk" ]]; then
        apk --update add --no-cache  -X http://dl-cdn.alpinelinux.org/alpine/edge/community openjdk11
...
build, test and inspect Dijets Core.
...
if [ ! -f rust-toolchain ]; then
	echo "Unknown location. Please run this from the dijets repository. Abort."
	exit 1
fi
