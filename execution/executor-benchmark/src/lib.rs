...
use executor::{
    ...
    metrics::{
        DIJETS_EXECUTOR_COMMIT_BLOCKS_SECONDS, DIJETS_EXECUTOR_EXECUTE_BLOCK_SECONDS,
...
impl TransactionGenerator {
    ...
    fn new_impl(
        ...
        for _i in 0..num_accounts {
            ...
            let address = dijets_types::account_address::from_public_key(&public_key);
...
impl TransactionExecutor {
    pub fn new(executor: Executor<DijetsVM>, parent_block_id: HashValue) -> Self {
        Self {
            executor,
            parent_block_id,
            version: 0,
    ...
    fn report_block(
        ...
        info!(
            ...
            DIJETS_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum(),
            DIJETS_EXECUTOR_EXECUTE_BLOCK_SECONDS.get_sample_sum() - DIJETS_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum(),
            DIJETS_EXECUTOR_COMMIT_BLOCKS_SECONDS.get_sample_sum(),
        );
        const NANOS_PER_SEC: f64 = 1_000_000_000.0;
        info!(
            "Accumulative per transaction: VM time: {:.0} ns, executor time: {:.0} ns, commit time: {:.0} ns",
            DIJETS_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum() * NANOS_PER_SEC
                / version as f64,
            (DIJETS_EXECUTOR_EXECUTE_BLOCK_SECONDS.get_sample_sum() - DIJETS_EXECUTOR_VM_EXECUTE_BLOCK_SECONDS.get_sample_sum()) * NANOS_PER_SEC
                / version as f64,
            DIJETS_EXECUTOR_COMMIT_BLOCKS_SECONDS.get_sample_sum() * NANOS_PER_SEC
                / version as f64,
        );
    }
}
...
pub fn create_storage_service_and_executor(
    config: &NodeConfig,
) -> (Arc<dyn DbReader>, Executor<DijetsVM>) {
    ...
    let waypoint = generate_waypoint::<DijetsVM>(&db_rw, get_genesis_txn(config).unwrap()).unwrap();
    maybe_bootstrap::<DijetsVM>(&db_rw, get_genesis_txn(config).unwrap(), waypoint).unwrap();
...
pub fn run_benchmark(
    ...
    let (mut config, genesis_key) = dijets_genesis_tool::test_config();
...
fn create_transaction(
    ...
    let now = dijets_infallible::duration_since_epoch();
