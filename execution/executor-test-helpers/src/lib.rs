...
use dijets_vm::{DijetsVM, VMExecutor};
...
pub fn start_storage_service() -> (NodeConfig, JoinHandle<()>, Arc<dyn DbReader>) {
    let (mut config, _genesis_key) = dijets_genesis_tool::test_config();
    let server_port = utils::get_available_port();
    config.storage.address = SocketAddr::new(IpAddr::V4(Ipv4Addr::LOCALHOST), server_port);
    let (db, db_rw) = DbReaderWriter::wrap(DijetsDB::new_for_test(&config.storage.dir()));
    bootstrap_genesis::<DijetsVM>(&db_rw, utils::get_genesis_txn(&config).unwrap()).unwrap();
