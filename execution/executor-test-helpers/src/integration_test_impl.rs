...
pub fn test_execution_with_storage_impl() -> Arc<DijetsDB> {
    ...
    let (dijets_db, db, executor, waypoint) = create_db_and_executor(path.path(), &genesis_txn);

    let parent_block_id = executor.committed_block_id();
    let signer = dijets_types::validator_signer::ValidatorSigner::new(
...
pub fn create_db_and_executor<P: AsRef<std::path::Path>>(
    path: P,
    genesis: &Transaction,
) -> (Arc<DijetsDB>, DbReaderWriter, Executor<DijetsVM>, Waypoint) {
    let (db, dbrw) = DbReaderWriter::wrap(DijetsDB::new_for_test(&path));
    let waypoint = bootstrap_genesis::<DijetsVM>(&dbrw, genesis).unwrap();
    let executor = Executor::<DijetsVM>::new(dbrw.clone());
