...
use dijetsdb::{DijetsDB, GetRestoreHandler};
use executor::{
...
fn test_empty_db() {
    ...
    let db_rw = DbReaderWriter::new(DijetsDB::new_for_test(&tmp_dir));

    // Executor won't be able to boot on empty db due to lack of StartupInfo.
    assert!(db_rw.reader.get_startup_info().unwrap().is_none());

    // Bootstrap empty DB.
    let waypoint = generate_waypoint::<DijetsVM>(&db_rw, &genesis_txn).expect("Should not fail.");
    maybe_bootstrap::<DijetsVM>(&db_rw, &genesis_txn, waypoint).unwrap();
    ...
    assert!(!maybe_bootstrap::<DijetsVM>(&db_rw, &genesis_txn, waypoint).unwrap());
}
...
fn execute_and_commit(txns: Vec<Transaction>, db: &DbReaderWriter, signer: &ValidatorSigner) {
    ...
    let executor = Executor::<DijetsVM>::new(db.clone());
    let output = executor
...
fn get_mint_transaction(
    dijets_root_key: &Ed25519PrivateKey,
    dijets_root_seq_num: u64,
    account: &AccountAddress,
    amount: u64,
) -> Transaction {
    get_test_signed_transaction(
        testnet_dd_account_address(),
        /* sequence_number = */ dijets_root_seq_num,
        dijets_root_key.clone(),
        dijets_root_key.public_key(),
...
fn test_pre_genesis() {
    ...
    let tmp_dir = TempPath::new();
    let (db, db_rw) = DbReaderWriter::wrap(DijetsDB::new_for_test(&tmp_dir));
    let signer = ValidatorSigner::new(genesis.1[0].data.address, genesis.1[0].key.clone());
    let waypoint = bootstrap_genesis::<DijetsVM>(&db_rw, &genesis_txn).unwrap();
    ...
    assert!(maybe_bootstrap::<DijetsVM>(&db_rw, &genesis_txn, waypoint).is_err());
    ...
    let waypoint = generate_waypoint::<DijetsVM>(&db_rw, &genesis_txn).unwrap();
    assert!(maybe_bootstrap::<DijetsVM>(&db_rw, &genesis_txn, waypoint).unwrap());
...
fn test_new_genesis() {
    ...
    // Create bootstrapped DB.
    let tmp_dir = TempPath::new();
    let db = DbReaderWriter::new(DijetsDB::new_for_test(&tmp_dir));
    let waypoint = bootstrap_genesis::<DijetsVM>(&db, &genesis_txn).unwrap();
    let signer = ValidatorSigner::new(genesis.1[0].data.address, genesis.1[0].key.clone());
    ...
    let waypoint = generate_waypoint::<DijetsVM>(&db, &genesis_txn).unwrap();
    assert!(maybe_bootstrap::<DijetsVM>(&db, &genesis_txn, waypoint).unwrap());
