...
use dijets_types::{
    account_config::{dijets_root_address, treasury_compliance_account_address, xus_tag},
...
#[test]
fn test_genesis() {
    let path = dijets_temppath::TempPath::new();
    ...
    let dijets_root_account = db
        .reader
        .get_account_state_with_proof(dijets_root_address(), 0, 0)
        .unwrap();
    dijets_root_account
        .verify(li, 0, dijets_root_address())
        .unwrap();
}
...
fn test_reconfiguration() {
    ...
    let validator_account_state_with_proof = db
        ...
        .unwrap();
    let dijets_root_account_state_with_proof = db
        .reader
        .get_account_state_with_proof(dijets_root_address(), current_version, current_version)
        .unwrap();
    assert_eq!(
        AccountState::try_from(&dijets_root_account_state_with_proof.blob.unwrap())
            .unwrap()
            .get_validator_set()
            .unwrap()
            .unwrap()
    ...
    let dijets_root_account_state_with_proof = db
        ...
        .get_account_state_with_proof(dijets_root_address(), current_version, current_version)
        .unwrap();
    let blob = &dijets_root_account_state_with_proof.blob.unwrap();
    assert_eq!(
...
fn test_change_publishing_option_to_custom() {
    ...
    let genesis_account = dijets_root_address();
