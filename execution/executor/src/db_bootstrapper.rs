...
use dijets_crypto::{hash::PRE_GENESIS_BLOCK_ID, HashValue};
use dijets_logger::prelude::*;
use dijets_state_view::{StateView, StateViewId};
use dijets_types::{
...
fn get_state_timestamp(state_view: &VerifiedStateView) -> Result<u64> {
    let rsrc_bytes = &state_view
        .get(&AccessPath::new(
            dijets_root_address(),
            DijetsTimestampResource::resource_path(),
        ))?
        .ok_or_else(|| format_err!("DijetsTimestampResource missing."))?;
    let rsrc = bcs::from_bytes::<DijetsTimestampResource>(rsrc_bytes)?;
    Ok(rsrc.dijets_timestamp.microseconds)
}
