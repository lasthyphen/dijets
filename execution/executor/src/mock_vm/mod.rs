...
use dijets_types::{
    ...
    account_config::{dijets_root_address, validator_set_address, XUS_NAME},
    chain_id::ChainId,
...
// We use 10 as the assertion error code for insufficient balance within the Dijets coin contract.
        ...
        for txn in transactions {
            match decode_transaction(txn.as_signed_user_txn().unwrap()) {
                ...
                MockVMTransaction::Reconfiguration => {
                    ...
                    read_balance_from_storage(state_view, &balance_ap(dijets_root_address()));
                    outputs.push(TransactionOutput::new(
