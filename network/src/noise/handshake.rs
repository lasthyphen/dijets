...
use dijets_crypto::{noise, x25519};
use dijets_infallible::{duration_since_epoch, RwLock};
use dijets_logger::trace;
...
pub enum HandshakeAuthMode {
    ...
    ///
    /// For example, in the Dijets validator network, validator peers will only
                ...
                match trusted_peers.read().get(&remote_peer_id) {
                    ...
                    None => {
                        ...
                        let derived_remote_peer_id =
                            dijets_types::account_address::from_identity_public_key(
                                remote_public_key,
                            );
...
mod test {
    ...
    use dijets_config::config::{Peer, PeerRole};
    use dijets_crypto::{test_utils::TEST_SEED, traits::Uniform as _};
    ...
    fn build_peers(
        ...
        let (client_auth, server_auth, client_peer_id, server_peer_id) = if is_mutual_auth {
            ...
            let client_peer_id =
                dijets_types::account_address::from_identity_public_key(client_public_key);
            let server_peer_id =
                dijets_types::account_address::from_identity_public_key(server_public_key);
            (
