...
use dijets_config::{config::PeerRole, network_id::NetworkContext};
...
fn build_test_peer(
    ...
    dijets_channel::Receiver<ProtocolId, PeerNotification>,
) {
    ...
    let (peer_reqs_tx, peer_reqs_rx) =
        dijets_channel::new(QueueStyle::FIFO, NETWORK_CHANNEL_SIZE, None);
...
#[derive(Clone)]
struct PeerHandle(dijets_channel::Sender<ProtocolId, PeerRequest>);
