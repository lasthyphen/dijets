...
fn build_test_peer_manager(
    ...
    PeerManager<
        ...
        MemorySocket,
    >,
    dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    dijets_channel::Sender<PeerId, ConnectionRequest>,
    ...
    let (peer_manager_request_tx, peer_manager_request_rx) =
        dijets_channel::new(QueueStyle::FIFO, 1, None);
    let (connection_reqs_tx, connection_reqs_rx) = dijets_channel::new(QueueStyle::FIFO, 1, None);
    let (hello_tx, hello_rx) = dijets_channel::new(QueueStyle::FIFO, 1, None);
