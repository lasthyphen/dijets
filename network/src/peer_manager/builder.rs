...
use crate::{
    ...
    transport::{self, Connection, DijetsNetTransport, DIJETS_TCP_TRANSPORT},
    ProtocolId,
};
use channel::{self, dijets_channel, message_queues::QueueStyle};
use dijets_config::{
    config::{PeerSet, RateLimitConfig, HANDSHAKE_VERSION},
    network_id::NetworkContext,
};
use dijets_crypto::x25519;
use dijets_infallible::RwLock;
use dijets_logger::prelude::*;
use dijets_metrics::IntCounterVec;
use dijets_rate_limiter::rate_limit::TokenBucketRateLimiter;
use dijets_time_service::TimeService;
use dijets_types::{chain_id::ChainId, network_address::NetworkAddress, PeerId};
...
struct PeerManagerContext {
    ...
    pm_reqs_tx: dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
    pm_reqs_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    connection_reqs_tx: dijets_channel::Sender<PeerId, ConnectionRequest>,
    connection_reqs_rx: dijets_channel::Receiver<PeerId, ConnectionRequest>,

    trusted_peers: Arc<RwLock<PeerSet>>,
    upstream_handlers:
        HashMap<ProtocolId, dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>,
...
impl PeerManagerContext {
    fn new(
        ...
        upstream_handlers: HashMap<
            ProtocolId,
            dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>,
        >,
        connection_event_handlers: Vec<conn_notifs_channel::Sender>,
    ...
    fn add_upstream_handler(
        ...
        channel: dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>,
    ) -> &mut Self {
...
type MemoryPeerManager =
    PeerManager<DijetsNetTransport<MemoryTransport>, NoiseStream<memsocket::MemorySocket>>;
type TcpPeerManager = PeerManager<DijetsNetTransport<TcpTransport>, NoiseStream<TcpSocket>>;
...
impl PeerManagerBuilder {
    pub fn create(
        ...
        let (pm_reqs_tx, pm_reqs_rx) = dijets_channel::new(
            QueueStyle::FIFO,
            channel_size,
        ...
        let (connection_reqs_tx, connection_reqs_rx) =
            dijets_channel::new(QueueStyle::FIFO, channel_size, None);
    ...
    pub fn connection_reqs_tx(&self) -> dijets_channel::Sender<PeerId, ConnectionRequest> {
    ...
    pub fn build(&mut self, executor: &Handle) -> &mut Self {
        use dijets_types::network_address::Protocol::*;
        ...
        self.peer_manager = match self.listen_address.as_slice() {
            [Ip4(_), Tcp(_)] | [Ip6(_), Tcp(_)] => {
                Some(TransportPeerManager::Tcp(self.build_with_transport(
                    DijetsNetTransport::new(
                        DIJETS_TCP_TRANSPORT.clone(),
    ...
    pub fn add_protocol_handler(
        ...
        dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerNotification>,
        ...
        let (network_notifs_tx, network_notifs_rx) =
            dijets_channel::new(queue_preference, max_queue_size_per_peer, counter);
