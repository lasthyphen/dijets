...
use dijets_types::{network_address::NetworkAddress, PeerId};
use futures::{
    channel::oneshot,
...
use dijets_config::config::{PeerRole, PeerSet};
...
pub struct PeerManagerRequestSender {
    inner: dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>,
}
...
pub struct ConnectionRequestSender {
    inner: dijets_channel::Sender<PeerId, ConnectionRequest>,
}

impl PeerManagerRequestSender {
    /// Construct a new PeerManagerRequestSender with a raw channel::Sender
    pub fn new(inner: dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>) -> Self {
        Self { inner }
    }
...
impl ConnectionRequestSender {
    /// Construct a new ConnectionRequestSender with a raw dijets_channel::Sender
    pub fn new(inner: dijets_channel::Sender<PeerId, ConnectionRequest>) -> Self {
        Self { inner }
    }
    ...
    TSocket: AsyncRead + AsyncWrite,
{
    ...
    active_peers: HashMap<
        ...
        (
            ConnectionMetadata,
            dijets_channel::Sender<ProtocolId, PeerRequest>,
        ),
    >,
    /// Known trusted peers from discovery
    trusted_peers: Arc<RwLock<PeerSet>>,
    /// Channel to receive requests from other actors.
    requests_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    ...
    TSocket: transport::TSocket,
{
    ...
    fn handle_connection_event(&mut self, event: TransportNotification<TSocket>) {
        ...
        match event {
            ...
            TransportNotification::Disconnected(lost_conn_metadata, reason) => {
                // See: https://github.com/dijets/dijets/issues/3128#issuecomment-605351504 for
    ...
    fn add_peer(&mut self, connection: Connection<TSocket>) {
        ...
        let (peer_reqs_tx, peer_reqs_rx) = dijets_channel::new(
            QueueStyle::FIFO,
            self.channel_size,
            Some(&counters::PENDING_NETWORK_REQUESTS),
        );
        // TODO: Add label for peer.
        let (peer_notifs_tx, peer_notifs_rx) = dijets_channel::new(
            QueueStyle::FIFO,
            self.channel_size,
    ...
    fn spawn_peer_network_events_handler(
        &self,
        peer_id: PeerId,
        network_events: dijets_channel::Receiver<ProtocolId, PeerNotification>,
    ) {
        ...
        match inbound_event {
            PeerNotification::RecvMessage(msg) => {
                ...
                if let Some(handler) = upstream_handlers.get_mut(&protocol_id) {
                    // Send over dijets channel for fairness.
