...
//! Convenience Network API for Dijets
...
pub struct NetworkEvents<TMessage> {
    ...
    event_stream: Select<
        ...
        >,
        Map<
            dijets_channel::Receiver<PeerId, ConnectionNotification>,
...
pub trait NewNetworkEvents {
    fn new(
        peer_mgr_notifs_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerNotification>,
        connection_notifs_rx: dijets_channel::Receiver<PeerId, ConnectionNotification>,
    ) -> Self;
}
...
/// a thin wrapper on `dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerRequest>`,
