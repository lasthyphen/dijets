...
//! These should serialize as per the [DijetsNet Handshake v1 Specification].
//!
...
//!
//! [DijetsNet Handshake v1 Specification]: https://github.com/dijets/dijets/blob/main/specifications/network/handshake-v1.md

use anyhow::anyhow;
use dijets_config::network_id::NetworkId;
...
/// Enum representing different versions of the Dijets network protocol. These
...
/// An enum to list the possible errors during the dijets handshake negotiation
#[derive(Debug, Error)]
pub enum HandshakeError {
    #[error("dijets-handshake: the received message has a different chain id: {0}, expected: {1}")]
    InvalidChainId(ChainId, ChainId),
    #[error(
        "dijets-handshake: the received message has an different network id: {0}, expected: {1}"
    )]
    InvalidNetworkId(NetworkId, NetworkId),
    #[error("dijets-handshake: could not find an intersection of supported protocol with the peer")]
    NoCommonProtocols,
}
