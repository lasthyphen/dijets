...
use dijets_time_service::{MockTimeService, TimeService};
...
struct TestHarness {
    ...
    peer_mgr_reqs_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    peer_mgr_notifs_tx: dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>,
...
impl TestHarness {
    fn new_permissive(ping_failures_tolerated: u64) -> (Self, HealthChecker) {
        ::dijets_logger::Logger::init_for_testing();
        let mock_time = TimeService::mock();

        let (peer_mgr_reqs_tx, peer_mgr_reqs_rx) = dijets_channel::new(QueueStyle::FIFO, 1, None);
        let (connection_reqs_tx, connection_reqs_rx) = dijets_channel::new(QueueStyle::FIFO, 1, None);
        let (peer_mgr_notifs_tx, peer_mgr_notifs_rx) = dijets_channel::new(QueueStyle::FIFO, 1, None);
