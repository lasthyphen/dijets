...
use dijets_time_service::{timeout, TimeService, TimeServiceTrait};
use dijets_types::{
    chain_id::ChainId,
...
/// tcp::Transport with Dijets-specific configuration applied.
pub const DIJETS_TCP_TRANSPORT: tcp::TcpTransport = tcp::TcpTransport {
    // Use default options.
    ttl: None,
    // Use TCP_NODELAY for dijets tcp connections.
    nodelay: Some(true),
};
...
async fn upgrade_inbound<T: TSocket>(
    ...
    // try to negotiate common dijetsnet version and supported application protocols
...
/// The common DijetsNet Transport.
///
...
// ideas in this PR thread: https://github.com/dijets/dijets/pull/3478#issuecomment-617385633
pub struct DijetsNetTransport<TTransport> {
...
impl<TTransport> DijetsNetTransport<TTransport>
where
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    fn parse_dial_addr(
        ...
        // parse out the dijetsnet protocols (noise ik and handshake)
...
// If using `DijetsNetTransport` as a `Transport` trait, then all upgrade futures
...
impl<TTransport: Transport> Transport for DijetsNetTransport<TTransport>
where
