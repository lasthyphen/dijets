...
use dijets_crypto::{test_utils::TEST_SEED, traits::Uniform, x25519};
use dijets_infallible::RwLock;
use dijets_time_service::MockTimeService;
use dijets_types::{
    chain_id::ChainId,
...
fn setup<TTransport>(
    ...
    Runtime,
    MockTimeService,
    (PeerId, DijetsNetTransport<TTransport>),
    (PeerId, DijetsNetTransport<TTransport>),
    Arc<RwLock<PeerSet>>,
    SupportedProtocols,
)
    ...
    TTransport::Listener: Send + 'static,
{
    ...
    let (listener_peer_id, dialer_peer_id, listener_auth_mode, dialer_auth_mode, trusted_peers) =
        match auth {
            ...
            Auth::MaybeMutual => {
                let listener_peer_id = dijets_types::account_address::from_identity_public_key(
                    listener_key.public_key(),
                );
                let dialer_peer_id =
                    dijets_types::account_address::from_identity_public_key(dialer_key.public_key());
                let trusted_peers = build_trusted_peers(
                    dialer_peer_id,
                    &dialer_key,
                    PeerRole::Validator,
    ...
    let listener_transport = DijetsNetTransport::new(
    ...
    let dialer_transport = DijetsNetTransport::new(
        base_transport,
...
////////////////////////////////////////
// DijetsNetTransport<MemoryTransport> //
////////////////////////////////////////
...
/////////////////////////////////////
// DijetsNetTransport<TcpTransport> //
/////////////////////////////////////
