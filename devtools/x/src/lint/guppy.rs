...
use crate::config::{
    BannedDepsConfig, DirectDepDupsConfig, EnforcedAttributesConfig, MoveToDijetsDepsConfig,
    OverlayConfig,
};
use anyhow::anyhow;
use guppy::{
...
// Ensure that Move crates do not depend on Dijets crates.
#[derive(Debug)]
pub struct MoveCratesDontDependOnDijetsCrates<'cfg> {
    config: &'cfg MoveToDijetsDepsConfig,
}

impl<'cfg> MoveCratesDontDependOnDijetsCrates<'cfg> {
    pub fn new(config: &'cfg MoveToDijetsDepsConfig) -> Self {
        Self { config }
    }
}

impl<'cfg> Linter for MoveCratesDontDependOnDijetsCrates<'cfg> {
    fn name(&self) -> &'static str {
        "move-crates-dont-depend-on-dijets-crates"
    }
}

impl<'cfg> PackageLinter for MoveCratesDontDependOnDijetsCrates<'cfg> {
    fn run<'l>(
        &self,
        ctx: &PackageContext<'l>,
        out: &mut LintFormatter<'l, '_>,
        ...
        // Determine if a crate is considered a Move crate or Dijets crate.
        //
        // Current criteria:
        //   1. All crates outside language are considered Dijets crates.
        ...
        let is_move_crate = |crate_path: &str, crate_name: &str| {
            if crate_path.starts_with("language/") {
                if self.config.dijets_crates_in_language.contains(crate_name) {
                    return false;
                }
                return true;
            }
                    ...
                    out.write(
                        ...
                        format!(
                            "depending on non-move crate `{}`\n\
                                ...
                                If you are creating a new Dijets crate in language, you need to add its name to the \
                                dijets_crates_in_language list in x.toml to make the linter recognize it.",
                            dep_name
                        ),
                    );
                }
