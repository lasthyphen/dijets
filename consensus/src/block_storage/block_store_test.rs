...
use dijets_crypto::{HashValue, PrivateKey};
use dijets_types::{
...
proptest! {
    ...
    fn test_block_store_insert(
        ...
        let authors: HashSet<Author> = private_keys.iter().map(|private_key| dijets_types::account_address::from_public_key(&private_key.public_key())).collect();
