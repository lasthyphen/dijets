...
pub trait PersistentLivenessStorage: Send + Sync {
    ...
    /// Returns a handle of the dijetsdb.
    fn dijets_db(&self) -> Arc<dyn DbReader>;
}
...
impl RootMetadata {
    ...
    pub fn new_empty() -> Self {
        Self::new(0, *dijets_crypto::hash::ACCUMULATOR_PLACEHOLDER_HASH, vec![])
    }
}
...
/// The proxy we use to persist data in dijets db storage service via grpc.
...
impl StorageWriteProxy {
    pub fn new(config: &NodeConfig, dijets_db: Arc<dyn DbReader>) -> Self {
        let db = Arc::new(ConsensusDB::new(config.storage.dir()));
        StorageWriteProxy { db, dijets_db }
    }
}
