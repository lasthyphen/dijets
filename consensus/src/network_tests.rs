...
use dijets_infallible::{Mutex, RwLock};
use dijets_types::{block_info::BlockInfo, PeerId};
...
pub struct NetworkPlayground {
    ...
    node_consensus_txs: Arc<
        Mutex<HashMap<TwinId, dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>>,
    >,
...
impl NetworkPlayground {
    ...
    async fn start_node_outbound_handler(
        drop_config: Arc<RwLock<DropConfig>>,
        src_twin_id: TwinId,
        mut network_reqs_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
    ...
    pub fn add_node(
        ...
        consensus_tx: dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>,
        ...
        network_reqs_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
...
mod tests {
    ...
    use dijets_types::validator_verifier::random_validator_verifier;
    ...
    fn test_network_api() {
        ...
        for (peer_id, peer) in peers.iter().enumerate() {
            let (network_reqs_tx, network_reqs_rx) = dijets_channel::new(QueueStyle::FIFO, 8, None);
            let (connection_reqs_tx, _) = dijets_channel::new(QueueStyle::FIFO, 8, None);
            let (consensus_tx, consensus_rx) = dijets_channel::new(QueueStyle::FIFO, 8, None);
    ...
    fn test_bad_message() {
        let (mut peer_mgr_notifs_tx, peer_mgr_notifs_rx) =
            dijets_channel::new(QueueStyle::FIFO, 8, None);
