...
pub struct NetworkReceivers {
    ...
    pub consensus_messages: dijets_channel::Receiver<
        (AccountAddress, Discriminant<ConsensusMsg>),
        (AccountAddress, ConsensusMsg),
    >,
    pub block_retrieval: dijets_channel::Receiver<AccountAddress, IncomingBlockRetrievalRequest>,
}
...
pub struct NetworkTask {
    consensus_messages_tx: dijets_channel::Sender<
        (AccountAddress, Discriminant<ConsensusMsg>),
        (AccountAddress, ConsensusMsg),
    >,
    block_retrieval_tx: dijets_channel::Sender<AccountAddress, IncomingBlockRetrievalRequest>,
...
impl NetworkTask {
    ...
    pub fn new(
        ...
        let (consensus_messages_tx, consensus_messages) =
            dijets_channel::new(QueueStyle::LIFO, 1, Some(&counters::CONSENSUS_CHANNEL_MSGS));
        let (block_retrieval_tx, block_retrieval) = dijets_channel::new(
            QueueStyle::LIFO,
            1,
                ...
                Event::RpcRequest(peer_id, msg, callback) => match msg {
                    ConsensusMsg::BlockRetrievalRequest(request) => {
                        ...
                        if let Err(e) = self.block_retrieval_tx.push(peer_id, req_with_callback) {
                            warn!(error = ?e, "dijets channel closed");
                        }
                    }
                    _ => {
