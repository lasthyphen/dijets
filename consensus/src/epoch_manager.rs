...
use crate::{
    ...
    liveness::{
        leader_reputation::{ActiveInactiveHeuristic, DijetsDBBackend, LeaderReputation},
...
use channel::{dijets_channel, Sender};
use consensus_types::{
    common::{Author, Round},
    epoch_retrieval::EpochRetrievalRequest,
};
use dijets_config::config::{ConsensusConfig, ConsensusProposerType, NodeConfig};
use dijets_infallible::{duration_since_epoch, Mutex};
    ...
    fn create_proposer_election(
        ...
        match &self.config.proposer_type {
            ...
            ConsensusProposerType::LeaderReputation(heuristic_config) => {
                let backend = Box::new(DijetsDBBackend::new(proposers.len(), self.storage.dijets_db()));
