...
use dijets_types::block_metadata::{new_block_event_key, NewBlockEvent};
use std::{
    cmp::Ordering,
...
pub struct DijetsDBBackend {
    window_size: usize,
    dijets_db: Arc<dyn DbReader>,
    window: Mutex<Vec<(u64, NewBlockEvent)>>,
}

impl DijetsDBBackend {
    pub fn new(window_size: usize, dijets_db: Arc<dyn DbReader>) -> Self {
        Self {
            window_size,
            dijets_db,
    ...
    fn refresh_window(&self, target_round: Round) -> anyhow::Result<()> {
        ...
        let buffer = 10;
        let events = self.dijets_db.get_events(
            &new_block_event_key(),
            u64::max_value(),
            Order::Descending,
...
impl MetadataBackend for DijetsDBBackend {
    ...
    fn get_block_metadata(&self, target_round: Round) -> Vec<NewBlockEvent> {
        ...
        if !(known_round == target_round
            || known_version == self.dijets_db.get_latest_version().unwrap_or(0))
        {
