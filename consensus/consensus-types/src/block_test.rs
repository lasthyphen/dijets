...
use dijets_types::{validator_signer::ValidatorSigner, validator_verifier::ValidatorVerifier};
...
fn test_same_qc_different_authors() {
    ...
    let round = 1;
    let payload = vec![];
    let current_timestamp = dijets_infallible::duration_since_epoch().as_micros() as u64;
