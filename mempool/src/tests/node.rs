...
use dijets_infallible::{Mutex, MutexGuard, RwLock};
use dijets_types::{account_address::AccountAddress, transaction::GovernanceRole, PeerId};
...
pub struct NodeNetworkInterface {
    ...
    pub(crate) network_reqs_rx: dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>,
...
fn setup_node_network_interface(
    ...
    let (network_reqs_tx, network_reqs_rx) =
        dijets_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
    let (connection_reqs_tx, _) = dijets_channel::new(QueueStyle::FIFO, MAX_QUEUE_SIZE, None);
...
fn start_node_mempool(
    ...
    let (_reconfig_events, reconfig_events_receiver) = dijets_channel::new(QueueStyle::LIFO, 1, None);
