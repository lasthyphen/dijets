...
impl MockSharedMempool {
    ...
    pub fn new(state_sync: Option<mpsc::Receiver<CommitNotification>>) -> Self {
        ...
        let (network_reqs_tx, _network_reqs_rx) = dijets_channel::new(QueueStyle::FIFO, 8, None);
        let (connection_reqs_tx, _) = dijets_channel::new(QueueStyle::FIFO, 8, None);
        let (_network_notifs_tx, network_notifs_rx) = dijets_channel::new(QueueStyle::FIFO, 8, None);
