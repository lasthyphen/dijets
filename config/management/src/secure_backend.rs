...
use dijets_config::config::{self, GitHubConfig, OnDiskStorageConfig, Token, VaultConfig};
use std::{
    collections::HashMap,
...
mod tests {
    ...
    fn test_github() {
        ...
        let github = format!(
            "backend=github;repository_owner=dijets;repository=dijets;token={}",
            path_str
        );
        storage(&github).unwrap();

        let github = format!(
            "backend=github;repository_owner=dijets;repository=dijets;token={};namespace=test",
            path_str
        );
        storage(&github).unwrap();

        let github = format!(
            "backend=github;repository_owner=dijets;repository=dijets;branch=genesis;token={};namespace=test",
            path_str
        );
        storage(&github).unwrap();
