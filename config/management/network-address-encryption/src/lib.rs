...
use dijets_global_constants::VALIDATOR_NETWORK_ADDRESS_KEYS;
use dijets_infallible::RwLock;
use dijets_secure_storage::{Error as StorageError, KVStorage, Storage};
use dijets_types::{
...
impl Encryptor<Storage> {
    ...
    pub fn empty() -> Self {
        let storage = Storage::InMemoryStorage(dijets_secure_storage::InMemoryStorage::new());
...
struct StorageKey(
    #[serde(
        serialize_with = "dijets_secure_storage::to_base64",
...
mod tests {
    use super::*;
    use dijets_secure_storage::{InMemoryStorage, Namespaced};
    ...
    fn initializer() {
        let storage = Storage::from(Namespaced::new(
            ...
            Box::new(Storage::VaultStorage(
                dijets_secure_storage::VaultStorage::new(
