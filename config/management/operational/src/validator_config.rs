...
use dijets_crypto::{ed25519::Ed25519PublicKey, x25519};
use dijets_global_constants::{
    CONSENSUS_KEY, FULLNODE_NETWORK_KEY, OPERATOR_ACCOUNT, OWNER_ACCOUNT, VALIDATOR_NETWORK_KEY,
};
use dijets_management::{error::Error, secure_backend::ValidatorBackend, storage::to_x25519};
...
pub struct ValidatorConfig {
    ...
    account_address: AccountAddress,
    #[structopt(flatten)]
    config: dijets_management::config::ConfigPath,
    /// JSON-RPC Endpoint (e.g. http://localhost:8080)
    #[structopt(long, required_unless = "config")]
    json_server: Option<String>,
    #[structopt(
...
impl DecryptedValidatorConfig {
    pub fn from_validator_config_resource(
        config_resource: &dijets_types::validator_config::ValidatorConfigResource,
        account_address: AccountAddress,
    ...
    pub fn from_validator_config(
        config: &dijets_types::validator_config::ValidatorConfig,
        account_address: AccountAddress,
        encryptor: &Encryptor<Storage>,
    ) -> Result<Self, Error> {
