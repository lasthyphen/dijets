...
use dijets_global_constants::DIJETS_ROOT_KEY;
use dijets_management::{
...
impl CreateAccount {
    fn execute(
        ...
        let mut transaction_context =
            build_and_submit_dijets_root_transaction(&config, seq_num, script, action)?;
...
struct RootValidatorOperation {
    ...
    json_server: Option<String>,
    #[structopt(flatten)]
    validator_config: dijets_management::validator_config::ValidatorConfig,
    #[structopt(flatten)]
    auto_validate: AutoValidate,
}
...
impl AddValidator {
    pub fn execute(self) -> Result<TransactionContext, Error> {
        ...
        .into_script_function();
        let mut transaction_context =
            build_and_submit_dijets_root_transaction(&config, seq_num, script, "add-validator")?;
...
impl RemoveValidator {
    pub fn execute(self) -> Result<TransactionContext, Error> {
        ...
        let mut transaction_context =
            build_and_submit_dijets_root_transaction(&config, seq_num, script, "remove-validator")?;
...
fn build_and_submit_dijets_root_transaction(
    config: &Config,
    seq_num: u64,
    ...
    let signed_txn = storage.sign(DIJETS_ROOT_KEY, action, txn)?;
