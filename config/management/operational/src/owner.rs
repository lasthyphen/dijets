...
use dijets_types::{account_address::AccountAddress, chain_id::ChainId};
...
impl SetValidatorOperator {
    pub fn execute(self) -> Result<TransactionContext, Error> {
        ...
        let owner_address = storage.account_address(dijets_global_constants::OWNER_ACCOUNT)?;
        ...
        let signed_txn = storage.sign(dijets_global_constants::OWNER_KEY, "set-operator", txn)?;
