...
fn compute_genesis(
    ...
    let dijetsdb = DijetsDB::open(db_path, false, None, RocksdbConfig::default())
    ...
    let waypoint = db_bootstrapper::generate_waypoint::<DijetsVM>(&db_rw, &genesis)
        .map_err(|e| Error::UnexpectedError(e.to_string()))?;
    db_bootstrapper::maybe_bootstrap::<DijetsVM>(&db_rw, &genesis, waypoint)
