...
use dijets_secure_storage::{CryptoStorage, KVStorage, Storage};
...
impl BuildSwarm for FullnodeBuilder {
        ...
        R: ::rand::RngCore + ::rand::CryptoRng,
    {
        ...
        let dijets_root_key_path = generate_key::load_key(&self.dijets_root_key_path);
        Ok((configs, dijets_root_key_path))
    }
}
...
pub fn test_config() -> (NodeConfig, Ed25519PrivateKey) {
    ...
    let mut test = dijets_config::config::TestConfig::new_with_temp_dir(Some(path));
    test.execution_key(
        storage
            .export_private_key(dijets_global_constants::EXECUTION_KEY)
            .unwrap(),
    );
    test.operator_key(
        storage
            .export_private_key(dijets_global_constants::OPERATOR_KEY)
            .unwrap(),
    );
    test.owner_key(
        storage
            .export_private_key(dijets_global_constants::OWNER_KEY)
            .unwrap(),
    );
    config.test = Some(test);

    let owner_account = storage
        .get(dijets_global_constants::OWNER_ACCOUNT)
        .unwrap()
        .value;
    let mut sr_test = dijets_config::config::SafetyRulesTestConfig::new(owner_account);
    sr_test.consensus_key(
        storage
            .export_private_key(dijets_global_constants::CONSENSUS_KEY)
            .unwrap(),
    );
