...
use dijets_management::{error::Error, execute_command};
use dijets_types::{transaction::Transaction, waypoint::Waypoint};
...
pub enum Command {
    ...
    InsertWaypoint(dijets_management::waypoint::InsertWaypoint),
    #[structopt(about = "Submits an Ed25519PublicKey for the dijets root")]
    DijetsRootKey(crate::key::DijetsRootKey),
...
impl From<&Command> for CommandName {
    fn from(command: &Command) -> Self {
        match command {
            ...
            Command::DijetsRootKey(_) => CommandName::DijetsRootKey,
...
impl std::fmt::Display for CommandName {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        let name = match self {
            ...
            CommandName::DijetsRootKey => "dijets-root-key",
...
impl Command {
    pub fn execute(self) -> Result<String, Error> {
        match &self {
            ...
            Command::DijetsRootKey(_) => self.dijets_root_key().map(|_| "Success!".to_string()),
    ...
    pub fn dijets_root_key(self) -> Result<Ed25519PublicKey, Error> {
        execute_command!(self, Command::DijetsRootKey, CommandName::DijetsRootKey)
    }
...
pub mod tests {
    ...
    use dijets_global_constants::{OPERATOR_KEY, OWNER_KEY};
    use dijets_management::constants;
    use dijets_secure_storage::KVStorage;
    use dijets_types::{account_address, chain_id::ChainId, transaction::TransactionPayload};
    use std::{
        fs::File,
        io::{Read, Write},
    };
    ...
    fn test_end_to_end() {
        ...
        let layout_text = "\
            ...
            dijets_root = \"dave_shared\"\n\
            treasury_compliance = \"dave_shared\"\n\
        ";

        let temppath = dijets_temppath::TempPath::new();
        ...
        let tempdir = dijets_temppath::TempPath::new();
        tempdir.create_as_dir().unwrap();
        for b in dijets_framework_releases::current_module_blobs() {
            let mut temppath =
                dijets_temppath::TempPath::new_with_temp_dir(tempdir.path().to_path_buf());
            temppath.create_as_file().unwrap();
        ...
        helper
            .dijets_root_key(dave_ns, &(dave_ns.to_string() + shared))
            .unwrap();
        helper
        ...
        let genesis_path = dijets_temppath::TempPath::new();
    ...
    fn test_validator_config() {
        ...
        let owner_account =
            dijets_config::utils::validator_owner_account_from_name(owner_name.as_bytes());
