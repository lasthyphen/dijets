...
use dijets_global_constants::{
    CONSENSUS_KEY, DIJETS_ROOT_KEY, EXECUTION_KEY, FULLNODE_NETWORK_KEY, OPERATOR_KEY, OWNER_KEY,
    SAFETY_DATA, TREASURY_COMPLIANCE_KEY, VALIDATOR_NETWORK_KEY, WAYPOINT,
};
use dijets_management::{error::Error, secure_backend::DISK};
use dijets_secure_storage::{CryptoStorage, KVStorage, Namespaced, OnDiskStorage, Storage};
use dijets_types::{
    chain_id::ChainId,
...
pub struct StorageHelper {
    temppath: dijets_temppath::TempPath,
}
...
impl StorageHelper {
    ...
    pub fn initialize(&self, namespace: String, seed: [u8; 32]) {
        ...
        storage
            .import_private_key(DIJETS_ROOT_KEY, Ed25519PrivateKey::generate(&mut rng))
            .unwrap();
        // TODO(davidiw) use distinct keys in tests for treasury and dijets root keys
        let dijets_root_key = storage.export_private_key(DIJETS_ROOT_KEY).unwrap();
        storage
            .import_private_key(TREASURY_COMPLIANCE_KEY, dijets_root_key)
            .unwrap();
        storage
        ...
        let mut encryptor = dijets_network_address_encryption::Encryptor::new(storage);
