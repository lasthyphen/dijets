...
dijets_management::secure_backend!(
    ValidatorBackend,
    validator_backend,
...
impl Key {
    fn submit_key(
        ...
        let key = if let Some(path_to_key) = &self.path_to_key {
            dijets_management::read_key_from_file(path_to_key)
            ...
            if let Some(account_name) = account_name {
                let peer_id = dijets_types::account_address::from_public_key(&key);
...
impl DijetsRootKey {
    pub fn execute(self) -> Result<Ed25519PublicKey, Error> {
        self.key
            .submit_key(dijets_global_constants::DIJETS_ROOT_KEY, None)
    }
}
...
impl OperatorKey {
    pub fn execute(self) -> Result<Ed25519PublicKey, Error> {
        self.key.submit_key(
            dijets_global_constants::OPERATOR_KEY,
            Some(dijets_global_constants::OPERATOR_ACCOUNT),
        )
    }
}
...
impl OwnerKey {
    pub fn execute(self) -> Result<Ed25519PublicKey, Error> {
        self.key.submit_key(dijets_global_constants::OWNER_KEY, None)
    }
}
...
impl TreasuryComplianceKey {
    pub fn execute(self) -> Result<Ed25519PublicKey, Error> {
        self.key
            .submit_key(dijets_global_constants::TREASURY_COMPLIANCE_KEY, None)
    }
}
...
mod tests {
    ...
    #[test]
    fn test_owner_key() {
        test_key(dijets_global_constants::OWNER_KEY, StorageHelper::owner_key);
    }
