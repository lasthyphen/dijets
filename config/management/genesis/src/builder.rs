...
use dijets_global_constants::{
    DIJETS_ROOT_KEY, MOVE_MODULES, OPERATOR_KEY, OWNER_KEY, TREASURY_COMPLIANCE_KEY,
};
use dijets_management::constants::{self, VALIDATOR_CONFIG, VALIDATOR_OPERATOR};
use dijets_secure_storage::{KVStorage, Namespaced};
use dijets_types::{
    chain_id::ChainId,
...
impl<S: KVStorage> GenesisBuilder<S> {
    ...
    pub fn set_root_key(&mut self, root_key: Ed25519PublicKey) -> Result<()> {
        ...
        self.with_namespace_mut(&layout.dijets_root)
            .set(DIJETS_ROOT_KEY, root_key)
            .map_err(Into::into)
    }

    pub fn root_key(&self) -> Result<Ed25519PublicKey> {
        let layout = self.layout()?;
        self.with_namespace(&layout.dijets_root)
            .get(DIJETS_ROOT_KEY)
            .map(|r| r.value)
            .map_err(Into::into)
    }
    ...
    pub fn validators(&self) -> Result<Vec<Validator>> {
        ...
        for owner in &layout.owners {
            ...
            let address = dijets_config::utils::default_validator_owner_auth_key_from_name(&name)
                .derived_address();
            let auth_key = self
                .owner_key(owner)
    ...
    pub fn build(
        ...
        let dijets_root_key = self.root_key()?;
