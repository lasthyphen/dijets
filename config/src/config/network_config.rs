...
use dijets_crypto::{x25519, Uniform};
...
impl NetworkConfig {
    ...
    fn prepare_identity(&mut self) {
        match &mut self.identity {
            ...
            Identity::None => {
                ...
                let peer_id =
                    dijets_types::account_address::from_identity_public_key(key.public_key());
                self.identity = Identity::from_config(key, peer_id);
            }
            Identity::FromConfig(config) => {
                let peer_id =
                    dijets_types::account_address::from_identity_public_key(config.key.public_key());
    ...
    pub fn verify_seeds(&self) -> Result<(), Error> {
        ...
        for (peer_id, seed) in self.seeds.iter() {
            ...
            // Require there to be a pubkey somewhere, either in the address (assumed by `is_dijetsnet_addr`)
            crate::config::invariant(
