...
use dijets_crypto::PrivateKey;
use serde::{de::DeserializeOwned, Deserialize, Serialize};

/// ConfigKey places a clonable wrapper around PrivateKeys for config purposes only. The only time
/// configs have keys is either for testing or for low security requirements. Dijets recommends that
...
impl<T: DeserializeOwned + PrivateKey + Serialize> ConfigKey<T> {
    ...
    pub fn public_key(&self) -> T::PublicKeyMaterial {
        dijets_crypto::PrivateKey::public_key(&self.key)
    }
}
...
#[cfg(test)]
impl<T: PrivateKey + Serialize + dijets_crypto::Uniform> Default for ConfigKey<T> {
    fn default() -> Self {
        Self {
            key: dijets_crypto::Uniform::generate_for_testing(),
        }
    }
}
