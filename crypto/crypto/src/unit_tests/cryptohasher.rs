...
use crate as dijets_crypto;
use crate::{
    hash::{CryptoHash, CryptoHasher, DIJETS_HASH_PREFIX},
    HashValue,
};
...
fn test_cryptohasher_name() {
    let mut salt = DIJETS_HASH_PREFIX.to_vec();
...
fn prefixed_sha3(input: &[u8]) -> [u8; 32] {
    ...
    let salt: Vec<u8> = [DIJETS_HASH_PREFIX, input].concat();
    sha3.update(&salt);
