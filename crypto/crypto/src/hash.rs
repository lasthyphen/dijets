...
//!    creates a collision.
//!
//! Regarding (1), this library makes it easy for Dijets developers to create as
//! many new "hashable" Rust types as needed so that each Rust type hashed and signed
//! in Dijets has a unique meaning, that is, unambiguously captures the intent of a signer.
//!
...
//! use the derive macros of `serde` and `dijets_crypto_derive` as follows:
//! ```
//! use dijets_crypto::hash::CryptoHash;
//! use dijets_crypto_derive::{CryptoHasher, BCSCryptoHash};
...
//!
//! ```
//! use dijets_crypto_derive::CryptoHasher;
...
//!
//! ```
//! use dijets_crypto::hash::{CryptoHasher, TestOnlyHasher};
//!
...
/// A prefix used to begin the salt of every dijets hashable structure. The salt
/// consists in this global prefix, concatenated with the specified
/// serialization name of the struct.
pub(crate) const DIJETS_HASH_PREFIX: &[u8] = b"DIJETS::";
...
impl DefaultHasher {
    ...
    pub fn prefixed_hash(buffer: &[u8]) -> [u8; HashValue::LENGTH] {
        ...
        let salt: Vec<u8> = [DIJETS_HASH_PREFIX, buffer].concat();
...
/// # Example
/// ```
/// use dijets_crypto::hash::TestOnlyHash;
///
