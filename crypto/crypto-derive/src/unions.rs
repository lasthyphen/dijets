...
pub fn impl_enum_tryfrom(name: &Ident, variants: &DataEnum) -> proc_macro2::TokenStream {
    ...
    quote! {
        impl core::convert::TryFrom<&[u8]> for #name {
            type Error = dijets_crypto::CryptoMaterialError;
...
pub fn impl_enum_publickey(
    ...
    res.extend(quote! {
        impl dijets_crypto::PublicKey for #name {
...
pub fn impl_enum_signingkey(
    ...
    let res = quote! {
        impl dijets_crypto::SigningKey for #name {
            type VerifyingKeyMaterial = #pkt;
            type SignatureMaterial = #st;

            fn sign<T: dijets_crypto::hash::CryptoHash + serde::Serialize>(&self, message: &T) -> Self::SignatureMaterial {
                match self {
                    #match_struct_arms
                }
            }
...
pub fn impl_enum_signature(
    ...
    res.extend(quote! {
        ...
        impl dijets_crypto::Signature for #name {
            type VerifyingKeyMaterial = #pub_kt;
            type SigningKeyMaterial = #priv_kt;

            fn verify<T: dijets_crypto::hash::CryptoHash + serde::Serialize>(&self, message: &T, public_key: &Self::VerifyingKeyMaterial) -> std::result::Result<(), dijets_crypto::error::Error> {
                match (self, public_key) {
                    #match_struct_arms
                    _ => dijets_crypto::error::bail!(
            ...
            fn verify_arbitrary_msg(&self, message: &[u8], public_key: &Self::VerifyingKeyMaterial) -> std::result::Result<(), dijets_crypto::error::Error> {
                match (self, public_key) {
                    #match_arms
                    _ => dijets_crypto::error::bail!(
                        "provided the wrong alternative in {:?}!",
