...
use dijets_infallible::duration_since_epoch;
use dijets_jellyfish_merkle::{restore::JellyfishMerkleRestore, NodeBatch, TreeWriter};
use dijets_types::{account_state_blob::AccountStateBlob, transaction::Version, waypoint::Waypoint};
use dijetsdb::{backup::restore_handler::RestoreHandler, DijetsDB, GetRestoreHandler};
use std::{
    collections::HashMap,
    convert::TryFrom,
    mem::size_of,
...
impl TryFrom<GlobalRestoreOpt> for GlobalRestoreOptions {
    ...
    fn try_from(opt: GlobalRestoreOpt) -> Result<Self> {
        ...
        let run_mode = if let Some(db_dir) = &opt.db_dir {
            let restore_handler = Arc::new(DijetsDB::open(
                db_dir,
                false, /* read_only */
                None,  /* pruner */
