...
impl TransactionRestoreController {
    ...
    fn transaction_replayer(&mut self, first_version: Version) -> Result<&mut Executor<DijetsVM>> {
        if self.state.transaction_replayer.is_none() {
            if let RestoreRunMode::Restore { restore_handler } = self.run_mode.as_ref() {
                let replayer = Executor::new_on_unbootstrapped_db(
                    DbReaderWriter::from_arc(Arc::clone(&restore_handler.dijetsdb)),
...
struct State {
    ...
    transaction_replayer: Option<Executor<DijetsVM>>,
}
