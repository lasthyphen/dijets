...
use dijetsdb::backup::backup_handler::DbState;
...
impl BackupCoordinator {
    ...
    pub async fn run(&self) -> Result<()> {
        // Connect to both the local Dijets node and the backup storage.
...
impl BackupCoordinator {
    async fn try_refresh_db_state(&self, db_state_broadcast: &watch::Sender<Option<DbState>>) {
        match self.client.get_db_state().await {
            ...
            Err(e) => warn!(
                "Failed pulling DbState from local Dijets node: {}. Will keep trying.",
                e
            ),
        };
    }
