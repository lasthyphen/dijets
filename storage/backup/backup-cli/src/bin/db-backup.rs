...
use dijets_logger::{prelude::*, Level, Logger};
use dijets_secure_push_metrics::MetricsPusher;

#[derive(StructOpt)]
#[structopt(about = "Dijets backup tool.")]
enum Command {
...
#[derive(StructOpt)]
enum OneShotCommand {
    #[structopt(about = "Query the backup service builtin in the local Dijets node.")]
...
enum OneShotQueryType {
    #[structopt(
        about = "Queries the latest epoch, committed version and synced version of the local Dijets \
