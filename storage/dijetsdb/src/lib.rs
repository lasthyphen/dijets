...
//! This crate provides [`DijetsDB`] which represents physical storage of the core Dijets data
//! structures.
//!
//! It relays read/write operations on the physical storage via [`schemadb`] to the underlying
//! Key-Value storage system, and implements dijets data structures on top of it.

#[cfg(any(feature = "dijetssum"))]
pub mod dijetssum;
...
#[cfg(feature = "fuzzing")]
pub use dijetsdb_test::test_save_blocks_impl;

use crate::{
    backup::{backup_handler::BackupHandler, restore_handler::RestoreHandler},
    change_set::{ChangeSet, SealedChangeSet},
    errors::DijetsDbError,
    event_store::EventStore,
    ledger_counters::LedgerCounters,
    ledger_store::LedgerStore,
    metrics::{
        DIJETS_STORAGE_API_LATENCY_SECONDS, DIJETS_STORAGE_COMMITTED_TXNS,
        DIJETS_STORAGE_LATEST_TXN_VERSION, DIJETS_STORAGE_LEDGER_VERSION,
        DIJETS_STORAGE_NEXT_BLOCK_EPOCH, DIJETS_STORAGE_OTHER_TIMERS_SECONDS,
        DIJETS_STORAGE_ROCKSDB_PROPERTIES,
    },
    pruner::Pruner,
    schema::*,
...
use dijets_crypto::hash::{CryptoHash, HashValue, SPARSE_MERKLE_PLACEHOLDER_HASH};
...
static ROCKSDB_PROPERTY_MAP: Lazy<HashMap<&str, &str>> = Lazy::new(|| {
    [
        (
            "dijets_rocksdb_live_sst_files_size_bytes",
            "rocksdb.live-sst-files-size",
        ),
        (
            "dijets_rocksdb_all_memtables_size_bytes",
            "rocksdb.size-all-mem-tables",
        ),
        (
            "dijets_rocksdb_num_running_compactions",
            "rocksdb.num-running-compactions",
        ),
        (
            "dijets_rocksdb_num_running_flushes",
            "rocksdb.num-running-flushes",
        ),
        (
            "dijets_rocksdb_block_cache_usage_bytes",
            "rocksdb.block-cache-usage",
        ),
        (
            "dijets_rocksdb_cf_size_bytes",
...
fn error_if_too_many_requested(num_requested: u64, max_allowed: u64) -> Result<()> {
    if num_requested > max_allowed {
        Err(DijetsDbError::TooManyRequested(num_requested, max_allowed).into())
    } else {
        Ok(())
    }
}
...
fn update_rocksdb_properties(db: &DB) -> Result<()> {
    let _timer = DIJETS_STORAGE_OTHER_TIMERS_SECONDS
        .with_label_values(&["update_rocksdb_properties"])
        .start_timer();
    for cf_name in DijetsDB::column_families() {
...
/// access to the core Dijets data structures.
#[derive(Debug)]
pub struct DijetsDB {
    db: Arc<DB>,
...
impl DijetsDB {
    ...
    pub fn open<P: AsRef<Path> + Clone>(
        ...
        let path = db_root_path.as_ref().join("dijetsdb");
    ...
    pub fn open_as_secondary<P: AsRef<Path> + Clone>(
        ...
        let primary_path = db_root_path.as_ref().join("dijetsdb");
    ...
    pub fn new_for_test<P: AsRef<Path> + Clone>(db_root_path: P) -> Self {
        ...
        .expect("Unable to open DijetsDB")
    }
...
impl ModuleResolver for DijetsDB {
...
impl ResourceResolver for DijetsDB {
    type Error = anyhow::Error;
...
impl MoveDbReader for DijetsDB {}

impl DbWriter for DijetsDB {
    ...
    fn save_transactions(
        ...
        gauged_api("save_transactions", || {
            ...
            if let Some(x) = ledger_info_with_sigs {
                ...
                DIJETS_STORAGE_LEDGER_VERSION.set(x.ledger_info().version() as i64);
                DIJETS_STORAGE_NEXT_BLOCK_EPOCH.set(x.ledger_info().next_block_epoch() as i64);
            }
            ...
            if num_txns > 0 {
                ...
                DIJETS_STORAGE_COMMITTED_TXNS.inc_by(num_txns);
                DIJETS_STORAGE_LATEST_TXN_VERSION.set(last_version as i64);
                counters
...
impl GetRestoreHandler for Arc<DijetsDB> {
    fn get_restore_handler(&self) -> RestoreHandler {
        RestoreHandler::new(
            Arc::clone(&self.db),
            Arc::clone(self),
    ...
    let res_type = match &res {
        ...
        Err(e) => {
            warn!(
                api_name = api_name,
                error = ?e,
                "DijetsDB API returned error."
            );
            "Err"
        }
    };
    DIJETS_STORAGE_API_LATENCY_SECONDS
