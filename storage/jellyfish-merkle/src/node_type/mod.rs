...
use crate::metrics::{DIJETS_JELLYFISH_INTERNAL_ENCODED_BYTES, DIJETS_JELLYFISH_LEAF_ENCODED_BYTES};
    ...
    pub fn encode(&self) -> Result<Vec<u8>> {
        ...
        match self {
            ...
            Node::Internal(internal_node) => {
                ...
                DIJETS_JELLYFISH_INTERNAL_ENCODED_BYTES.inc_by(out.len() as u64);
            }
            Node::Leaf(leaf_node) => {
                out.push(NodeTag::Leaf as u8);
                out.extend(bcs::to_bytes(&leaf_node)?);
                DIJETS_JELLYFISH_LEAF_ENCODED_BYTES.inc_by(out.len() as u64);
            }
        }
        Ok(out)
    }
