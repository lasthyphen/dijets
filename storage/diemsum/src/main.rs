...
use dijets_types::{account_address::AccountAddress, event::EventKey};
use dijetsdb::dijetssum::Dijetssum;
use serde::Serialize;
...
#[structopt(
    name = "Dijetssum",
    about = "A command line tool that offers multiple data access commands for DijetsDB"
)]
struct Opt {
    /// The parent dir of dijetsdb
...
fn run_cmd() -> Result<()> {
    ...
    let dijetssum = Dijetssum::new(opt.db_dir)?;
    let is_json = opt.json;
    match opt.cmd {
        Command::Status => {
            print(dijetssum.get_startup_info()?, is_json)?;
        }
        Command::Txn(txn_op) => match txn_op {
            TxnOp::Get { version } => {
                print(&dijetssum.get_txn_by_version(version)?, is_json)?;
            }
            TxnOp::Scan {
                from_version,
                to_version,
                ...
                let to_version = match to_version {
                    ...
                    None => dijetssum.get_committed_version()?,
                };
                print(
                    dijetssum.scan_txn_by_version(from_version, to_version)?,
                    is_json,
                )?;
            }
        },
        Command::Account(account_cmd) => {
            ...
            match account_cmd.account_op {
                AccountOp::Get { version } => {
                    print(
                        dijetssum.get_account_state_by_version(address, version)?,
                        is_json,
                    )?;
                }
            }
        ...
        Command::Event(event_cmd) => match event_cmd {
            EventCmd::EventByKey {
                ...
                EventOp::Get { seq } => {
                    print(dijetssum.scan_events_by_seq(&event_key, seq, seq)?, is_json)?;
                }
                EventOp::Scan { from_seq, to_seq } => {
                    ...
                    print(
                        dijetssum.scan_events_by_seq(&event_key, from_seq, to_seq)?,
                        is_json,
                    )?;
                }
            },
            EventCmd::EventByVersion { version } => {
                print(dijetssum.get_events_by_version(version)?, is_json)?;
            }
        },
    }
    Ok(())
