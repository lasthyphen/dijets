...
use dijets_crypto::hash::HashValue;
use dijets_jellyfish_merkle::metrics::{
    DIJETS_JELLYFISH_INTERNAL_ENCODED_BYTES, DIJETS_JELLYFISH_LEAF_ENCODED_BYTES,
...
use dijetsdb::{
    metrics::DIJETS_STORAGE_ROCKSDB_PROPERTIES, schema::JELLYFISH_MERKLE_NODE_CF_NAME, DijetsDB,
};
...
pub fn run_benchmark(
    ...
    if db_dir.exists() {
        fs::remove_dir_all(db_dir.join("dijetsdb")).unwrap();
    }
    // create if not exists
    fs::create_dir_all(db_dir.clone()).unwrap();

    let db = DijetsDB::open(
        &db_dir,
        false,        /* readonly */
        prune_window, /* pruner */
    ...
    let db_size = DIJETS_STORAGE_ROCKSDB_PROPERTIES
        .with_label_values(&[
            JELLYFISH_MERKLE_NODE_CF_NAME,
            "dijets_rocksdb_live_sst_files_size_bytes",
        ])
        .get();
    let data_size = DIJETS_STORAGE_ROCKSDB_PROPERTIES
        .with_label_values(&[JELLYFISH_MERKLE_NODE_CF_NAME, "dijets_rocksdb_cf_size_bytes"])
        .get();
    let reads = DIJETS_JELLYFISH_STORAGE_READS.get();
    let leaf_bytes = DIJETS_JELLYFISH_LEAF_ENCODED_BYTES.get();
    let internal_bytes = DIJETS_JELLYFISH_INTERNAL_ENCODED_BYTES.get();
    println!(
        "created a DijetsDB til version {}, where {} accounts with avg blob size {} bytes exist.",
