...
use super::*;
use dijets_config::{config::NodeConfig, utils};
use dijets_types::account_address::HashAccountAddress;
#[cfg(test)]
...
fn start_test_storage_with_client() -> (JoinHandle<()>, dijets_temppath::TempPath, StorageClient) {
    let mut config = NodeConfig::random();
    let tmp_dir = dijets_temppath::TempPath::new();
    ...
    let db = Arc::new(DijetsDB::new_for_test(&tmp_dir));
