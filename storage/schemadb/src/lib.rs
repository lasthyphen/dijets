...
//! access to raw keys and values. This library also enforces a set of Dijets specific DB options,
...
use crate::{
    metrics::{
        DIJETS_SCHEMADB_BATCH_COMMIT_BYTES, DIJETS_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS,
        DIJETS_SCHEMADB_DELETES, DIJETS_SCHEMADB_GET_BYTES, DIJETS_SCHEMADB_GET_LATENCY_SECONDS,
        DIJETS_SCHEMADB_ITER_BYTES, DIJETS_SCHEMADB_ITER_LATENCY_SECONDS, DIJETS_SCHEMADB_PUT_BYTES,
    },
    ...
    S: Schema,
{
    ...
    fn next_impl(&mut self) -> Result<Option<(S::Key, S::Value)>> {
        let _timer = DIJETS_SCHEMADB_ITER_LATENCY_SECONDS
...
impl DB {
    ...
    pub fn get<S: Schema>(&self, schema_key: &S::Key) -> Result<Option<S::Value>> {
        let _timer = DIJETS_SCHEMADB_GET_LATENCY_SECONDS
            .with_label_values(&[S::COLUMN_FAMILY_NAME])
            .start_timer();
    ...
    pub fn write_schemas(&self, batch: SchemaBatch) -> Result<()> {
        let _timer = DIJETS_SCHEMADB_BATCH_COMMIT_LATENCY_SECONDS
        ...
        for (cf_name, rows) in &batch.rows {
            for (key, write_op) in rows {
                match write_op {
                    ...
                    WriteOp::Deletion => {
                        DIJETS_SCHEMADB_DELETES.with_label_values(&[cf_name]).inc();
                    }
                }
            }
        }
