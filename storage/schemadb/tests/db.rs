...
fn open_db(dir: &dijets_temppath::TempPath) -> DB {
...
fn open_db_read_only(dir: &dijets_temppath::TempPath) -> DB {
    DB::open_readonly(
        &dir.path(),
        "test",
        get_column_families(),
...
fn open_db_as_secondary(dir: &dijets_temppath::TempPath, dir_sec: &dijets_temppath::TempPath) -> DB {
...
struct TestDB {
    _tmpdir: dijets_temppath::TempPath,
    db: DB,
}

impl TestDB {
    fn new() -> Self {
        let tmpdir = dijets_temppath::TempPath::new();
...
fn test_open_as_secondary() {
    let tmpdir = dijets_temppath::TempPath::new();
    let tmpdir_sec = dijets_temppath::TempPath::new();
