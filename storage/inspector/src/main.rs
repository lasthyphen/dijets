...
use dijets_framework_releases::name_for_script;
...
fn print_head(db: &DijetsDB) -> Result<()> {
    let si = db
        .get_startup_info()
...
fn print_txn(db: &DijetsDB, version: u64) {
    let tx_list = db
...
fn print_account(db: &DijetsDB, addr: AccountAddress) {
    let maybe_blob = db
...
fn main() {
    ::dijets_logger::DijetsLogger::builder().build();
