...
use crate::{
    metrics::{
        DIJETS_STORAGE_OTHER_TIMERS_SECONDS, DIJETS_STORAGE_PRUNER_LEAST_READABLE_STATE_VERSION,
        DIJETS_STORAGE_PRUNE_WINDOW,
    },
    schema::{
...
use dijets_jellyfish_merkle::StaleNodeIndex;
use dijets_logger::prelude::*;
use dijets_types::transaction::Version;
...
/// The `Pruner` is meant to be part of a `DijetsDB` instance and runs in the background to prune old
/// data.
///
...
impl Pruner {
    ...
    pub fn new(db: Arc<DB>, historical_versions_to_keep: u64) -> Self {
        ...
        DIJETS_STORAGE_PRUNE_WINDOW.set(historical_versions_to_keep as i64);
        let worker_thread = std::thread::Builder::new()
            .name("dijetsdb_pruner".into())
...
impl Worker {
    ...
    fn record_progress(&mut self, least_readable_version: Version) {
        ...
        DIJETS_STORAGE_PRUNER_LEAST_READABLE_STATE_VERSION.set(least_readable_version as i64);
    }
