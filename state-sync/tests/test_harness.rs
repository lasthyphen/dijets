...
use dijets_mempool::mocks::MockSharedMempool;
...
pub struct StateSyncEnvironment {
    ...
    network_notifs_txs:
        HashMap<PeerId, dijets_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>,
    network_reqs_rxs:
        HashMap<PeerId, dijets_channel::Receiver<(PeerId, ProtocolId), PeerManagerRequest>>,
    ...
    fn setup_network_handles(
        ...
        if mock_network {
            ...
            for (idx, network) in networks.into_iter().enumerate() {
                ...
                let (connection_reqs_tx, _) = dijets_channel::new(QueueStyle::LIFO, 1, None);
...
fn setup_state_sync_config(
    ...
    let mut config = dijets_config::config::NodeConfig::default_for_validator();
