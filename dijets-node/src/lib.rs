...
use dijets_json_rpc::bootstrap_from_config as bootstrap_rpc;
use dijets_logger::{prelude::*, Logger};
use dijets_mempool::gen_mempool_reconfig_subscription;
use dijets_metrics::metric_server;
use dijets_time_service::TimeService;
use dijets_types::{
    account_config::dijets_root_address, account_state::AccountState, chain_id::ChainId,
...
pub fn start(config: &NodeConfig, log_file: Option<PathBuf>) {
    ...
    let mut logger = dijets_logger::Logger::new();
    logger
    ...
    info!(config = config, "Loaded DijetsNode config");
    ...
    R: ::rand::RngCore + ::rand::CryptoRng,
{
    ...
    let config_temp_path = dijets_temppath::TempPath::new();
    ...
    let mut builder = dijets_genesis_tool::validator_builder::ValidatorBuilder::new(
        &config_path,
    ...
    let test_config =
        dijets_genesis_tool::swarm_config::SwarmConfig::build_with_rng(&builder, &config_path, rng)
            .unwrap();
    ...
    println!("\tDijets root key path: {:?}", test_config.dijets_root_key_path);
    ...
    println!();
    println!("Dijets is running, press ctrl-c to exit");
    println!();
    ...
    let blob = db
        ...
        .get_account_state_with_proof_by_version(
            ...
            (&*db.reader)
                ...
                .expect("[dijets-node] failed fetching synced version."),
        )
        .expect("[dijets-node] failed to get Dijets root address account state")
        .0
        .expect("[dijets-node] missing Dijets root address account state");
    AccountState::try_from(&blob)
        .expect("[dijets-node] failed to convert blob to account state")
        .get_chain_id_resource()
        .expect("[dijets-node] failed to get chain ID resource")
        .expect("[dijets-node] missing chain ID resource")
        .chain_id()
}

fn setup_chunk_executor(db: DbReaderWriter) -> Box<dyn ChunkExecutor> {
    Box::new(Executor::<DijetsVM>::new(db))
}
...
pub fn setup_environment(node_config: &NodeConfig, logger: Option<Arc<Logger>>) -> DijetsHandle {
    ...
    let (dijets_db, db_rw) = DbReaderWriter::wrap(
        DijetsDB::open(
    ...
    let _simple_storage_service = start_storage_service_with_db(node_config, Arc::clone(&dijets_db));
    ...
    if let Some(genesis) = get_genesis_txn(node_config) {
        maybe_bootstrap::<DijetsVM>(&db_rw, genesis, genesis_waypoint)
    ...
    for (idx, network_config) in network_configs.into_iter().enumerate() {
        ...
        let (mempool_sender, mempool_events) = network_builder.add_protocol_handler(
            dijets_mempool::network::network_endpoint_config(MEMPOOL_NETWORK_CHANNEL_BUFFER_SIZE),
        );
    ...
    let rpc_runtime = bootstrap_rpc(node_config, chain_id, dijets_db.clone(), mp_client_sender);

    let mut consensus_runtime = None;
    let (consensus_to_mempool_sender, consensus_requests) = channel(INTRA_NODE_CHANNEL_BUFFER_SIZE);

    instant = Instant::now();
    let mempool = dijets_mempool::bootstrap(
        node_config,
        Arc::clone(&db_rw.reader),
