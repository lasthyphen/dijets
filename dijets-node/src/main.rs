...
use dijets_types::on_chain_config::VMPublishingOption;
use hex::FromHex;
...
fn main() {
    ...
    if args.test {
        ...
        dijets_node::load_test_environment(args.config, args.random_ports, publishing_option, rng);
    } else {
        let config = NodeConfig::load(args.config.unwrap()).expect("Failed to load node config");
        println!("Using node config {:?}", &config);
        dijets_node::start(&config, None);
    };
}
