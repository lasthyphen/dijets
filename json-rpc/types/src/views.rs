...
use dijets_crypto::hash::{CryptoHash, HashValue};
use dijets_transaction_builder::{error_explain, stdlib::ScriptCall};
use dijets_types::{
    account_config::{
...
pub enum EventDataView {
    ...
    VASPDomain {
        ...
        domain: DijetsIdVaspDomainIdentifier,
...
impl MetadataView {
    ...
    pub fn with_dijets_root(&mut self, dijets_root: &AccountState) -> Result<()> {
        if let Some(vm_publishing_option) = dijets_root.get_vm_publishing_option()? {
        ...
        if let Some(dijets_version) = dijets_root.get_dijets_version()? {
            self.dijets_version = Some(dijets_version.major);
        }
        if let Some(limit) = dijets_root.get_resource::<Limit>()? {
...
mod tests {
    ...
    use dijets_types::{contract_event::ContractEvent, event::EventKey};
