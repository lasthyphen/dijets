...
use anyhow::Result;
use dijets_types::{
    account_address::AccountAddress, account_config::dijets_root_address,
...
pub fn get_metadata(
    ...
    if version == ledger_version {
        if let Some(dijets_root) = get_account_state(db, dijets_root_address(), version)? {
            metadata_view.with_dijets_root(&dijets_root)?;
        }
    }
    Ok(metadata_view)
}
...
pub fn get_currencies(
    ...
    if let Some(account_state) = get_account_state(db, dijets_root_address(), ledger_version)? {
        Ok(account_state
