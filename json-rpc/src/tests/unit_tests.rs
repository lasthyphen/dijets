...
use dijets_client::{views::TransactionDataView, BlockingClient, MethodRequest};
use dijets_config::{config::DEFAULT_CONTENT_LENGTH_LIMIT, utils};
use dijets_crypto::{ed25519::Ed25519PrivateKey, hash::CryptoHash, HashValue, PrivateKey, Uniform};
use dijets_metrics::get_all_metrics;
use dijets_types::{
...
fn test_json_rpc_protocol_invalid_requests() {
    ...
    let calls = vec![
        (
            ...
            json!({
                ...
                "id": 1,
                "jsonrpc": "2.0",
                "dijets_chain_id": ChainId::test().id(),
                "dijets_ledger_timestampusec": timestamp,
                "dijets_ledger_version": version
            }),
        ),
        (
    ...
    for (name, request, expected) in calls {
        ...
        assert_eq!(
            headers.get("X-Dijets-Chain-Id").unwrap().to_str().unwrap(),
            "4"
        );
        assert_eq!(
            headers
                .get("X-Dijets-Ledger-Version")
                .unwrap()
                .to_str()
                .unwrap(),
            version.to_string()
        ...
        assert_eq!(
            headers
                .get("X-Dijets-Ledger-TimestampUsec")
                .unwrap()
                .to_str()
                .unwrap(),
            timestamp.to_string()
...
fn test_metrics() {
    ...
    let client = reqwest::blocking::ClientBuilder::new()
        .user_agent("dijets-client-sdk-python / 2.11.15")
        .build()
        .expect("Client::new()");
    ...
    let expected_metrics = vec![
        // rpc request count
        "dijets_client_service_rpc_requests_count{type=single}",
        "dijets_client_service_rpc_requests_count{type=batch}",
        // rpc request latency
        "dijets_client_service_rpc_request_latency_seconds{type=single}",
        "dijets_client_service_rpc_request_latency_seconds{type=batch}",
        // method request count
        "dijets_client_service_requests_count{method=get_currencies,result=success,sdk_lang=python,sdk_ver=2.11.15,type=single}",
        // method latency
        "dijets_client_service_method_latency_seconds{method=get_currencies,type=single}",
        "dijets_client_service_method_latency_seconds{method=get_currencies,type=batch}",
        // invalid params
        "dijets_client_service_invalid_requests_count{errortype=invalid_params,method=get_currencies,sdk_lang=python,sdk_ver=2.11.15,type=single}",
    ];
...
fn test_sdk_info_from_user_agent() {
    ...
    assert_eq!(
        sdk_info_from_user_agent(Some("dijets-client-sdk-python / 0.12.223")),
        SdkInfo::default()
    );
    assert_eq!(
        sdk_info_from_user_agent(Some("dijets-client-sdk-python / 105.12.22")),
        SdkInfo::default()
    );

    // Valid user agents:
    assert_eq!(
        sdk_info_from_user_agent(Some("dijets-client-SdK-JaVa/ 3.21.09")),
        SdkInfo {
    ...
    assert_eq!(
        sdk_info_from_user_agent(Some("dijets-client-SdK-cpp/3.21.09")),
        SdkInfo {
    ...
    assert_eq!(
        sdk_info_from_user_agent(Some("dijets-client-sdk-python / 0.1.22")),
        SdkInfo {
...
fn get_first_account_from_mock_db(mock_db: &MockDijetsDB) -> AccountAddress {
    *mock_db
        .all_accounts
        .keys()
        .next()
...
fn get_first_state_proof_from_mock_db(mock_db: &MockDijetsDB) -> AccountStateWithProof {
    mock_db
