...
use dijets_config::config::StreamConfig;
...
use crate::{
    ...
    tests::utils::{mock_db, MockDijetsDB},
};

use dijets_json_rpc_types::stream::response::StreamJsonRpcResponse;
...
pub async fn connect_to_ws(
    ...
    let ws_client = warp::test::ws()
        .path("/v1/stream/ws")
        .header("user-agent", "dijets-client-sdk-python / 0.1.22")
