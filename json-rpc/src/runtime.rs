...
use crate::{
    ...
    response::{JsonRpcResponse, X_DIJETS_CHAIN_ID, X_DIJETS_TIMESTAMP_USEC_ID, X_DIJETS_VERSION_ID},
    stream_rpc,
    util::{sdk_info_from_user_agent, SdkInfo},
};
use anyhow::{ensure, Result};
use dijets_config::config::{NodeConfig, RoleType, StreamConfig};
use dijets_json_rpc_types::Method;
use dijets_logger::{debug, Schema};
use dijets_mempool::MempoolClientSender;
use dijets_types::{chain_id::ChainId, ledger_info::LedgerInfoWithSignatures};
    ...
    ($trace_id: expr, $resp: expr, $is_batch: expr) => {
        ...
        match &$resp.error {
            Some(error) if is_internal_error(&error.code) => {
                dijets_logger::error!(log)
            }
            _ => dijets_logger::trace!(log),
        }
    };
}
...
async fn rpc_request_handler(
    ...
    match dijets_json_rpc_types::request::JsonRpcRequest::from_value(request) {
        Ok(request) => {
