...
use dijets_types::account_state_blob::AccountStateWithProof;
...
pub fn request_fuzzer(json_request: serde_json::Value) {
    ...
    let db = tests::utils::MockDijetsDB {
        version: 1,
    ...
    rt.spawn(async move {
        if let Some((_, cb)) = mp_events.next().await {
            cb.send(Ok((
                dijets_types::mempool_status::MempoolStatus::new(
                    dijets_types::mempool_status::MempoolStatusCode::Accepted,
                ),
                None,
            )))
            .unwrap();
