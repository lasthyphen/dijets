...
use dijets_sdk::{
    ...
    types::{
        ...
        dijets_id_identifier::DijetsIdVaspDomainIdentifier,
        event::EventKey,
        ledger_info::LedgerInfoWithSignatures,
        on_chain_config::DIJETS_MAX_KNOWN_VERSION,
...
impl PublicUsageTest for BlockMetadata {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        ...
        let dijets_chain_id = &resps[0]["dijets_chain_id"];
        let dijets_ledger_timestampusec = &resps[0]["dijets_ledger_timestampusec"];
        let dijets_ledger_version = &resps[0]["dijets_ledger_version"];

        assert_eq!(dijets_chain_id, &resps[1]["dijets_chain_id"]);
        assert_eq!(
            dijets_ledger_timestampusec,
            &resps[1]["dijets_ledger_timestampusec"]
        );
        assert_eq!(dijets_ledger_version, &resps[1]["dijets_ledger_version"]);

        // parse metadata
        assert_eq!(&metadata["chain_id"], dijets_chain_id);
        assert_eq!(&metadata["timestamp"], dijets_ledger_timestampusec);
        assert_eq!(&metadata["version"], dijets_ledger_version);
        ...
        assert_eq!(metadata["dijets_version"], 3);
        assert_eq!(metadata["dual_attestation_limit"], 1000000000);
        assert_ne!(dijets_ledger_timestampusec, 0);
        assert_ne!(dijets_ledger_version, 0);
...
impl PublicUsageTest for OldMetadata {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        ...
        assert_eq!(metadata["dijets_version"], json!(null));
        Ok(())
    }
}
...
impl PublicUsageTest for UnknownAccountRoleType {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        let env = JsonRpcTestHelper::new(ctx.url().to_owned());
        let address = format!("{:x}", dijets_sdk::types::account_config::dijets_root_address());
        ...
        assert_eq!(
            ...
            json!({
                ...
                "version": resp.dijets_ledger_version,
            }),
        );
        Ok(())
    }
...
impl PublicUsageTest for PeerToPeerWithEvents {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        let env = JsonRpcTestHelper::new(ctx.url().to_owned());
        let factory = ctx.transaction_factory().with_dijets_version(0); // Force use of Scripts not ScriptFunctions

        let prev_ledger_version = env.send("get_metadata", json!([])).dijets_ledger_version;
        ...
        assert_eq!(
            true,
            version > prev_ledger_version && version <= resp.dijets_ledger_version
        );
    ...
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        ...
        assert_eq!(
            ...
            json!({
                ...
                "location": "00000000000000000000000000000001::DijetsAccount",
                "type": "move_abort"
            })
        );
...
impl PublicUsageTest for GetTransactionsWithProofs {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        ...
        // We test 2 cases:
        //      1. base_version + limit > resp.dijets_ledger_version
        //      2. base_version + limit < resp.dijets_ledger_version
        for base_version in &[resp.dijets_ledger_version, 0] {
            ...
            let epoch_proofs: dijets_sdk::types::epoch_change::EpochChangeProof =
            ...
            let raw_hex_li = proofs["ledger_info_to_transaction_infos_proof"]
                .as_str()
                .unwrap();
            let li_to_tip: dijets_sdk::types::proof::TransactionAccumulatorRangeProof =
            ...
            let txs_infos: Vec<dijets_sdk::types::transaction::TransactionInfo> =
...
impl AdminTest for AddAndRemoveVaspDomain {
    fn run<'t>(&self, ctx: &mut AdminContext<'t>) -> Result<()> {
        ...
        // add domain
        let domain = DijetsIdVaspDomainIdentifier::new("dijets")
            .unwrap()
            .as_str()
            .as_bytes()
            .to_vec();
        ...
        let resp = env.send("get_account", json!([address]));
        let result = resp.result.unwrap();
        assert_eq!(result["role"]["vasp_domains"], json!(["dijets"]),);
...
impl PublicUsageTest for GetResourcesTest {
    fn run<'t>(&self, ctx: &mut PublicUsageContext<'t>) -> Result<()> {
        ...
        let currency_info = resources
            .get("0x1::Dijets::CurrencyInfo<0x1::XDX::XDX>")
            .unwrap();
        assert_eq!(
            currency_info,
            &json!({
        ...
        assert_eq!(
            ...
            json!({
                ...
                "script": {
                    ...
                    "arguments": [
                        "{U64: 0}",
                        format!("{{U8Vector: 0x{}}}", new_key.public_key().to_string()),
                    ],
                    "code": hex::encode(script.code()),
...
pub struct UpgradeDijetsVersion;

impl Test for UpgradeDijetsVersion {
    fn name(&self) -> &'static str {
        "jsonrpc::upgrade-dijets-version"
    }
}

impl AdminTest for UpgradeDijetsVersion {
    fn run<'t>(&self, ctx: &mut AdminContext<'t>) -> Result<()> {
        let env = JsonRpcTestHelper::new(ctx.chain_info().json_rpc().to_owned());
        let factory = ctx.chain_info().transaction_factory();
        let txn = ctx
            ...
            .sign_with_transaction_builder(
                factory.update_dijets_version(0, DIJETS_MAX_KNOWN_VERSION.major + 1),
            );
        env.submit_and_wait(&txn);
        Ok(())
    }
