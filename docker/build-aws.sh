...
while [[ "$1" =~ ^- ]]; do case $1 in
  --build-all )
    BUILD_PROJECTS=(dijets-validator dijets-cluster-test dijets-init dijets-faucet dijets-safety-rules dijets-tools)
    ;;
  ...
  --build-all-cti )
    BUILD_PROJECTS=(dijets-validator dijets-cluster-test dijets-init dijets-safety-rules)
    ;;
  --build-cluster-test )
    BUILD_PROJECTS=(dijets-cluster-test)
    ;;
  --build-validator )
    BUILD_PROJECTS=(dijets-validator)
    ;;
  --build-init )
    BUILD_PROJECTS=(dijets-init)
    ;;
  --build-faucet )
      BUILD_PROJECTS=(dijets-faucet)
    ;;
  --build-safety-rules )
    BUILD_PROJECTS=(dijets-safety-rules)
    ;;
  --build-tools )
    BUILD_PROJECTS=(dijets-tools)
    ;;
  --build-forge )
    BUILD_PROJECTS=(dijets-forge dijets-validator dijets-init dijets-faucet dijets-safety-rules dijets-tools)
    ;;
  --version )
    shift;
    if [[ "$1" =~ ^pull ]]; then
