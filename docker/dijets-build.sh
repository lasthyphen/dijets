...
DOCKERFILE=$1
if [ -z "$DIJETS_BUILD_TAG" ]; then
  TAGS="--tag $2"
else
  TAGS="--tag $2 --tag $DIJETS_BUILD_TAG"
fi
...
then
  echo "Usage dijets-build.sh <Docker_file> <Local_tag> <Other_args>"
fi
...
if [ "$1" = "--incremental" ]; then
  ...
  if ! (docker ps | grep "$CONTAINER" > /dev/null); then
    if (docker ps -a | grep "$CONTAINER" > /dev/null); then
      ...
      mkdir -p "$OUT_TARGET"
      docker run -u root -d -t -v "$OUT_TARGET:/out-target" -v "$DIR/..:/dijets" -w /dijets --name "$CONTAINER" "$IMAGE" sh > /dev/null
  ...
  sed -e "s+$DOCKERFILE_BUILD_RE+RUN mkdir -p /dijets/target/release/; cp target-out-docker/* /dijets/target/release/+g" \
