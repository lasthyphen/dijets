...
function usage {
  echo "Usage:"
  echo "Copies a dijets dockerhub image to another docker location (server / repo / tag )"
  ...
  echo "-g target org, defaults to 'dijets'"
...
for image in "${TARGET_IMAGES[@]}"; do
  ...
  docker pull --disable-content-trust=false docker.io/dijets/"$renamed_image":"$DOCKERHUB_TAG"
done
...
for image in "${TARGET_IMAGES[@]}"; do
  renamed_image="${image//-/_}"
  docker tag dijets/"$renamed_image":"$DOCKERHUB_TAG" "$TARGET_REPO"/"$TARGET_ORG"/"$renamed_image":"$OUTPUT_TAG"
done
